...
MACRO(TULIP_SET_CXX_COMPILER_FLAG_DEBUG flag)
  STRING(FIND "${CMAKE_CXX_FLAGS_DEBUG}" "${flag}" FLAG_POS)
  IF (${FLAG_POS} EQUAL -1)
    SET(CMAKE_CXX_FLAGS_DEBUG "${CMAKE_CXX_FLAGS_DEBUG} ${flag}")
  ENDIF()
ENDMACRO(TULIP_SET_CXX_COMPILER_FLAG_DEBUG)

MACRO(TULIP_SET_C_COMPILER_FLAG_DEBUG flag)
  STRING(FIND "${CMAKE_C_FLAGS_DEBUG}" "${flag}" FLAG_POS)
  IF (${FLAG_POS} EQUAL -1)
    SET(CMAKE_C_FLAGS_DEBUG "${CMAKE_C_FLAGS_DEBUG} ${flag}")
  ENDIF()
ENDMACRO(TULIP_SET_C_COMPILER_FLAG_DEBUG)

MACRO(TULIP_SET_CXX_COMPILER_FLAG_RELWITHDEBINFO flag)
  STRING(FIND "${CMAKE_CXX_FLAGS_RELWITHDEBINFO}" "${flag}" FLAG_POS)
  IF (${FLAG_POS} EQUAL -1)
    SET(CMAKE_CXX_FLAGS_RELWITHDEBINFO "${CMAKE_CXX_FLAGS_RELWITHDEBINFO} ${flag}")
  ENDIF()
ENDMACRO(TULIP_SET_CXX_COMPILER_FLAG_RELWITHDEBINFO)

MACRO(TULIP_SET_C_COMPILER_FLAG_RELWITHDEBINFO flag)
  STRING(FIND "${CMAKE_C_FLAGS_RELWITHDEBINFO}" "${flag}" FLAG_POS)
  IF (${FLAG_POS} EQUAL -1)
    SET(CMAKE_C_FLAGS_RELWITHDEBINFO "${CMAKE_C_FLAGS_RELWITHDEBINFO} ${flag}")
  ENDIF()
ENDMACRO(TULIP_SET_C_COMPILER_FLAG_RELWITHDEBINFO)
...
MACRO(TULIP_SET_COMPILER_OPTIONS)
  ...
  # Prevent ccache cache miss by using relative paths in debug info rather than absolute
  IF(TULIP_USE_CCACHE)
    TULIP_SET_CXX_COMPILER_FLAG_RELWITHDEBINFO("-fdebug-prefix-map=${CMAKE_BINARY_DIR}=.")
    TULIP_SET_CXX_COMPILER_FLAG_DEBUG("-fdebug-prefix-map=${CMAKE_BINARY_DIR}=.")
  ENDIF(TULIP_USE_CCACHE)
  
  # Ninja fails to color console output
  IF(${CMAKE_GENERATOR} MATCHES "Ninja")
    IF ("${CMAKE_CXX_COMPILER_ID}" STREQUAL "GNU")
      TULIP_SET_CXX_COMPILER_FLAG("-fdiagnostics-color=always")
    ELSEIF ("${CMAKE_CXX_COMPILER_ID}" STREQUAL "Clang")
      TULIP_SET_CXX_COMPILER_FLAG("-fcolor-diagnostics")
    ENDIF()
  ENDIF()
