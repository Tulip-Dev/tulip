...
void IteratorTest::testFilterIterator() {
  std::set<tlp::node> nodesSet;
  for (tlp::node n : tlp::filterIterator(graph->getNodes(), [](const tlp::node &n) { return n.id % 2 == 0; })) {
    nodesSet.insert(n);
  }
...
void IteratorTest::testIteratorMap() {
  tlp::iteratorMap(graph->getNodes(), [this](const tlp::node &n) { idMetric->setNodeValue(n, n.id + 1); });
  for (tlp::node n : graph->getNodes()) {
...
void IteratorTest::testIteratorReduce() {
  ...
  unsigned int val = tlp::iteratorReduce(graph->getNodes(), 0u, [](unsigned int curVal, const tlp::node &n) { return curVal + n.id; });
