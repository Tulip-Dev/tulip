...
//==========================================================
template <typename PropType> bool BasicLayoutTest::computeProperty(const std::string &algorithm, const std::string &graphType, PropType *prop) {
...
void BasicLayoutTest::testFastOverlapRemoval() {
  ...
  string errorMsg;
  bool result = graph->applyPropertyAlgorithm("Fast Overlap Removal", &layout, errorMsg, nullptr, &ds);
  CPPUNIT_ASSERT(result);
}
