...
//==========================================================
template <typename PropType> bool BasicPluginsTest::computeProperty(const std::string &algorithm, const std::string &graphType, PropType *prop) {
...
void BasicPluginsTest::testExportImportTLPB() {
  ...
  Graph *g = importGraph("TLP Import", ds, nullptr, graph);
  CPPUNIT_ASSERT(g == graph);
  std::ostream *os = tlp::getOutputFileStream("tlpb_importexport_test.tlpb", std::ios::out | std::ios::binary);
...
void BasicPluginsTest::testImportGEXF() {
  ...
  const char *gexf_files[] = {
      "data/basic.gexf",      "data/data.gexf",      "data/hierarchy1.gexf", "data/hierarchy2.gexf",       "data/hierarchy3.gexf",
      "data/hierarchy4.gexf", "data/phylogeny.gexf", "data/viz.gexf",        "data/WebAtlas_EuroSiS.gexf", nullptr};
...
void BasicPluginsTest::testMultipleEdgeSelection() {
  BooleanProperty selection(graph);
  bool result = computeProperty<BooleanProperty>("Multiple Edges Selection", "Planar Graph", &selection);
  CPPUNIT_ASSERT(result);
  for (node n : graph->getNodes()) {
    CPPUNIT_ASSERT(selection.getNodeValue(n) == false);
  }
