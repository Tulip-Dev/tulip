...
QList<QWidget*> %ProjectName:
c%::configurationWidgets() const {
...
void %ProjectName:
c%::setState(const tlp::DataSet& data) {
...
QGraphicsView* %ProjectName:
c%::graphicsView() const {
  return _graphicsView;
}

QGraphicsItem* %ProjectName:
c%::centralItem() const {
...
void %ProjectName:
c%::interactorsInstalled(const QList<tlp::Interactor*>& interactors) {
}

void %ProjectName:
c%::currentInteractorChanged(tlp::Interactor*) {
}

void %ProjectName:
c%::graphChanged(tlp::Graph*) {
  // Point the visualization to the new graph
}

@if "%BaseView%" == "View"
void %ProjectName:
c%::graphDeleted(tlp::Graph* parentGraph) {
...
void %ProjectName:
c%::fillContextMenu(QMenu* menu,const QPointF& point) {
