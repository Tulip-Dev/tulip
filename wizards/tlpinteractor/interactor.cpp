...
void %ProjectName:
c%Composite::construct() {
push_back(new %ProjectName:c%Component());
  // Push other components here
}

bool %ProjectName:
c%Composite::isCompatible(const std::string& viewName) {
...
QWidget* %ProjectName:
c%Composite::configurationWidget() const {
  return new QLabel(trUtf8("My interactor help string"));
}

unsigned int %ProjectName:
c%Composite::priority() const {
  return 0;
}

QCursor %ProjectName:
c%Composite::cursor() const {
  return QCursor();
}

void %ProjectName:
c%Composite::undoIsDone() {
}
...
void %ProjectName:
c%Component::viewChanged(tlp::View* newView) {
}

bool %ProjectName:
c%Component::eventFilter(QObject* obj, QEvent* event) {
  // User input handling
  return false;
}
...
bool %ProjectName:
c%Component::draw(tlp::GlMainWidget* glMainWidget) {
  // Do some drawing here
  return true;
}

bool %ProjectName:
c%Component::compute(tlp::GlMainWidget* glMainWidget) {
