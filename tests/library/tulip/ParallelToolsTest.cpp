...
void ParallelToolsTest::tearDown() { delete _graph; }

void ParallelToolsTest::testParallelMapIndices() {
  std::vector<unsigned int> v(100);
  tlp::TLP_PARALLEL_MAP_INDICES(v.size(),
                                [&](unsigned int i) { v[i] = 2 * i; });
...
void ParallelToolsTest::testParallelMapNodes() {
  ...
  TLP_PARALLEL_MAP_NODES(_graph,
                         [&](const tlp::node &n) { deg[n] = _graph->deg(n); });
...
void ParallelToolsTest::testParallelMapNodesAndIndices() {
  ...
  TLP_PARALLEL_MAP_NODES_AND_INDICES(
      _graph, [&](const tlp::node &n, unsigned int i) { nodes[i] = n; });
  unsigned int i = 0;
  for (auto n : _graph->nodes()) {
...
void ParallelToolsTest::testParallelMapEdges() {
  ...
  TLP_PARALLEL_MAP_EDGES(_graph, [&](const tlp::edge &e) {
    selfLoop[e] = _graph->source(e) == _graph->target(e);
  });
  for (auto e : _graph->edges()) {
    CPPUNIT_ASSERT_EQUAL(selfLoop[e],
                         int(_graph->source(e) == _graph->target(e)));
  }
}
...
void ParallelToolsTest::testParallelMapEdgesAndIndices() {
  ...
  TLP_PARALLEL_MAP_EDGES_AND_INDICES(
      _graph, [&](const tlp::edge &e, unsigned int i) { edges[i] = e; });
  unsigned int i = 0;
  for (auto e : _graph->edges()) {
...
void ParallelToolsTest::testCriticalSection() {
  ...
  TLP_PARALLEL_MAP_NODES(_graph, [&](const tlp::node &n) {
    ...
    TLP_LOCK_SECTION(maxDeg) { maxDegPar = std::max(deg, maxDegPar); }
...
void ParallelToolsTest::testNumberOfThreads() {
  ...
  tlp::TLP_PARALLEL_MAP_INDICES(nbThreads, [&](unsigned int i) {
    tids[i] = tlp::ThreadManager::getThreadNumber();
  });
