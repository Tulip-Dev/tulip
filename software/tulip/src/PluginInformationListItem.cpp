...
PluginInformationListItem::PluginInformationListItem(PluginInformation info,
                                                     QWidget *parent)
  ...
  if (!versionInfo.isValid) {
    ...
    if (info.availableVersion.isValid &&
        info.availableVersion.version != versionInfo.version) {
      _ui->statusIcon->setPixmap(
          QPixmap(":/tulip/app/icons/16/package-upgrade.png"));
      _ui->installFrame->show();
    }

    else
      _ui->statusIcon->setPixmap(
          QPixmap(":/tulip/app/icons/16/package-installed-updated.png"));
...
PluginInformationListItem::~PluginInformationListItem() { delete _ui; }

QWidget *PluginInformationListItem::description() { return _ui->desc; }
...
void PluginInformationListItem::install() {
  ...
  PluginManager::markForInstallation(_info.name, this,
...
void PluginInformationListItem::enterEvent(QEvent *) { emit focused(); }

void PluginInformationListItem::downloadProgress(qint64 received,
                                                 qint64 total) {
