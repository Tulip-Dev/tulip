...
void TulipApp::startTulip() {
  ...
  if(ControllerPluginManager::objMap().empty()){
  ...
  if(ControllerPluginManager::objMap().size() < 2) {
    controllerAutoLoad=false;
    string name = ControllerPluginManager::objMap().begin()->first;
...
string name;
  if(fromMenu) {
    name = ControllerPluginManager::objMap().begin()->first;
}
  else {
...
bool TulipApp::createController(const string &name,const string &graphName) {
  if(!controllerAutoLoad){
    ...
    Controller *newController = ControllerPluginManager::getPluginObject(name, NULL);
...
void TulipApp::fileOpen(string *plugin, QString &s) {
  ...
  if (!cancel) {
    ...
    if (progressBar->state()==TLP_CANCEL || !result ) {
      ...
      for (it=ControllerPluginManager::objMap().begin();it != ControllerPluginManager::objMap().end();++it) {
      ...
      if(controllersName.size()==1){
        ...
        if(!ControllerPluginManager::pluginExists(controllerName)) {
...
// In this case doesn't add sub menu in new menu
  if(ControllerPluginManager::objMap().size() > 1) {
    ...
    for (PluginManager<Controller, ControllerContext*>::ObjectCreator::const_iterator it=ControllerPluginManager::objMap().begin();it != ControllerPluginManager::objMap().end();++it) {
