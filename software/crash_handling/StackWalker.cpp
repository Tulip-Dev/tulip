...
void StackWalkerGCC::printCallStack(std::ostream &os, unsigned int maxDepth) {
  ...
  for (; i < size; ++i) {
    char *mangled_name = 0, *runtime_offset = 0, *offset_end = 0, *runtime_addr = 0, *runtime_addr_end = 0, *dsoName = 0;
    ...
    *runtime_addr_end = '\0';
    ...
    if (mangled_name && runtime_offset && mangled_name < runtime_offset) {
...
void StackWalkerMSVC::printCallStack(std::ostream &os, unsigned int maxDepth) {
  ...
  for (frame = 0;; frame++) {
    result = StackWalk(machine, process, thread, &stack, context, nullptr, SymFunctionTableAccess, SymGetModuleBase, nullptr);
