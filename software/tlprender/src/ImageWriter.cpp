...
ImageWriter::ImageWriter(const GLubyte *imagebuffer, const int width, const int height)
    : buffer(imagebuffer), width(width), height(height), title(""), software(""), source(""), layout(""), comment(""), titleState(false),
      softwareState(false), sourceState(false), layoutState(false), commentState(false) {
}
...
ImageWriter *ImageWriter::getImageWriter(string &type, const GLubyte *buffer, const int width, const int height) {
  ...
  if (type == "png")
    return new tlprender::PNGWriter(buffer, width, height);
  else
#endif
#ifdef HAVE_LIBJPEG
      if (type == "jpeg")
    return new tlprender::JPEGWriter(buffer, width, height);
  else
#endif
  {
    return nullptr;
