...
CORE_FILES=$(find ${TLP_SRC_DIR}/library/tulip-core/src -name '*.cpp' -and ! -name 'TulipFontAwesome.cpp')
...
em++ -s ALLOW_MEMORY_GROWTH=1 -std=c++0x -DNDEBUG -I${TLP_SRC_DIR}/thirdparty -I${TLP_SRC_DIR}/thirdparty/gzstream -I${TLP_SRC_DIR}/thirdparty/yajl/src ${OGDF_INCLUDES} -I${TLP_SRC_DIR}/library/tulip-core/include/ -I${TLP_INSTALL_DIR}/include/ ${CORE_FILES} ${OGDF_FILES} ${METRIC_FILES} ${SELECTION_FILES} ${SIZES_FILES} ${TEST_FILES} ${LAYOUT_FILES} ${CLUSTERING_FILES} ${EDGE_BUNDLING_FILES} -o libtulip.o
