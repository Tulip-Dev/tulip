...
if [ "$TRAVIS_BUILD_THIRD_PARTY_ONLY" != "ON" ]; then
...
cmake .. -DCMAKE_BUILD_TYPE=Release -DCMAKE_INSTALL_PREFIX=$PWD/install ${CLANG_COMPILER_DEFINES} ${PYTHON_DEFINE} -DTRAVIS_BUILD_THIRDPARTY_ONLY=${TRAVIS_BUILD_THIRD_PARTY_ONLY} -DTULIP_BUILD_CORE_ONLY=${TULIP_BUILD_CORE_ONLY} -DTULIP_BUILD_DOC=${TULIP_BUILD_DOC} -DTULIP_BUILD_TESTS=${TULIP_BUILD_TESTS} -DTULIP_USE_CCACHE=$TULIP_USE_CCACHE ${TULIP_GUI_DEFINES}
# compile Tulip
