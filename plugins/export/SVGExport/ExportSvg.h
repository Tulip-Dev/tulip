...
class ExportSvg : public ExportInterface {
  ...
  bool createEdge(
      ...
      const std::vector<tlp::Coord> &bends, const QString &color,
...
public:
  ExportSvg(tlp::PluginProgress *pp, std::ostream &os,
            const bool autoformatting, const bool woff2);
  ...
  bool writeMetaGraph(const int transform_X, const int transform_Y,
  ...
  bool exportEdge(
      ...
      const unsigned id_tgt_shape,
      const std::vector<tlp::Coord> &edgeVertice) override;
  bool exportEdgeExtremity(
      const unsigned id_src_shape, const unsigned id_tgt_shape,
      const tlp::EdgeExtremityShape::EdgeExtremityShapes src_anchor_shape_type,
      const tlp::EdgeExtremityShape::EdgeExtremityShapes tgt_anchor_shape_type,
      const tlp::Color &color, const unsigned int id_src_gradient,
      const unsigned int id_tgt_gradient, const std::string &iconName) override;
  bool addLabel(const std::string &type, const std::string &label,
                const tlp::Color &labelcolor, const tlp::Coord &coord,
                const unsigned &fsize, const tlp::Size &size) override;
  bool addBorder(const tlp::Color &borderColor,
                 const double borderwidth) override;
