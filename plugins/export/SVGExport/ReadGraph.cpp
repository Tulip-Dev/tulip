...
static bool treatEdges(Graph *graph, tlp::PluginProgress *pp, ExportInterface& exportint, unsigned &i, const int nb_elements, tlp::SizeProperty *sizes, tlp::ColorProperty *colors, tlp::LayoutProperty *layout, tlp::IntegerProperty *shape,tlp::IntegerProperty *srcanchorshape, tlp::IntegerProperty *tgtanchorshape, tlp::StringProperty *label,tlp::ColorProperty *labelcolor, bool edge_color_interpolation, bool edge_size_interpolation, bool edge_extremities, const bool edge_labels, IntegerProperty* fontsize, StringProperty* iconName) {
  ...
  forEach(e, graph->getEdges()) {
    ...
    if(src_anchor_shape_type != EdgeExtremityShape::None || tgt_anchor_shape_type != EdgeExtremityShape::None) {
      ret = exportint.exportEdgeExtremity(id_src_shape, id_tgt_shape, src_anchor_shape_type, tgt_anchor_shape_type, colors->getEdgeValue(e), id_src_grad, id_tgt_grad, iconName->getEdgeValue(e));
...
static bool treatNodes(Graph *graph, tlp::PluginProgress *pp, ExportInterface& exportint, unsigned &i, const int nb_elements,
                       ...
                       tlp::ColorProperty *labelcolor, tlp::ColorProperty *bordercolor, std::vector<tlp::node> &metanodeVertices, const
                       bool node_labels, IntegerProperty* fontsize, StringProperty* iconName) {
  ...
  forEach(n, graph->getNodes()) {
    ...
    ret = exportint.addShape(static_cast<NodeShape::NodeShapes>(shape->getNodeValue(n)), c, s, bordercolor->getNodeValue(n), borderwidth->getNodeValue(n), colors->getNodeValue(n), iconName->getNodeValue(n));
...
bool ReadGraph::readGraph(Graph *graph, tlp::DataSet *ds, tlp::PluginProgress *pp, ExportInterface& exportint) {
  ...
  // Analysing edges
  ret = treatEdges(graph, pp, exportint, i,  nb_elements, sizes, colors, layout, shape, srcanchorshape, tgtanchorshape, label, labelcolor, edge_color_interpolation, edge_size_interpolation, edge_extremities, edge_labels, fontsize, fontAwesomeIconName);
  ...
  ret =  treatNodes(graph, pp, exportint, i,  nb_elements, sizes, colors, layout, shape, rotation, borderwidth, label, labelcolor, bordercolor, metanodeVertices, node_labels, fontsize, fontAwesomeIconName);
  ...
  while(!metanodeVertices.empty()) {
    ...
    for(vector<node>::const_iterator it = metanodeVertices.begin(); it != metanodeVertices.end(); ++it) {
      ...
      ret = treatEdges(metagraph, pp, exportint, i,  nb_elements, sizes, colors, layout, shape, srcanchorshape, tgtanchorshape, label, labelcolor, edge_color_interpolation, edge_size_interpolation, edge_extremities, metanode_labels,fontsize,fontAwesomeIconName);
      ...
      ret = treatNodes(metagraph, pp, exportint, i,  nb_elements, sizes, colors, layout, shape, rotation, borderwidth, label, labelcolor, bordercolor, subMetanodeVertices, metanode_labels,fontsize, fontAwesomeIconName);
