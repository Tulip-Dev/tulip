...
public:
  PLUGININFORMATION("GML Export", "Auber David", "31/07/2001", "Exports a Tulip graph in a file using the GML format (used by Graphlet).<br/>See "
  ...
  bool exportGraph(ostream &os) {
    ...
    if (itN->hasNext()) {
      ...
      for (; itN->hasNext();) {
        ...
        os << "width 0.12" << endl;
        os << "fill \"#" << hex << setfill('0') << setw(2) << (int)colors->getNodeValue(itn).getR() << hex << setfill('0') << setw(2)
           << (int)colors->getNodeValue(itn).getG() << hex << setfill('0') << setw(2) << (int)colors->getNodeValue(itn).getB() << "\"" << endl;
