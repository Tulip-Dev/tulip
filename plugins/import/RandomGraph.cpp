...
public:
    ...
    bool importGraph() override {
        ...
        if(density_g > 1){
            if(directed)
                pluginProgress->setError(string("Error: For ")+std::to_string(nbNodes)+ string(" nodes, the maximum number of edges is ")+std::to_string(nbNodes * (nbNodes - 1)));
            else
                pluginProgress->setError(string("Error: For ")+std::to_string(nbNodes)+ string(" nodes, the maximum number of edges is ")+std::to_string(nbNodes * (nbNodes - 1) / 2));
            return false;
        }
        ...
        while (myGraph.size() < nb_disctint_pairs_needed) {
            if (myGraph.size() % nbNodes == 1 && (pluginProgress->progress(myGraph.size(), nb_disctint_pairs_needed) != TLP_CONTINUE))
