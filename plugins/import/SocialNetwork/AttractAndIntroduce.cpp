...
public:
  PLUGININFORMATION(
      "Attract And Introduce Model", "Arnaud Sallabery & Patrick Mary",
      "25/03/2014",
  ...
  bool importGraph() override {
    ...
    if (alpha < 0 || alpha > 1) {
      pluginProgress->setError(
          "alpha is not a percentage,\nit is not between [0, 1]");
      return false;
    } else if (beta < 0 || beta > 1) {
      pluginProgress->setError(
          "beta is not a probability,\nit is is not between [0, 1]");
      return false;
    }
    ...
    for (unsigned int i = 0; i < nbNodes; ++i) {
      pAttractProperty[i] =
          ((1 - alpha) > randomDouble(1.0)) ? 0 : randomDouble(1.0);
