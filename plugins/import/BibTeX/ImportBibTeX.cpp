...
static string &forceUtf8String(string &str) {
  ...
  for (unsigned int i = 0; i < str.size(); ++i) {
    if ((str[i] > '\301') && (str[i] < '\340') && (i + 1) < str.size() &&
        ...
        ((unsigned char)str[i + 1] > '\177') && (str[i + 1] < '\300')) {
      ++i;
      continue;
    }
    if ((str[i] > '\337') && (str[i] < '\360') && (i + 2) < str.size() &&
        ...
        ((unsigned char)str[i + 1] > '\177') && (str[i + 1] < '\300') && ((unsigned char)str[i + 2] > '\177') && (str[i + 2] < '\300')) {
      i += 2;
      continue;
    }
...
public:
  ...
  ImportBibTeX(const tlp::PluginContext *context) : ImportModule(context) {
    addInParameter<string>("file::filename",
                           HTML_HELP_OPEN() HTML_HELP_DEF("type", "pathname")
                               HTML_HELP_BODY() "This parameter indicates the pathname of the file(.bib) to import." HTML_HELP_CLOSE(),
                           "");
    addInParameter<StringCollection>(
        "Nodes to import", HTML_HELP_OPEN() HTML_HELP_DEF("type", "String Collection")
                               HTML_HELP_DEF("values", "Authors <i>(Create nodes for authors only, publications are represented as edges between "
                                                       "authors)</i><br/>Authors and Publications <i>(Create nodes for both authors and "
                                                       "publications)</i><br/>Publications <i>(Create nodes for publications only)</i>")
                                   HTML_HELP_DEF("default", "Authors") HTML_HELP_BODY() "The type of nodes to create" HTML_HELP_CLOSE(),
        NODES_TO_IMPORT);
  }
  ...
  bool importGraph() {
    ...
    BooleanProperty *fromLabriProp = createAuthNodes ? graph->getProperty<BooleanProperty>("from LaBRI") : NULL;
    IntegerVectorProperty *labriAuthorsProp = createPubliNodes ? graph->getProperty<IntegerVectorProperty>("LaBRI authors") : NULL;
    StringVectorProperty *labriTeamsProp = createPubliNodes ? graph->getProperty<StringVectorProperty>("LaBRI teams") : NULL;
    StringVectorProperty *authProp = createPubliNodes ? graph->getProperty<StringVectorProperty>("authors") : NULL;
    StringProperty *authNameProp = createAuthNodes ? graph->getProperty<StringProperty>("name") : NULL;
    StringProperty *labriTeamProp = createAuthNodes ? graph->getProperty<StringProperty>("LaBRI team") : NULL;
    ...
    try {
      ...
      for (; it != itEnd; ++it, ++i) {
        ...
        if (createPubliNodes) {
          ...
          icon->setNodeValue(publi, "file-o");
        }
        ...
        for (; fit != fe.fields().end(); ++fit) {
          ...
          bool isNumber = !field.valueParts().empty() && (field.valueParts()[0].type() == xdkbib::Number);
          ...
          if (isAuthor) {
            ...
            char *authorsPtr;
            unsigned int labriIndex = labriAuthors ? (atoi(strtok_r(labriAuthors, " \n", &authorsPtr)) - 1) : 0;
            ...
            if (createPubliNodes) {
              ...
              if (authorNodes.empty()) {
                tlp::warning() << "Warning: found the publication '" << key.c_str() << "' without author" << std::endl;
    ...
    if (createAuthNodes) {
      ...
      string err;
      return graph->applyPropertyAlgorithm("FM^3 (OGDF)", graph->getProperty<LayoutProperty>("viewLayout"), err, pluginProgress);
    }
    return true;
  }
};
