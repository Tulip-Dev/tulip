...
public:
  ...
  void buildRow(const vector<node> &nodes, vector<pair<node, node>> &ends, unsigned int iRow, unsigned width, int conn, bool isTore, double spacing) {
    ...
    for (unsigned int i = 0; i < width; ++i) {
      ...
      if (conn == 6) {
        layout->setNodeValue(current, Coord(i * 2 * h + shift + i * spacing, iRow * (1.0 - hHeight + spacing), 0));
      } else
  ...
  void connectRow(const vector<node> &nodes, vector<pair<node, node>> &ends, unsigned int row1, unsigned int row2, unsigned int width, int conn,
                  bool isTore) {
