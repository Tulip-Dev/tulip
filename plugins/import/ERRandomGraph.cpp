 ...
 *
 *  User can specify the number of nodes and the probability of having an edge
 * between two nodes.
 */
class ERRandomGraph : public ImportModule {
public:
  PLUGININFORMATION(
      "Erdős-Rényi Random Graph", "Bruno Pinaud", "08/09/2014",
  ...
  bool importGraph() override {
    ...
    if ((proba < 0) || (proba > 1)) {
      if (pluginProgress)
        pluginProgress->setError(
            string("Error: the probability must be between ]0, 1[."));
    ...
    while (i != nbNodes) {
      ...
      if (pluginProgress &&
          pluginProgress->progress(i, nbNodes) != TLP_CONTINUE)
