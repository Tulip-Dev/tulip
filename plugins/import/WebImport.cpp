...
static const char *not_html_extensions[] = {
    ...
    ".pdf", ".png", ".ps",  ".rss", ".tar", ".tgz", ".wav", ".zip", ".z",    0 /* must be the last */
};
...
static const char *paramHelp[] = {
    // server
    "This parameter defines the web server that you want to inspect. No need for http:// at the beginning; http protocol is always assumed. No need "
    "for / at the end.",
...
struct WebImport : public ImportModule {
  ...
  //========================================================
  bool addEdge(const UrlElement &source, const UrlElement &target, const char *type, const Color *color) {
    node sNode, tNode;
  ...
  bool importGraph() {
    ...
    graph->getProperty<IntegerProperty>("viewShape")->setAllNodeValue(14); // GlyphManager::getInst().glyphId("2D - Circle")
