...
public:
  PLUGININFORMATION("UCINET", "Patrick Mary", "12/09/2011", "Imports a new graph from a text file (.txt) in UCINET DL input format<br/>as it is "
                                                            "described in the UCINET reference manual "
                                                            "(<b>http://www.analytictech.com/ucinet/documentation/reference.rtf</b>)",
                    "1.0", "File")
  ...
  ImportUCINET(const tlp::PluginContext *context)
      : ImportModule(context), nbNodes(0), defaultMetric("weight"), n(0), nr(0), nc(0), nm(0), current(0), dl_found(false), diagonal(true),
        diagonal_found(false), labels_known(false), title_found(false), expectedLine(DL_HEADER), embedding(DL_NONE), dataFormat(DL_FM) {
  ...
  enum TypeOfLine { DL_HEADER = 0, DL_ROW_LABELS, DL_COL_LABELS, DL_LABELS, DL_MATRIX_LABELS, DL_DATA };
    ...
    while ((result = nextToken(str, " \r\t,=", token, pos)) && !token.empty()) {
      ...
      if (nocasecmp(token, "row")) {
        ...
        if (nocasecmp(token, "labels")) {
          ...
          if (!nextToken(str, " \r\t,", token, pos) || token.empty() || !nocasecmp(token, "embedded")) {
            error << "invalid specification for parameter ROWS";
            return false;
          }
  ...
  bool readLabels(const string &str, stringstream &error, TLP_HASH_MAP<std::string, node> &labelsHMap, unsigned int nbLabels, unsigned int offset) {
    ...
    for (unsigned int i = 0; i < labels.size(); ++i, ++current) {
      ...
      std::transform(labels[i].begin(), labels[i].end(), labels[i].begin(), (int (*)(int))std::toupper);
  ...
  void checkColumnLabels(vector<std::string> &tokens, unsigned int &ir, unsigned int &ic, unsigned int &i) {
  ...
  node getNodeFromInfo(string &token, unsigned int &i, bool findCol) {
    if (embedding == DL_NONE || (embedding != DL_ALL && !(embedding & (findCol ? DL_COLS : DL_ROWS)))) {
    ...
    transform(token.begin(), token.end(), upcasetoken.begin(), (int (*)(int))std::toupper);
  ...
  bool readData(vector<std::string> &tokens, stringstream &error, unsigned int &ir, unsigned int &ic, DoubleProperty *metric) {
    ...
    case DL_UH: {
      ...
      for (; i < tokens.size(); ++i) {
        ...
        // check end of row
        if ((dataFormat == DL_LH && (diagonal ? (ic > ir) : (ic == ir))) || (ic == (nc ? nc : nbNodes)))
          ++ir, ic = 0, current = 0;
      }
    ...
    case DL_NL2: {
      ...
      node src = getNodeFromInfo(tokens[0], dataFormat == DL_NL1 ? ic : ir, false);
    ...
    case DL_EL2: {
      ...
      node src = getNodeFromInfo(tokens[0], (dataFormat == DL_EL1) ? ic : ir, false);
  ...
  bool importGraph() {
    ...
    while (!in->eof() && std::getline(*in, line)) {
      ...
      case DL_ROW_LABELS:
        result = readLabels(line, errors, rowLabelToNode, nr ? nr : n, nc);
        break;
      ...
      case DL_DATA: {
        ...
        if ((tokens.size() == 1) && (tokens[0] == "!") && (nm > 1) && (im < nm) && ((dataFormat == DL_EL1) || (dataFormat == DL_EL2))) {
          ++im;
          break;
        }
      ...
      if (pluginProgress && ((lineNumber % 100) == 0) && (pluginProgress->progress(lineNumber, 3 * nbNodes) != TLP_CONTINUE)) {
        delete in;
        return false;
      }
    }
