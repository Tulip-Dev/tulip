...
GlLabelledColorScale::GlLabelledColorScale(Coord position, Size size, ColorScale *colorScale, double minValue, double maxValue, bool colorScaleAtTop)
    : GlComposite(), position(position), size(size), minValue(minValue), maxValue(maxValue), colorScaleAtTop(colorScaleAtTop) {
...
void GlLabelledColorScale::buildComposite(ColorScale *colorScale) {
  ...
  if (colorScaleAtTop) {
    ...
    maxValueLabelPosition.set((position.getX() + size.getW()) - valueLabelSize.getW() / 2, minValueLabelPosition.getY(), 0);
    scalePosition.set(position.getX(), position.getY() + labelHeightSize + (scaleThickness / 2));
  } else {
    minValueLabelPosition.set(position.getX() + valueLabelSize.getW() / 2, position.getY() + size.getH() - labelHeightSize / 2, 0);
    maxValueLabelPosition.set((position.getX() + size.getW()) - valueLabelSize.getW() / 2, minValueLabelPosition.getY(), 0);
  ...
  glColorScale = new GlColorScale(colorScale, scalePosition, scaleLength, scaleThickness, GlColorScale::Horizontal);
