...
SOMMapElement::SOMMapElement(tlp::Coord position, tlp::Size size, SOMMap *map, tlp::ColorProperty *colorProperty)
...
float SOMMapElement::computeMaximizedRadiusForHexagone(unsigned int width, unsigned int height, tlp::Size &size) {
  ...
  if (conn == SOMMap::six) {
    ...
    for (unsigned int i = 0; i < map->getHeight(); ++i) {
      for (unsigned int j = 0; j < map->getWidth(); ++j) {
        ...
        tlp::GlCircle *hex = new tlp::GlCircle(center, r, c, c, true, false, M_PI / 2.0f, 6);
        oss.str("");
        oss << j << "," << i;
    ...
    for (unsigned int i = 0; i < map->getHeight(); ++i) {
      for (unsigned int j = 0; j < map->getWidth(); ++j) {
        ...
        topLeft += basePos;
        Coord bottomRight(topLeft.getX() + elementSize.getX(), topLeft.getY() - elementSize.getY(), 0);
...
tlp::Coord SOMMapElement::getTopLeftPositionForElement(unsigned int x, unsigned int y) {
  ...
  if (som->getConnectivity() == SOMMap::six) {
    ...
    pos.set(x * (size.getW() / som->getWidth()), (som->getHeight() - y) * (size.getH() / som->getHeight()), 0);
    pos += position;
  }
