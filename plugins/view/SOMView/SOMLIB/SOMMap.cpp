...
SOMMap::SOMMap(Graph *root, unsigned int width, unsigned int height, SOMMapConnectivity connectivity, bool oppositeConnected)
    : tlp::GraphDecorator(root), width(width), height(height), connectivity(connectivity), oppositeConnected(oppositeConnected), graphCreated(false) {
  initMap();
}
SOMMap::SOMMap(unsigned int width, unsigned int height, SOMMapConnectivity connectivity, bool oppositeConnected)
    : tlp::GraphDecorator(newGraph()), width(width), height(height), connectivity(connectivity), oppositeConnected(oppositeConnected),
      graphCreated(true) {
  initMap();
}
...
void SOMMap::registerModification(const vector<string> &propertiesToListen) {
  ...
  for (vector<string>::const_iterator it = propertiesToListen.begin(); it != propertiesToListen.end(); ++it) {
  ...
  for (node n : getNodes()) {
    ...
    for (unsigned int propertyNumber = 0; propertyNumber < properties.size(); ++propertyNumber) {
      ...
      if (properties[propertyNumber]->getTypename().compare("double") == 0) {
        ((DoubleProperty *)properties[propertyNumber])->setNodeValue(n, nodeToNodeVec[n][propertyNumber]);
      } else {
        std::cerr << __PRETTY_FUNCTION__ << ":" << __LINE__ << " unmanaged type " << properties[propertyNumber]->getTypename() << std::endl;
        assert(false);
      }
    }
  }
