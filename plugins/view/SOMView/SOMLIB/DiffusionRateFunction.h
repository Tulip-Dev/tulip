 ...
 * Interface for distance rate computation functions. Distance rate is used in
 * learning rate computation to handle the distance factor. These objects take a
 * TimeDecreasingFunction as the diffusion coefficient evolve with distance but
 * also with time.
...
public:
  ...
  /**
   * Compute the diffusion rate in function of the current time and the
   * distance.
   * @param distance The distance between the BMU and the node. This distance is
   * the number of node to go through from BMU to current node.
