 ...
 * Self Organizing Map representation. Use the Tulip graph library to represent a SOM. Store the weight of each node in a map and can transmit them to
 * Graph Property.
 * Grid is generated by the Grid import algorithm from Tulip. Give to user function to handle the graph like a traditional grid (with x and y
 * position).
 */
namespace tlp {
...
public:
   ...
   * @return
   */
  SOMMap(Graph *root, unsigned int width, unsigned int height, SOMMapConnectivity connectivity = four, bool oppositeConnected = false);
  /**
   ...
   * @return
   */
  SOMMap(unsigned int width, unsigned int height, SOMMapConnectivity connectivity = four, bool oppositeConnected = false);
  virtual ~SOMMap();
