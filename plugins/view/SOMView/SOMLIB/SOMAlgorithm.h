...
public:
  /**
   * Build the SOMAlgorithm object used to compute Self Organizing Map with the
   * given time and space function.
   * @param learningRateFunction The function used to compute the learning rate
   * in function of time.
   * @param diffusionRateFunction The function used to compute the distance rate
   * in function of time.
   */
  ...
  /**
   * Init the given SOM with the given sample. At this time set all the weight
   * node of the map with a weight take randomly in the input sample.
   * @param map The SOM
  ...
  /**
   * Train the SOM with the given sample. Use numberOfIteration time the entire
   * inputSample to train the SOM. See the train function for more details.
   * @param map The SOM.
   * @param inputSample The input Sample.
   * @param numberOfIteration The number of time that the input sample will be
  ...
  /**
   * Train the SOM with the given sample (update value of each SOM node's weight
   * with the input sample see SOM algorithm for more details.). Training
   * process has this order while currentNumber of iteration < maxIteration :
  ...
  /**
   * Return a node with the smallest euclidian distance between its weight
   * vector and the given input vector. If there is one or more node with the
   * smallest distance choose one randomly.
   * @param map The SOM.
   * @param input The input vector.
   * @param dist The euclidian distance between input vector and selected node
   * weight.
   * @return
   */
  ...
  /**
   * Propagate modification on the SOM. Compute the learning coefficient in
   * function of the current iteration and the distance between the first
   * updated node and the modified one. Propagate modification while learning
   * rate is not null.
   * @param map The SOM.
  ...
  void propagateModification(SOMMap *map, const DynamicVector<double> &input,
                             node bmu, unsigned int currentTime,
                             unsigned int maxTime, unsigned int sampleSize);

  /**
   * Function use to automatically perform initialisation and training on a SOM
   * with a given sample of vector.
   * @param map The SOM.
   * @param inputSample
   * @param nTimes The number of time that the input sample will be used to
   * train the map.
   * @param pluginProgress
   */
  ...
  /**
   * Perform the mapping operation of each node of the input sample on the
   * center tab. At the end each vector of the input sample will be associated
   * with a SOM node.
   * @param map The SOM
   * @param inputSample
   * @param mappingTab The tab used to perform the mapping between input nodes
   * and SOM nodes. For each node in the map correspond a table of linked input
   * node.
   * @param medDist The medium distance of the mapping.
   * @param maxElement The maximum number of element linked with a SOM node.
   */
  void
  computeMapping(SOMMap *map, InputSample &inputSample,
