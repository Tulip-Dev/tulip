...
public:
   ...
   */
  SOMAlgorithm(TimeDecreasingFunction *learningRateFunction = nullptr, DiffusionRateFunction *diffusionRateFunction = nullptr);
   ...
   */
  void initMap(SOMMap *map, InputSample &inputSample, tlp::PluginProgress *pluginProgress = nullptr);
   ...
   */
  void trainNInputSample(SOMMap *map, InputSample &inputSample, unsigned int numberOfIteration, tlp::PluginProgress *pluginProgress = nullptr);

  /**
   * Train the SOM with the given sample (update value of each SOM node's weight with the input sample see SOM algorithm for more details.). Training
   * process has this order
   ...
   * @param pluginProgress
   */
  void train(SOMMap *map, InputSample &inputSample, unsigned int maxIteration, tlp::PluginProgress *pluginProgress = nullptr);

  /**
   * Return a node with the smallest euclidian distance between its weight vector and the given input vector. If there is one or more node with the
  ...
  /**
   * Propagate modification on the SOM. Compute the learning coefficent in function of the current iteration and the distance between the first
   * updated node and the modified one.
   ...
   */
  void propagateModification(SOMMap *map, const DynamicVector<double> &input, node bmu, unsigned int currentTime, unsigned int maxTime,
   ...
   * @param pluginProgress
   */
  void run(SOMMap *map, InputSample &inputSample, unsigned int nTimes, tlp::PluginProgress *pluginProgress = nullptr);

  /**
   * Perform the mapping operation of each node of the input sample on the center tab. At the end each vector of the input sample will be associated
   * with a SOM node.
   * @param map The SOM
   * @param inputSample
   * @param mappingTab The tab used to perform the mapping between input nodes and SOM nodes. For each node in the map correspond a table of linked
   * input node.
   * @param medDist The medium distance of the mapping.
   * @param maxElement The maximum number of element linked with a SOM node.
   */
  void computeMapping(SOMMap *map, InputSample &inputSample, std::map<tlp::node, std::set<tlp::node>> &mappingTab, double &medDist,
