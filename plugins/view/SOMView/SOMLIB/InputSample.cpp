...
InputSample::InputSample(Graph *graph, const vector<string> &propertiesToListen) : rootGraph(graph) {
  // mWeightTab.setAll(DynamicVector<double> ());
  mWeightTab.clear();
...
void InputSample::initPropertiesObs() {
  for (vector<NumericProperty *>::iterator it = propertiesList.begin(); it != propertiesList.end(); ++it) {
    (*it)->addObserver(this);
  }
}
...
void InputSample::update(std::set<Observable *>::iterator begin, std::set<Observable *>::iterator end) {
  bool updated = false;
  ...
  for (std::set<Observable *>::iterator it = begin; it != end; ++it) {
    ...
    for (std::vector<tlp::NumericProperty *>::iterator itP = propertiesList.begin(); itP != propertiesList.end(); ++itP) {
      if ((*it) == (*itP)) {
        // mWeightTab.setAll(DynamicVector<double> ());
        mWeightTab.clear();
...
void InputSample::addNode(Graph *, const node n) {
  if (usingNormalizedValues) {
    for (unsigned int i = 0; i < propertiesList.size(); ++i) {
      meanProperties[i] = (double(rootGraph->numberOfNodes() - 1) * meanProperties[i] + propertiesList[i]->getNodeDoubleValue(n)) /
