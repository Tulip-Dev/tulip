...
void SOMAlgorithm::run(SOMMap *map, InputSample &inputSample, unsigned int nTimes, tlp::PluginProgress *pluginProgress) {
  // Map initialisation
  if (pluginProgress)
 ...
 */
void SOMAlgorithm::initMap(SOMMap *map, InputSample &inputSample, tlp::PluginProgress *pluginProgress) {
...
void SOMAlgorithm::trainNInputSample(SOMMap *map, InputSample &inputSample, unsigned int nTimes, tlp::PluginProgress *pluginProgress) {
  train(map, inputSample, nTimes * inputSample.getSampleSize(), pluginProgress);
}

void SOMAlgorithm::train(SOMMap *map, InputSample &inputSample, unsigned int maxIteration, tlp::PluginProgress *pluginProgress) {
  ...
  while (currentIteration < maxIteration) {
    ...
    propagateModification(map, currentInputVector, bmu, currentIteration, maxIteration, inputSample.getSampleSize());
...
void SOMAlgorithm::propagateModification(SOMMap *map, const DynamicVector<double> &input, node bmu, unsigned int currentIteration,
                                         unsigned int maxIteration, unsigned int sampleSize) {
  ...
  double learningRate = learningRateFunction->computeCurrentTimeRate(currentIteration, maxIteration, sampleSize);
  ...
  while (!toVisit.empty()) {
    ...
    double diffusionRate = diffusionRateFunction->computeSpaceRate(distance.get(current.id), currentIteration, maxIteration, sampleSize);
    ...
    if (diffusionRate > 0) {
      for (node neighbor : map->getInOutNodes(current)) {
        // not already treated
...
void SOMAlgorithm::computeMapping(SOMMap *map, InputSample &inputSample, std::map<tlp::node, std::set<tlp::node>> &mappingTab, double &medDist,
  ...
  for (node n : inputSample.getNodes()) {
