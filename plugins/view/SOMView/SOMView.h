...
class SOMView : public GlMainView {
  ...
  PLUGININFORMATION(ViewName::SOMViewName, "Dubois Jonathan", "02/2009",
                    ...
                    "The model was first described as an artificial neural network by the Finnish professor Teuvo Kohonen."
                    "This view is useful to visualize high-dimensional data in a low-dimensional representation."
                    "<p>Self organizing maps operate in two modes: training and mapping.<ul><li>Training builds the map using input "
                    "examples.</li><li>Mapping automatically classifies data on the map generated by training.</li></ul></p>",
                    "1.1", "View")
public:
...
private:
  ...
  /**
   * Init the GlMainWidget object. At this time only init the previewWidget as the mapWidget is initialized when a new graph is given in the function
   * changeMapViewGraph.
   */
   ...
   * @return
   */
  ColorProperty *computePropertyColor(const std::string &propertyName, double &minValue, double &maxValue);
   ...
   */
  void computeColor(SOMMap *som, tlp::NumericProperty *property, tlp::ColorScale &colorScale, tlp::ColorProperty *result);
