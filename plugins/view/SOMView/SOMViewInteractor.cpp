...
SOMViewNavigation::SOMViewNavigation(PluginContext *) : SOMViewInteractor(":/tulip/gui/icons/i_navigation.png", "Navigate") {
...
SOMViewSelection::SOMViewSelection(PluginContext *) : SOMViewInteractor(":/tulip/gui/icons/i_selection.png", "Select") {
...
SOMViewProperties::SOMViewProperties(PluginContext *) : SOMViewInteractor(":/tulip/gui/icons/i_select.png", "Properties") {
...
SOMViewThreshold::SOMViewThreshold(PluginContext *) : SOMViewInteractor(":/i_slider.png", "Threshold Selection") {
  ...
  setConfigurationWidgetText(QString("<H1>Threshold Interactor</H1><p>This interactor is used to select nodes with a value between those indicated "
                                     "by the two sliders</p><p>Move the each slider to change the bound.</p><p>Press the Ctrl button to add the new "
                                     "threshold selection to the current selection. If Ctrl is not pressed the old selection will be replaced</p>"));
}
