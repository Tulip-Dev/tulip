...
SOMView::SOMView(PluginContext *)
    : graphComposite(nullptr), graphLayoutProperty(nullptr), graphSizeProperty(nullptr), mask(nullptr), somMask(nullptr),
      mapCompositeElements(nullptr), som(nullptr), previewWidget(nullptr), properties(nullptr), somMapIsBuild(false), isConstruct(false) {
...
void SOMView::buildSOMMap() {
  ...
  mapCompositeElements = new SOMMapElement(
      Coord(0 + ((somMawWidth - somSize.getW()) / 2), (scaleHeight + spacing) + ((somMaxHeight - somSize.getH()) / 2), 0), somSize, som, nullptr);
...
void SOMView::computeMapping() {
  ...
  Size graphDiffSize(graphMinSize.getW() == graphMaxSize.getW() ? 1. : (graphMaxSize.getW() - graphMinSize.getW()),
                     graphMinSize.getH() == graphMaxSize.getH() ? 1. : (graphMaxSize.getH() - graphMinSize.getH()), 0);
  ...
  for (map<node, set<node>>::iterator it = mappingTab.begin(); it != mappingTab.end(); ++it) {
    ...
    for (set<node>::iterator it2 = it->second.begin(); it2 != it->second.end(); ++it2) {
      // Compute node center
      nodeCoord.set(nodeDisplayAreaTopLeft[0] + (num % colNumber) * maxElementWidth + maxElementWidth / 2,
                    nodeDisplayAreaTopLeft[1] - ((floor(num / colNumber) * maxElementHeight) + maxElementHeight / 2), 0);
      ...
      if (mt == SOMPropertiesWidget::NoSizeMapping || graphMaxSize == graphMinSize) {
        ...
        nodeSize.set(minElementWidth + ((realSize.getW() - graphMinSize.getW()) / (graphDiffSize.getW())) * (maxElementWidth - minElementWidth),
                     minElementHeight + ((realSize.getH() - graphMinSize.getH()) / (graphDiffSize.getH())) * (maxElementHeight - minElementHeight),
                     0);
...
void SOMView::getPreviewsAtViewportCoord(int x, int y, std::vector<SOMPreviewComposite *> &result) {
  ...
  previewWidget->getScene()->selectEntities(RenderingSimpleEntities, x, y, 0, 0, nullptr, selectedEntities);
...
void SOMView::gridStructurePropertiesUpdated() {
  ...
  if (!checkGridValidity()) {
    QMessageBox::critical(nullptr, tr("Bad grid"), tr("Cannot connect opposite nodes in an hexagonal grid with odd height"));
    return;
  }
...
bool SOMView::checkGridValidity() const {
  return !(properties->getGridHeight() % 2 != 0 && properties->getConnectivityIndex() == 1 && properties->getOppositeConnected());
}
