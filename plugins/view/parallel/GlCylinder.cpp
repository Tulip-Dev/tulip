...
const int dth = 10;		// no. of angular cylinder subdivsions 
const int dz = 10;		// no. of cylinder subdivsions in z direction
...
void setMaterial(const tlp::Color &c) {
  ...
  GlCylinder::GlCylinder(const Coord &start, const Coord &end, float startRad, float endRad, const Color &c, const bool withCaps) :
  ...
  GlCylinder::GlCylinder(const Coord &start, const Coord &end, float startRad, float endRad, const std::string& textFile, const Color &c, const bool withCaps) : startPoint(start), endPoint(end), startRadius(startRad), endRadius(endRad), displayCaps(withCaps), color(c),  textureFile(textFile)  {
