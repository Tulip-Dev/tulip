...
QSet<QString> AutoCompletionDataBase::getGraphPropertiesListIfContext(const QString &context, const QString &editedFunction) const  {
	...
	if (graph && cleanContext.lastIndexOf("[") != -1) {
		...
		if (type == "tlp.Graph") {
			...
			ret = getGraphPropertiesList(graph, prefix);
		}
