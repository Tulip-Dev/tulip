...
const string viewPropertiesName[] = {
    "viewBorderColor", "viewBorderWidth",   "viewColor",  "viewFont",
...
const unsigned int nbViewProperties =
    sizeof(viewPropertiesName) / sizeof(string);

const vector<string> graphViewProperties(viewPropertiesName,
                                         viewPropertiesName + nbViewProperties);

static vector<string>
getGraphPropertiesListAccordingToType(Graph *graph, const string &typeName) {
  vector<string> ret;

  for (const string &propertyName : graph->getProperties()) {
    if (std::find(graphViewProperties.begin(), graphViewProperties.end(),
                  propertyName) == graphViewProperties.end()) {
...
GeolocalisationConfigWidget::GeolocalisationConfigWidget(QWidget *parent)
  ...
  _ui->setupUi(this);
  connect(_ui->addressLocRB, SIGNAL(toggled(bool)), this,
          SLOT(enableDisableComboBoxes()));
  connect(_ui->latLngRB, SIGNAL(toggled(bool)), this,
          SLOT(enableDisableComboBoxes()));
  connect(_ui->genLayoutButton, SIGNAL(clicked()), this,
          SIGNAL(computeGeoLayout()));
  connect(_ui->createLatLngPropsCB, SIGNAL(clicked(bool)),
          _ui->resetLatLngValuesCB, SLOT(setEnabled(bool)));
}

GeolocalisationConfigWidget::~GeolocalisationConfigWidget() { delete _ui; }

void GeolocalisationConfigWidget::setGraph(Graph *graph) {
  _ui->addressPropCB->clear();
  vector<string> &&stringProperties =
      getGraphPropertiesListAccordingToType(graph, "string");
  ...
  _ui->latPropCB->clear();
  _ui->lngPropCB->clear();
  vector<string> &&doubleProperties =
      getGraphPropertiesListAccordingToType(graph, "double");
...
void GeolocalisationConfigWidget::setLatLngGeoLocMethod(
    const std::string &latitudePropertyName,
    const std::string &longitudePropertyName) {
  _ui->latLngRB->setChecked(true);
  int latPropIndex =
      _ui->latPropCB->findText(tlpStringToQString(latitudePropertyName));
  int lngPropIndex =
      _ui->lngPropCB->findText(tlpStringToQString(longitudePropertyName));
...
void GeolocalisationConfigWidget::setEdgesPathsPropertyName(
  ...
  int edgesPathsPropertyIndex = _ui->edgesPathsPropertyCB->findText(
      tlpStringToQString(edgesPathsPropertyName));
