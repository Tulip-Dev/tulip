...
public:
  PLUGININFORMATION(
      ...
      "<p>Specifically, the adjacency matrix of a finite graph G on n vertices is the n x n matrix where the non-diagonal entry a<sub>ij</sub> is "
      "the number of edges from vertex <i>i</i> to vertex <i>j</i>, and the diagonal entry a<sub>ii</sub>, depending on the convention, is either "
      "once or twice the number of edges (loops) from vertex <i>i</i> to itself. "
