...
void GlMatrixBackgroundGrid::draw(float lod, tlp::Camera *camera) {
  ...
  int N = _view->graph()->numberOfNodes();
  double startX = max<double>(0.5, floor(topLeft[0]) - 0.5), startY = min<double>(-0.5, ceil(topLeft[1]) + 0.5),
         endX = min<double>(0.5 + N, ceil(bottomRight[0]) + 0.5), endY = max<double>(-0.5 - N, floor(bottomRight[1]) - 0.5);
