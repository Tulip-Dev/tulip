...
HistogramInteractor::HistogramInteractor(const QString &iconPath, const QString &text) : GLInteractorComposite(QIcon(iconPath), text) {}

bool HistogramInteractor::isCompatible(const std::string &viewName) const {
	return (viewName == HistogramView::viewName);
}

INTERACTORPLUGINVIEWEXTENSION(HistogramInteractorZoom,"HistogramZoomInteractor","InteractorRectangleZoom",HistogramView::viewName, "Tulip Team" ,"02/04/09","Histogram rectangle zoom interactor","1.0")
INTERACTORPLUGINVIEWEXTENSION(HistogramInteractorGetInformation,"HistogramInteractorGetInformation","InteractorGetInformation",HistogramView::viewName, "Tulip Team" ,"02/04/09","Histogram get information interactor","1.0")
INTERACTORPLUGINVIEWEXTENSION(HistogramInteractorSelection,"HistogramSelectionInteractor","InteractorSelection",HistogramView::viewName, "Tulip Team" ,"02/04/09","Histogram selection interactor","1.0")

PLUGIN(HistogramInteractorNavigation)
PLUGIN(HistogramInteractorMetricMapping)
PLUGIN(HistogramInteractorStatistics)

QString interactorHistoNavigationHelpText =
	"<html>"
	"<head>"
	"<title></title>"
	"</head>"
...
QString interactorHistoColorMappingHelpText =
	"<html>"
	"<head>"
	"<title></title>"
	"</head>"
...
QWidget *HistogramInteractorNavigation::configurationWidget() const {
  QLabel *_label = new QLabel(interactorHistoNavigationHelpText);
...
QWidget *HistogramInteractorMetricMapping::configurationWidget() const {
  QLabel *_label = new QLabel(interactorHistoColorMappingHelpText);
  _label->setWordWrap(true);
  _label->setAlignment(Qt::AlignTop);
  _label->setSizePolicy(QSizePolicy::Expanding,QSizePolicy::Expanding);
  return _label;
...
HistogramInteractorStatistics::HistogramInteractorStatistics(const PluginContext *) : HistogramInteractor(":/i_histo_statistics.png", "Statistics") {}

void HistogramInteractorStatistics::construct() {
	histoStatsConfigWidget = new HistoStatsConfigWidget();
	HistogramStatistics *histoStatistics = new HistogramStatistics();
  histoStatistics->setConfigWidget(histoStatsConfigWidget);
