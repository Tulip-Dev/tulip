...
GlEditableComplexPolygon::GlEditableComplexPolygon(std::vector<Coord> polygonPoints, const Color &color)
...
ScatterPlotCorrelCoeffSelector::ScatterPlotCorrelCoeffSelector(ScatterPlotCorrelCoeffSelectorOptionsWidget *optionsWidget)
    : optionsWidget(optionsWidget), scatterView(nullptr), selectedPolygon(nullptr), selectedPolygonPoint(nullptr), dragStarted(false), x(0), y(0) {
  basicCircle.resizePoints(30);
  basicCircle.setFillMode(true);
  basicCircle.setOutlineMode(true);
...
ScatterPlotCorrelCoeffSelector::ScatterPlotCorrelCoeffSelector(const ScatterPlotCorrelCoeffSelector &scatterPlotCorrelCoeffSelector)
    : scatterView(nullptr), selectedPolygon(nullptr), selectedPolygonPoint(nullptr), dragStarted(false), x(0), y(0) {
...
bool ScatterPlotCorrelCoeffSelector::eventFilter(QObject *obj, QEvent *e) {
  ...
  if (e->type() == QEvent::MouseMove) {
    ...
    if (polygonEdit.empty()) {
      ...
      bool underFirstPoint = (pointerGlViewportCoord.getX() > (startPolygonPointScr.getX() - POINT_RADIUS) &&
...
bool ScatterPlotCorrelCoeffSelector::draw(GlMainWidget *glMainWidget) {
  ...
  if (selectedPolygon != nullptr) {
    ...
    GlLabel coeffLabel(Coord(xAxis->getAxisBaseCoord().getX() + (1.0f / 2.0f) * xAxis->getAxisLength(),
