...
/** This plugin is an implementation of a strongly connected components
 * decomposition.
 *
 *  \note This algorithm assigns to each node a value defined as following : If
 * two nodes are in the same strongly connected component they have the same
...
private:
  unsigned attachNumerotation(tlp::node, std::unordered_map<tlp::node, bool> &,
                              ...
                              unsigned &, std::stack<tlp::node> &, unsigned &);
};
