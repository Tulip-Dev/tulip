...
ModularityVertexPartition::ModularityVertexPartition(Graph *graph, vector<size_t> const &membership)
...
#ifdef DEBUG
  cerr << "exit double ModularityVertexPartition::diff_move((" << v << ", " << new_comm << ")"
       << endl;
...
#endif
    mod +=
        w - w_out * w_in / ((this->graph->is_directed() ? 1.0 : 4.0) * this->graph->total_weight());
  }
