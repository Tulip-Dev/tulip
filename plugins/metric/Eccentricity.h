 ...
 *
 *
 * Closeness Centrality is the mean of shortest-paths lengths from a node to
 * others. The normalized values are computed using the reciprocal of the sum of
 * these distances (see
 * "http://en.wikipedia.org/wiki/Closeness_(graph_theory)#Closeness_centrality"
 * for more details).
 *
 *  \note The complexity of the algorithm is O(|V| * |E|) time and O(1) space
 *        for unweighted graphs and O(|V| * |E| \log |V|) time for weighted
 * graphs.
 *
 *   <b>HISTORY</b>
 *
