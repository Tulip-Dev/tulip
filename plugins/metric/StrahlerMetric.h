...
public:
  PLUGININFORMATION(
      ...
      "Conference on Computer Vision and Graphics, pages 56-69 (2002)",
      "1.0", "Graph")
  StrahlerMetric(const tlp::PluginContext *context);
  bool run();

private:
  Strahler topSortStrahler(tlp::node n, int &curPref, TLP_HASH_MAP<tlp::node, int> &tofree, TLP_HASH_MAP<tlp::node, int> &prefix,
                           TLP_HASH_MAP<tlp::node, bool> &visited, TLP_HASH_MAP<tlp::node, bool> &finished,
