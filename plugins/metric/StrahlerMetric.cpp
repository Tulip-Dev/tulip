...
Strahler StrahlerMetric::topSortStrahler(tlp::node n, int &curPref, TLP_HASH_MAP<node, int> &tofree, TLP_HASH_MAP<node, int> &prefix,
                                         TLP_HASH_MAP<node, bool> &visited, TLP_HASH_MAP<node, bool> &finished,
...
static const char *paramHelp[] = {
    // All nodes
    "If true, for each node the Strahler number is computed from a spanning tree having that node as root: complexity o(n^2). If false the Strahler "
...
bool StrahlerMetric::run() {
  ...
  */
  ...
  while (itN->hasNext()) {
    ...
    if (allNodes) {
      if (pluginProgress && ((++i % 100) == 0) && (pluginProgress->progress(i, graph->numberOfNodes()) != TLP_CONTINUE))
        break;
