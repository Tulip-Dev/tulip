...
static const char *paramHelp[] = {
    ...
    "<li>false: uniform quantification (the values of property are sorted, and the same size increment is used between consecutive "
    "values).</li></ul>",
...
public:
  ...
  MetricSizeMapping(const PluginContext *context)
    ...
    addInParameter<StringCollection>("area proportional", paramHelp[7], "Area Proportional;Quadratic/Cubic", true,
  ...
  bool run() {
    ...
    if (nodeoredge) {
      ...
      while (itN->hasNext()) {
        ...
        if (zaxis)
          size[2] = static_cast<float>(sizos);
