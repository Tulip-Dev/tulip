...
template <typename TYPE> void fillAlgorithms(QComboBox *combo, QString title, int id) {
...
// Numeric comparers
#define NUM_CMP(N, OP)                                                                                                                               \
  bool FiltersManagerCompareItem::N##Comparer::compare(const std::string &a, const std::string &b) {                                                 \
    return a OP b;                                                                                                                                   \
  }                                                                                                                                                  \
  bool FiltersManagerCompareItem::N##Comparer::compare(double a, double b) {                                                                         \
    return a OP b;                                                                                                                                   \
  }
NUM_CMP(Equal, ==)
NUM_CMP(Different, !=)
...
bool FiltersManagerCompareItem::isComparisonNumeric(QComboBox *combo) const {
  ...
  return (elType == E_CustomValue && isCustomDouble) ||
         (elType == E_Property &&
          (dynamic_cast<DoubleProperty *>(comboProperty(combo)) != nullptr || dynamic_cast<IntegerProperty *>(comboProperty(combo)) != nullptr)) ||
...
QString FiltersManagerCompareItem::title() const {
  return (_ui->selectModeCombo->currentIndex() == 0 ? "+  " : "-  ") + _ui->elem1->currentText() + " " + _ui->operatorCombo->currentText() + " " +
...
public:
  ...
  double getNodeValue(node n) {
    if (_intProp != nullptr)
      return (double)(_intProp->getNodeValue(n));

    return _doubleProp->getNodeValue(n);
  }
  double getEdgeValue(edge e) {
    if (_intProp != nullptr)
      return (double)(_intProp->getEdgeValue(e));
