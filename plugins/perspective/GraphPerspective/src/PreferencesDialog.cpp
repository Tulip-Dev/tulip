...
PreferencesDialog::PreferencesDialog(QWidget *parent)
  ...
  _ui->setupUi(this);
  _ui->graphDefaultsTable->setItemDelegate(
      new tlp::TulipItemDelegate(_ui->graphDefaultsTable));
  ...
  connect(_ui->graphDefaultsTable,
          SIGNAL(customContextMenuRequested(const QPoint &)), this,
          SLOT(showGraphDefaultsContextMenu(const QPoint &)));
  connect(_ui->randomSeedCheck, SIGNAL(stateChanged(int)), this,
          SLOT(randomSeedCheckChanged(int)));
  ...
  for (int i = 0; i < _ui->graphDefaultsTable->rowCount(); ++i) {
    ...
    item->setToolTip(
        QString(
            "Click mouse right button to display a contextual menu allowing to "
            "reset the default values of <b>") +
        item->text() + "</b>.");
  }

  _ui->graphDefaultsTable->horizontalHeader()->setSectionResizeMode(
...
inline void setDefaultNodeValueInProperty(const std::string &propertyName,
                                          const TYPE &value, bool &graphPush) {
...
template <typename PROP, typename TYPE>
inline void setDefaultEdgeValueInProperty(const std::string &propertyName,
                                          const TYPE &value, bool &graphPush) {
  for (Graph *root : getRootGraphs()) {
    if (root->existLocalProperty(propertyName)) {
      PROP *prop = dynamic_cast<PROP *>(root->getProperty(propertyName));
...
void PreferencesDialog::writeSettings() {
  ...
  if (TulipSettings::defaultColor(tlp::NODE) !=
    ...
    TulipSettings::setDefaultColor(
        tlp::NODE, model->data(model->index(0, 1)).value<tlp::Color>());
  ...
  if (TulipSettings::defaultColor(tlp::EDGE) !=
    ...
    TulipSettings::setDefaultColor(
        tlp::EDGE, model->data(model->index(0, 2)).value<tlp::Color>());
  ...
  if (TulipSettings::defaultSize(tlp::NODE) !=
      model->data(model->index(1, 1)).value<tlp::Size>()) {
    TulipSettings::setDefaultSize(
        tlp::NODE, model->data(model->index(1, 1)).value<tlp::Size>());
    setDefaultNodeValueInProperty<SizeProperty>(
        "viewSize", TulipSettings::defaultSize(tlp::NODE), graphPush);
  }

  if (TulipSettings::defaultSize(tlp::EDGE) !=
      model->data(model->index(1, 2)).value<tlp::Size>()) {
    TulipSettings::setDefaultSize(
        tlp::EDGE, model->data(model->index(1, 2)).value<tlp::Size>());

    if (applyDrawingDefaults)
      setDefaultEdgeValueInProperty<SizeProperty>(
          "viewSize", TulipSettings::defaultSize(tlp::EDGE), graphPush);
  }
  ...
  if (TulipSettings::defaultLabelColor() !=
    ...
    if (applyDrawingDefaults) {
      setDefaultNodeValueInProperty<ColorProperty>(
          "viewLabelColor", TulipSettings::defaultLabelColor(), graphPush);
      setDefaultEdgeValueInProperty<ColorProperty>(
          "viewLabelColor", TulipSettings::defaultLabelColor(), graphPush);
    }
  }
  ...
  TulipSettings::setResultPropertyStored(
      _ui->resultPropertyStoredCheck->isChecked());
...
void PreferencesDialog::readSettings() {
  ...
  model->setData(model->index(4, 1), QVariant::fromValue<tlp::Color>(
                                         TulipSettings::defaultLabelColor()));
  ...
  _ui->applyDrawingDefaultsCheck->setEnabled(
      !iteratorEmpty(tlp::getRootGraphs()));
  ...
  _ui->resultPropertyStoredCheck->setChecked(
      TulipSettings::isResultPropertyStored());
  ...
  _ui->displayModeCombo->setCurrentIndex(
      TulipSettings::isDisplayInDarkMode() ? 1 : 0);

  if (TulipSettings::isUseTlpbFileFormat()) {
    _ui->usetlpbformat->setChecked(true);
  } else
    connect(_ui->usetlpbformat, SIGNAL(stateChanged(int)), this,
            SLOT(usetlpbformat(int)));
  ...
  _ui->randomSeedEdit->setText(isSeedRandom ? QString()
                                            : QString::number(seed));
}
...
void PreferencesDialog::cellChanged(int row, int column) {
  if (row >= 3) {
    ...
    QAbstractItemModel *model = _ui->graphDefaultsTable->model();
    model->setData(model->index(row, column == 1 ? 2 : 1),
  ...
  case 0: // default color
    if (updateMode == RESET_NODE)
      model->setData(model->index(0, 1),
                     QVariant::fromValue<tlp::Color>(
                         TulipSettings::defaultColor(tlp::NODE, true)));
    else
      model->setData(model->index(0, 2),
                     QVariant::fromValue<tlp::Color>(
                         TulipSettings::defaultColor(tlp::EDGE, true)));
  ...
  case 1: // default size
    if (updateMode == RESET_NODE)
      model->setData(model->index(1, 1),
                     QVariant::fromValue<tlp::Size>(
                         TulipSettings::defaultSize(tlp::NODE, true)));
    else
      model->setData(model->index(1, 2),
                     QVariant::fromValue<tlp::Size>(
                         TulipSettings::defaultSize(tlp::EDGE, true)));
  ...
  case 4: // default label color
    if (updateMode == RESET_NODE)
      model->setData(model->index(4, 1),
                     QVariant::fromValue<tlp::Color>(
                         TulipSettings::defaultLabelColor(true)));
    else
...
void PreferencesDialog::showGraphDefaultsContextMenu(const QPoint &p) {
  ...
  if (idx.column() == 0) {
    ...
    int row = idx.row();
    QString defaultProp =
        _ui->graphDefaultsTable->item(row, idx.column())->text();
    ...
    if (row < 3) {
      QMenu *subMenu =
          contextMenu.addMenu(QString("Reset to Tulip predefined"));
      subMenu->setToolTip(QString(
          "Choose the type of elements for which the default value will be reset"));
      ...
      action =
          contextMenu.addAction(QString("Reset to Tulip predefined value"));
      action->setData(QVariant(int(RESET_BOTH)));
      action->setToolTip(QString("Reset ") + defaultProp +
                         " to the Tulip predefined value");
    }
