...
void PreferencesDialog::usetlpbformat(int state) {
  if (state == Qt::Checked) {
    QMessageBox::warning(this, "Use tlpb file format", "Be careful: using the tlpb file format means faster Tulip project loading/saving but you "
                                                       "will loose compatibility with previous versions of Tulip.");
  }
}
...
void PreferencesDialog::readSettings() {
  ...
  model->setData(model->index(2, 1),
                 QVariant::fromValue<NodeShape::NodeShapes>(static_cast<NodeShape::NodeShapes>(TulipSettings::instance().defaultShape(tlp::NODE))));
  model->setData(model->index(2, 2),
                 QVariant::fromValue<EdgeShape::EdgeShapes>(static_cast<EdgeShape::EdgeShapes>(TulipSettings::instance().defaultShape(tlp::EDGE))));
