...
public:
  ...
  InteractorSelection(const tlp::PluginContext *)
      : NodeLinkDiagramComponentInteractor(FontIconManager::instance()->getMaterialDesignIcon(md::selection, Qt::white),
    ...
    setConfigurationWidgetText(QString("<h3>Selection nodes/edges in a rectangle</h3>") +
                               "Draw selection rectangle.<br/><b>Mouse left</b> down indicates the first corner, <b>Mouse left</b> up indicates the "
                               "opposite corner,<br/>all graph elements instersecting the rectangle are selected<br/><br/>" +
  ...
  bool isCompatible(const std::string &viewName) const {
    return ((viewName == NodeLinkDiagramComponent::viewName) || (viewName == ViewName::GeographicViewName) ||
            (viewName == ViewName::HistogramViewName) || (viewName == ViewName::MatrixViewName) || (viewName == ViewName::PixelOrientedViewName) ||
