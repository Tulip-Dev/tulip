  ...
  void construct() override {
    ...
    setConfigurationWidgetText(QString("<h3>Explore %1</h3>").arg(view()->name().c_str()) +
        ...
        (view()->name() == NodeLinkDiagramComponent::viewName ?
         "<u>3D" : "<u>2D") +" Navigation in the view</u><br/><br/>" +
        ...
        + (view()->name() == NodeLinkDiagramComponent::viewName ?
           (QString("Meta node navigation: <ul><li><b>double Mouse left</b> click go inside the metanode<br/>(press Shift to avoid element properties display)</li>") +
#if !defined(__APPLE__)
