...
public:
  ...
  InteractorNavigation(const tlp::PluginContext *)
      : NodeLinkDiagramComponentInteractor(FontIconManager::instance()->getMaterialDesignIcon(md::cursorpointer, Qt::white), "Navigate in graph") {
    setPriority(StandardInteractorPriority::Navigation);
    setConfigurationWidgetText(QString("<h3>Navigate in graph</h3>") + "3D Navigation in the graph<br/><br/>" +
  ...
  bool isCompatible(const std::string &viewName) const {
    return ((viewName == NodeLinkDiagramComponent::viewName) || (viewName == ViewName::MatrixViewName) ||
