...
PathFinderComponent::PathFinderComponent(PathFinder *parent) : parent(parent), graphPopable(false), timerId(0), lastX(0), lastY(0), glMW(nullptr) {
}
...
void PathFinderComponent::selectPath(GlMainWidget *glMainWidget, Graph *graph) {
  ...
  if (src.isValid() && tgt.isValid()) { // We only select a path if source and target are valid
    ...
    if (!pathFound) {
      ...
      QMessageBox::warning(0, "Path finder", "There exists not path between the selected nodes.");
