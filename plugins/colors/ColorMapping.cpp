...
static const char *paramHelp[] = {
    ...
    "between 0 and the log of the mapped maximum value of graph elements.<BR>"
    ...
    "Finally, if enumerated, the input property can be of <b>any type</b>. Each possible value is mapped to a distinct color without any specific "
    "order.",
...
public:
  ...
  bool run() {
    ...
    if (eltTypes.getCurrent() != ENUMERATED_ELT) {
      ...
      unsigned int maxIter = (targetType.getCurrent() == NODES_TARGET) ? graph->numberOfNodes() : graph->numberOfEdges();
      unsigned int iter = 0;

      for (std::vector<std::pair<std::string, Color>>::iterator it = enumeratedMappingResultVector.begin(); it != enumeratedMappingResultVector.end();
           ++it) {
