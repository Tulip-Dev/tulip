...
#define ELT_TWOLAYERCROSSLIST                                                                                                                        \
  "BarycenterHeuristic;MedianHeuristic;SplitHeuristic;SiftingHeuristic;GreedyInsertHeuristic;GreedySwitchHeuristic;GlobalSiftingHeuristic;"          \
...
static const char *paramHelp[] = {
    ...
    // runs
    "Determines, how many times the crossing minimization is repeated. Each repetition (except for the first) starts with randomly permuted nodes on "
    "each layer. Deterministic behaviour can be achieved by setting runs to 1.",
    ...
    // transpose
    "If this option is set to true an additional fine tuning step is performed after each traversal, which tries to reduce the total number of "
    "crossings by switching adjacent vertices on the same layer.",
...
static const char *twoLayerCrossValuesDescription = "BarycenterHeuristic <i>(the barycenter heuristic for 2-layer crossing minimization)</i><br>"
...
public:
  ...
  PLUGININFORMATION(
      ...
      "Implements the classical layout algorithm by Sugiyama, Tagawa, and Toda. It is a layer-based approach for producing upward drawings.", "1.7",
      "Hierarchical")
