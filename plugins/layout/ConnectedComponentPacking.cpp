...
bool ConnectedComponentPacking::run() {
  ...
  for (unsigned int i = 0; i < ccNodes.size(); ++i) {
    ...
    const std::vector<node> &nodes = ccNodes[i];
    Iterator<node> *itN = new StlIterator<node, std::vector<node>::const_iterator>(nodes.begin(), nodes.end());
    const std::vector<edge> &edges = ccEdges[i];
    Iterator<edge> *itE = new StlIterator<edge, std::vector<edge>::const_iterator>(edges.begin(), edges.end());
