...
//===============================================================
double ConeTreeExtended::treePlace3D(tlp::node n, TLP_HASH_MAP<tlp::node, double> *posRelX, TLP_HASH_MAP<tlp::node, double> *posRelY) {
  (*posRelX)[n] = 0;
  (*posRelY)[n] = 0;
  ...
  for (unsigned int i = 0; i < subCircleRadius.size() - 1; ++i) {
    for (unsigned int j = i + 1; j < subCircleRadius.size(); ++j) {
      newRadius = std::max(newRadius, minRadius(static_cast<float>(subCircleRadius[i]), static_cast<float>(vangles[i]),
                                                static_cast<float>(subCircleRadius[j]), static_cast<float>(vangles[j])));
    }
  }
...
//===============================================================
void ConeTreeExtended::calcLayout(tlp::node n, TLP_HASH_MAP<tlp::node, double> *px, TLP_HASH_MAP<tlp::node, double> *py, double x, double y,
                                  int level) {
