...
RectanglePacking::RectanglePacking(int numberRects)
    : firstSequence(new RectangleRelativePositionList()), placesOfRectanglesInSecondSequence(new int[numberRects]), numberOfPositionnedRectangles(0),
      numberOfRectangles(numberRects), bestPlaceInFirstSequence(INT_MAX), bestPlaceInSecondSequence(INT_MAX), newRectangleWidth(FLT_MAX),
      newRectangleHeight(FLT_MAX), newRectangleLeftAbscissa(FLT_MAX), newRectangleLowOrdinate(FLT_MAX), bestRectangleLeftAbscissa(FLT_MAX),
      bestRectangleLowOrdinate(FLT_MAX), maxWidthOfBoundingBox(FLT_MAX), maxHeightOfBoundingBox(FLT_MAX), bestWidthOfBoundingBox(FLT_MAX),
      bestHeightOfBoundingBox(FLT_MAX) {
}
...
void RectanglePacking::optimalPositionOfNewRectangle(vector<Rectangle<float>>::iterator itNewRect) {
  ...
  for (positionInFirstSequence = 1; positionInFirstSequence <= numberOfPositionnedRectangles + 1; ++positionInFirstSequence) {
    for (positionInSecondSequence = 1; positionInSecondSequence <= numberOfPositionnedRectangles + 1; ++positionInSecondSequence) {
      ...
      if (((ratio <= 1.2f) && ((bestRatio > 1.2f) || (bestPerimeterOfBoundingRectangle > perimeterOfBoundingRectangle))) ||
...
void RectanglePacking::repositionOfRectanglesRightOrAboveNewRectangle(
    list<RectangleRelativePosition>::iterator itFirstRectangleRightOrAboveOfNewRectangle, int positionInFirstSequence, int positionInSecondSequence) {
  ...
  for (itRectOfSequenceToReposition = itFirstRectangleRightOrAboveOfNewRectangle; itRectOfSequenceToReposition != firstSequence->end();
    ...
    if (positionInFirstSequence < positionRectangleToReposition) {
      modificationCoordinatesRectanglesRightOrAboveNewRectangleInFonctionAlreadyRepositionnedRectangles(itFirstRectangleRightOrAboveOfNewRectangle,
                                                                                                        itRectOfSequenceToReposition);
    }
...
void RectanglePacking::modificationOfSequencePair(vector<Rectangle<float>>::iterator itNewRect,
                                                  list<RectangleRelativePosition>::iterator itBestPositionInFirstSequence) {
  ...
  firstSequence->addRectangleRelativePosition(itNewRect, numberOfPositionnedRectangles, newRectangleWidth, newRectangleHeight,
...
void RectanglePacking::coordinatesOfNewRectangle(list<RectangleRelativePosition>::iterator itRectLeftOrBelowOfNewRect, int,
...
void RectanglePacking::modificationCoordinatesRectanglesRightOrAboveNewRectangleInFonctionNewRectangle(
    list<RectangleRelativePosition>::iterator itRectOfSequenceToReposition, int positionInSecondSequence) {
...
void RectanglePacking::modificationCoordinatesRectanglesRightOrAboveNewRectangleInFonctionAlreadyRepositionnedRectangles(
  ...
  for (itRectOfSequenceAlreadyRepositionned = itFirstRectangleRightOrAboveOfNewRectangle;
       itRectOfSequenceAlreadyRepositionned != itRectOfSequenceToReposition; ++itRectOfSequenceAlreadyRepositionned) {
    if (placesOfRectanglesInSecondSequence[itRectOfSequenceAlreadyRepositionned->rectangleNumber - 1] >=
      ...
      if ((highOrdinate = itRectOfSequenceAlreadyRepositionned->rectangleTemporaryLowOrdinate +
                          itRectOfSequenceAlreadyRepositionned->rectangleHeight) > itRectOfSequenceToReposition->rectangleTemporaryLowOrdinate) {
        itRectOfSequenceToReposition->rectangleTemporaryLowOrdinate = highOrdinate;
      }
    } else {
      if ((rightAbscissa = itRectOfSequenceAlreadyRepositionned->rectangleTemporaryLeftAbscissa +
                           itRectOfSequenceAlreadyRepositionned->rectangleWidth) > itRectOfSequenceToReposition->rectangleTemporaryLeftAbscissa) {
        itRectOfSequenceToReposition->rectangleTemporaryLeftAbscissa = rightAbscissa;
      }
    }
  }
...
void RectanglePacking::dimensionsBoundingBoxOfAllOptimalPositionnedRectangles(
  ...
  if ((rightAbscissa = itRectOfSequenceToReposition->rectangleTemporaryLeftAbscissa + itRectOfSequenceToReposition->rectangleWidth) >
      maxWidthOfBoundingBox) {
    maxWidthOfBoundingBox = rightAbscissa;
  }

  if ((highOrdinate = itRectOfSequenceToReposition->rectangleTemporaryLowOrdinate + itRectOfSequenceToReposition->rectangleHeight) >
...
void RectanglePacking::continueColumn(vector<Rectangle<float>>::iterator itr, float &widthTemp, float &heightTemp, bool &boolWidth,
                                      bool &boolHeight) {
...
void RectanglePacking::optimalPositionOfNewRectangleLimPos(vector<Rectangle<float>>::iterator itNewRect, int numberTestedPositions) {
  ...
  for (positionInFirstSequence = numberOfPositionnedRectangles + 1; positionInFirstSequence >= numberOfLastTestedPosition;
