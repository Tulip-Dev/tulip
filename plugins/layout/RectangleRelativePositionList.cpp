...
void RectangleRelativePositionList::addRectangleRelativePosition(
    vector<Rectangle<float>>::iterator itr, int numRect, float wdth, float hght,
    float x, float y,
...
void RectangleRelativePositionList::allocateCoordinates() {
  ...
  for (list<RectangleRelativePosition>::iterator itr = this->begin();
       itr != this->end(); ++itr) {
    (*(itr->rectangleIterator))[0][0] = itr->rectangleLeftAbscissa;
    (*(itr->rectangleIterator))[0][1] = itr->rectangleLowOrdinate;
    (*(itr->rectangleIterator))[1][0] =
        (*(itr->rectangleIterator))[0][0] + itr->rectangleWidth;
    (*(itr->rectangleIterator))[1][1] =
        (*(itr->rectangleIterator))[0][1] + itr->rectangleHeight;
  }
}
...
void RectangleRelativePositionList::stockOfTemporaryBestCoordinates(
  ...
  list<RectangleRelativePosition>::reverse_iterator itRectToReposition =
      this->rbegin();
  int positionRectToReposition;

  for (positionRectToReposition = this->size();
       positionRectToReposition >= bestPlaceInFirstSequence &&
       itRectToReposition != this->rend();
