...
void BubbleTree::calcLayout2(tlp::node n, tlp::Vector<double, 5> &nrPos, TLP_HASH_MAP<tlp::node, tlp::Vector<double, 5>> *relativePosition,
                             const tlp::Vector<double, 3> &enclosingCircleCenter, const tlp::Vector<double, 3> &originNodePosition) {
  /*
  ...
  result->setNodeValue(n, Coord(static_cast<float>(enclosingCircleCenter[0] + zeta[0]), static_cast<float>(enclosingCircleCenter[1] + zeta[1]), 0.));
