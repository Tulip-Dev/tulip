...
void Grip::initialPlacement(unsigned int start, unsigned int end) {
  ...
  for (unsigned int i = start; i <= end; ++i) {
    ...
    Coord alea = Coord(alpha - (2. * alpha * (double)(randomInteger(1))), alpha - (2. * alpha * double(randomInteger(1))),
                       (alpha - (2. * alpha * (double)(randomInteger(1)))));
    ...
    for (unsigned int i = start; i <= end; ++i) {
      ...
      for (unsigned int j = 0; j < neighbors[currNode].size(); ++j) {
        ...
        if (!(euclidian_dist_sqr > 1E-4)) {
          ...
          c_tmp = Coord(alpha - (2. * alpha * (double)(randomInteger(1))), alpha - (2. * alpha * double(randomInteger(1))),
                        alpha - (2. * alpha * (double)(randomInteger(1))));
...
void Grip::init() {
  ...
  forEach(n, currentGraph->getNodes()) {
    Coord alea = Coord(diam - (2. * diam * (double)(randomInteger(1))), diam - (2. * diam * double(randomInteger(1))),
                       diam - (2. * diam * double(randomInteger(1))));
