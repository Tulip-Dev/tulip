...
//======================================================
Grip::Grip(const tlp::PluginContext *context) : LayoutAlgorithm(context), misf(NULL), edgeLength(0), level(0), currentGraph(NULL), _dim(0) {
...
void Grip::initialPlacement(unsigned int start, unsigned int end) {
  ...
  for (unsigned int i = start; i <= end; ++i) {
    ...
    misf->getNearest(currNode, neighbors[currNode], neighbors_dist[currNode], level, levelToNbNeighbors[level + 1]);
  }
  ...
  for (unsigned int i = start; i <= end; ++i) {
    ...
    Coord alea = Coord(alpha - (2. * alpha * (double)(randomInteger(2))), alpha - (2. * alpha * double(randomInteger(2))),
                       (alpha - (2. * alpha * (double)(randomInteger(2)))));
    ...
    for (unsigned int i = start; i <= end; ++i) {
      ...
      for (unsigned int j = 0; j < neighbors[currNode].size(); ++j) {
        ...
        if (!(euclidian_dist_sqr > 1E-4)) {
          ...
          c_tmp = Coord(alpha - (2. * alpha * (double)(randomInteger(2))), alpha - (2. * alpha * double(randomInteger(2))),
                        alpha - (2. * alpha * (double)(randomInteger(2))));
...
void Grip::init() {
  ...
  forEach(n, currentGraph->getNodes()) {
    Coord alea = Coord(diam - (2. * diam * (double)(randomInteger(2))), diam - (2. * diam * double(randomInteger(2))),
                       diam - (2. * diam * double(randomInteger(2))));
...
void Grip::set_nbr_size() {
  ...
  node n;
  forEach(n, currentGraph->getNodes()) maxCxty += currentGraph->deg(n);
  ...
  for (unsigned int i = 1; i < misf->index.size(); ++i) {
    if (i >= maxLevel)
      levelToNbNeighbors[i] =
          min((unsigned int)(sched(misf->index.size() - i, 0, 2, 10000, 1) * maxCxty / (misf->index[i])), (unsigned int)(misf->index[i] - 1));
    else
  ...
  if (misf->index.size() >= maxLevel)
    levelToNbNeighbors[misf->index.size()] =
        min((unsigned int)(sched(currentGraph->numberOfNodes(), 0, 2, 10000, 1) * (float)maxCxty / ((float)currentGraph->numberOfNodes())),
            (unsigned int)(currentGraph->numberOfNodes() - 1));
  else
