...
void MISFiltering::bfsDepth(node n, unsigned int depth) {
  ...
  for (unsigned int i = 0; i < nextNodes.size(); ++i) {
    ...
    for (node v : g_copy->getInOutNodes(current)) {
...
//========================================
void MISFiltering::getNearest(node n, vector<node> &neighbors, vector<unsigned int> &neighbors_dist, unsigned int level, unsigned int nbNeighbors) {
  vector<node> nextNodes;
