...
void ImprovedWalker::firstWalk(tlp::node v) {
  if (isLeaf(tree, v)) {
    ...
    if (vleftSibling != BADNODE)
      prelimX[v] += prelimX[vleftSibling] + nodeSpacing + oriSize->getNodeValue(v).getW() / 2.f + oriSize->getNodeValue(vleftSibling).getW() / 2.f;
  } else {
    ...
    if (leftBrother != BADNODE) {
      prelimX[v] = prelimX[leftBrother] + nodeSpacing + oriSize->getNodeValue(v).getW() / 2.f + oriSize->getNodeValue(leftBrother).getW() / 2.f;
...
void ImprovedWalker::combineSubtree(tlp::node v, tlp::node *defaultAncestor) {
  ...
  if (leftBrother != BADNODE) {
    ...
    while (nextRightContour(nodeInsideLeft) != BADNODE && nextLeftContour(nodeInsideRight) != BADNODE) {
      ...
      float shift = (prelimX[nodeInsideLeft] + shiftInsideLeft) - (prelimX[nodeInsideRight] + shiftInsideRight) + nodeSpacing +
                    oriSize->getNodeValue(nodeInsideLeft).getW() / 2.f + oriSize->getNodeValue(nodeInsideRight).getW() / 2.f;
    ...
    if (nextRightContour(nodeInsideLeft) != BADNODE && nextRightContour(nodeOutsideRight) == BADNODE) {
      thread[nodeOutsideRight] = nextRightContour(nodeInsideLeft);
      modChildX[nodeOutsideRight] += shiftInsideLeft - shiftOutsideRight;
    }

    if (nextLeftContour(nodeInsideRight) != BADNODE && nextLeftContour(nodeOutsideLeft) == BADNODE) {
