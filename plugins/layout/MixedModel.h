 ...
 * subgraphs, which is a maximal planar "sub-map". Then an area aware version of
 * Gutwenger and Mutzel 's algorithm is used, and if the connected component was
 * not planar, it adds the "unplanar" edges in 3D. Finally, it uses the
 * Connected Component Packing plugin of Tulip Software to pack the connected
 * components.\n
 *
 */
...
public:
  PLUGININFORMATION(
      "Mixed Model", "Romain BOURQUI ", "09/11/2005",
...
private:
  std::vector<tlp::edge>
  getPlanarSubGraph(tlp::PlanarConMap *graph,
