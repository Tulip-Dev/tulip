...
LinLogLayout::LinLogLayout(tlp::Graph *_graph, tlp::PluginProgress *_pluginProgress)
    : edgeWeight(nullptr), layoutResult(nullptr), linLogWeight(nullptr), skipNodes(nullptr), graph(_graph), pluginProgress(_pluginProgress), _dim(2),
      _nbNodes(0) {
...
bool LinLogLayout::initAlgo(tlp::LayoutProperty *_layout, tlp::NumericProperty *_weight, double _attrExponent, double _repuExponent,
                            double _gravFactor, unsigned int _max_iter, bool _is3D, bool _useOctTree, tlp::BooleanProperty *_skipNodes) {
  ...
  for (int step = 1; step <= nrIterations; ++step) {
    ...
    if (nrIterations >= 50 && finalRepuExponent < 1.0) {
      ...
      if (step <= 0.6 * nrIterations) {
        ...
        attrExponent += 1.1 * (1.0 - finalRepuExponent) * (0.9 - ((double)step) / nrIterations) / 0.3;
        repuExponent += 0.9 * (1.0 - finalRepuExponent) * (0.9 - ((double)step) / nrIterations) / 0.3;
      }
    }
...
OctTree *LinLogLayout::buildOctTree() {
  ...
  for (node u : linLogWeight->getNonDefaultValuatedNodes()) {
    const Coord &position = layoutResult->getNodeValue(u);
  ...
  OctTree *result = new OctTree(graph->getOneNode(), zero, minPos, maxPos, linLogWeight, true);
