 ...
 */
OctTree::OctTree(tlp::node _node, Coord _position, Coord _minPos, Coord _maxPos, tlp::DoubleProperty *_linLogWeight, bool _firstNode)
    : position(_position[0], _position[1], _position[2]), minPos(_minPos[0], _minPos[1], _minPos[2]), maxPos(_maxPos[0], _maxPos[1], _maxPos[2]) {
  ...
  // childrenSize = 0;
  childCount = 0;
  _children = nullptr; // new OctTree*[8]; //init à nullptr?
  this->weight = 0.0;
...
void OctTree::printTree(unsigned int depth) {
  ...
  std::cerr << "[d(" << depth << "),w(" << weight << "),n(" << node.id << "),l(" << isLeaf << "),p(" << position[0] << "," << position[1] << ","
            << position[2] << "),";
      ...
      while (endwhile == false) {
        if (childIndex < MAX_CHILDREN) {
          ...
          /*          else if ((_children[childIndex])->node == nullptr || oldNode == nullptr)
                {
                //std::cerr<<"this tree node or the searched one is nullptr: "<<oldNode<<" and "<<(_children[childIndex])->node<<"\n";
                childIndex++;
                }
          */
