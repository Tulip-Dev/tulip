...
static const char *paramHelp[] = {
    ...
    // initial layout
    "The layout property used to compute the initial position of the graph elements. If none is given the initial position will be computed by the "
    "algorithm.",

    // selection of unmovable nodes
    "This property is used to indicate the unmovable nodes, the ones for which a new position will not be computed by the algorithm. This property "
    "is taken into account only if a layout property has been given to get the initial position of the unmovable nodes.",

    // max iterations
    "This parameter allows to choose the number of iterations. The default value of 0 corresponds to (3 * nb_nodes * nb_nodes) if the graph has more "
    "than 100 nodes."
...
GEMLayout::GEMLayout(const tlp::PluginContext *context)
    : LayoutAlgorithm(context), Iteration(0), _temperature(0), _maxtemp(0), _oscillation(0), _rotation(0), i_maxtemp(IMAXTEMPDEF),
      a_maxtemp(AMAXTEMPDEF), i_starttemp(ISTARTTEMPDEF), a_starttemp(ASTARTTEMPDEF), i_finaltemp(IFINALTEMPDEF), a_finaltemp(AFINALTEMPDEF),
      i_maxiter(IMAXITERDEF), a_maxiter(AMAXITERDEF), i_gravity(IGRAVITYDEF), a_gravity(AGRAVITYDEF), i_oscillation(IOSCILLATIONDEF),
      a_oscillation(AOSCILLATIONDEF), i_rotation(IROTATIONDEF), a_rotation(AROTATIONDEF), i_shake(ISHAKEDEF), a_shake(ASHAKEDEF), _dim(2),
      _nbNodes(0), _useLength(false), metric(nullptr), fixedNodes(nullptr), max_iter(0) {
