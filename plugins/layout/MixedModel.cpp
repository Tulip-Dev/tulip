...
bool MixedModel::run() {
  ...
  if (nbConnectedComponent != 1) {
    ...
    graph->applyPropertyAlgorithm(string("Connected Component Packing"), &layout, err, nullptr, &tmp);
...
void MixedModel::assignInOutPoints() { // on considère qu'il n'y a pas d'arc double
  ...
  for (unsigned int k = 0; k < V.size(); ++k) {
    ...
    if (pluginProgress->progress(minProgress + (deltaProgress * k) / V.size(), 1000) != TLP_CONTINUE)
      return;
...
void MixedModel::computeCoords() {
  ...
  for (unsigned int i = 0; i < V[0].size(); ++i) {
    ...
    if (i == 0)
      x = (out_l < static_cast<double>(c.getX() / 2.f)) ? (c.getX() / 2.f) : static_cast<float>(out_l);
    else
      x = static_cast<float>(out_r_moins1) + ((out_l < static_cast<double>(c.getX() / 2.f)) ? (c.getX() / 2.f) : static_cast<float>(out_l)) + spacing;
  ...
  for (unsigned int k = 1; k < size_V; ++k) { // parcours de chaque partition
    ...
    int dxl = (int)(OutPoints[EdgesIN[V[k][0]][0]].getX()), dxr = (int)(OutPoints[EdgesIN[V[k][p - 1]][n - 1]].getX());
