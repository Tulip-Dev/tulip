...
static void genCylinderGlyph(const float zBottom, const float zTop, vector<Coord> &vertices, vector<unsigned short> &indices, vector<Vec2f> &texCoords) {
  ...
  for (unsigned short i = 0 ; i < nbContourPoints - 1 ; ++i) {
    indices.insert(indices.end(), {ushort_cast(startIdx+i), ushort_cast(startIdx+i+1), ushort_cast(startIdx+i+nbContourPoints),
                                   ushort_cast(startIdx+i+1), ushort_cast(startIdx+i+nbContourPoints+1), ushort_cast(startIdx+i+nbContourPoints)});
  }

  indices.insert(indices.end(), {ushort_cast(startIdx+nbContourPoints-1), startIdx, ushort_cast(startIdx+2*nbContourPoints-1),
                                 startIdx, ushort_cast(startIdx+nbContourPoints), ushort_cast(startIdx+2*nbContourPoints-1)});
...
public:
  ...
  GLYPHINFORMATION("3D - Half Cylinder", "3D - Half Cylinder extremity", "Antoine Lambert", "20/05/2016", "Half Cylinder", "1.0", tlp::NodeShape::HalfCylinder)
