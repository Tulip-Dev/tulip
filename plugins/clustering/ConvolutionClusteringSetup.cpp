...
public:
  HistogramWidget(ConvolutionClusteringSetup *dialog, QWidget *parent) : QWidget(parent), setupDialog(dialog) {
  }
  ...
  void paintEvent(QPaintEvent *) {
    ...
    painter.fillRect(0, 0, 2 * histogram.size() + legendWidth, histogram.size() + legendWidth, QBrush(QColor(255, 255, 255)));
    // draw bars
    QColor c;
    ...
    painter.drawLine(borderWidth, histogram.size() + borderWidth, 2 * histogram.size() + axisWidth, histogram.size() + borderWidth);
    ...
    while (!localMinimum.empty()) {
      ...
      painter.drawLine(borderWidth + i * 2, borderWidth, borderWidth + i * 2, histogram.size() + borderWidth);
    }
  }
