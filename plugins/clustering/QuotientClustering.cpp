  ...
  bool run() {
    ...
    if (clustersLayout) {
      for(tlp::Graph *cluster : graph->getSubGraphs()) {
        ...
        cluster->applyPropertyAlgorithm("Fast Overlap Removal", cluster->getLocalProperty<LayoutProperty>("viewLayout"), errMsg, nullptr, &layoutParams);
      }
    }
    ...
    if (quotientLayout) {
      ...
      quotientGraph->applyPropertyAlgorithm(layoutName, quotientGraph->getLocalProperty<LayoutProperty>("viewLayout"), errMsg, nullptr, &layoutParams);
      double border = std::min(minSize[0], minSize[1]);
      layoutParams.set("x border", border);
      layoutParams.set("y border", border);
      quotientGraph->applyPropertyAlgorithm("Fast Overlap Removal", quotientGraph->getLocalProperty<LayoutProperty>("viewLayout"), errMsg, nullptr, &layoutParams);
    }
