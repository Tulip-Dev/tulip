...
static const char *paramHelp[] = {
    ...
    // meta-node label
    "Property used to label meta-nodes. An arbitrary underlying node is chosen and its associated value for the given property becomes the meta-node "
    "label.",
...
public:
  void computeMetaValue(AbstractProperty<IntegerType, IntegerType, NumericProperty> *card, edge mE, Iterator<edge> *itE, Graph *) {
...
public:
  ...
  bool run() {
    ...
    // for most properties
    DoubleProperty::PredefinedMetaValueCalculator nodeFn = (DoubleProperty::PredefinedMetaValueCalculator)nodeFunctions.getCurrent();
    ...
    TLP_HASH_MAP<PropertyInterface *, PropertyInterface::MetaValueCalculator *>::iterator itC = prevCalcs.begin();
    ...
    if (!oriented) {
      ...
      while (itE->hasNext()) {
        ...
        if (op.isValid() && edgesToDel.find(mE) == edgesToDel.end() && edgesToDel.find(op) == edgesToDel.end()) {
