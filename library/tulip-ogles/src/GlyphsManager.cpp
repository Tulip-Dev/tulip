...
GlyphsManager::GlyphsManager() {
  ...
  FontIconGlyph *fontIconGlyph = new FontIconGlyph(nullptr, TulipFontAwesome::getFontAwesomeTrueTypeFileLocation(),
                                                   TulipFontAwesome::getFontAwesomeIconCodePoint(TulipFontAwesome::QuestionCircle));

  _glyphs[circleGlyph->id()] = circleGlyph;
  _glyphs[triangleGlyph->id()] = triangleGlyph;
  _glyphs[fontIconGlyph->id()] = fontIconGlyph;
}
...
Glyph *GlyphsManager::getGlyph(int glyphId) {
  ...
  if (glyphId >= 0x1e000) {
    if (_iconGlyphs.find(glyphId) == _iconGlyphs.end()) {
      _iconGlyphs[glyphId] = new FontIconGlyph(nullptr, TulipMaterialDesignIcons::getMaterialDesignIconsTrueTypeFileLocation(), glyphId - 0xf000);
    }
    return _iconGlyphs[glyphId];
  } else if (glyphId >= 0xf000) {
    if (_iconGlyphs.find(glyphId) == _iconGlyphs.end()) {
      _iconGlyphs[glyphId] = new FontIconGlyph(nullptr, TulipFontAwesome::getFontAwesomeTrueTypeFileLocation(), glyphId);
    }
    return _iconGlyphs[glyphId];
  }
