...
/**
 * @ingroup OpenGL
 * @brief Class to create a complex polygon (concave polygon or polygon with
 * hole) If you want to create a complex polygon you have 4 constructors :
 ...
 *     GlComplexPolygon *complexPolygon=new
 * GlComplexPolygon(coords,Color(255,0,0,255));
 *     layer->addGlEntity(complexPolygon,"complexPolygon");
 * \endcode
 *
 * Constructors with vector of vector of Coords : to create a complex polygon
 * with hole
 *   - In this case you have two constructor : with and without outline color
 ...
 * \endcode
 *
 * In constructors you can specify the polygon border style : polygonEdgesType
 * parameter (0 -> straight lines, 1 -> catmull rom curves, 2 -> bezier curves)
 * You can also specify the texture name if you want to create a textured
 * complex polygon
 *
 * In complex polygon you can add a smooth border : see activateQuadBorder(..)
 * function And you can specify the texture zoom : see setTextureZoom(...)
 * function
 */
...
public:
   ...
   * @warning don't use this constructor if you want to create a complex
   * polygon, see others constructors
   */
  GlComplexPolygon() {}
  /**
   * @brief Constructor with a vector of coords, a fill color, a polygon edges
   * type(0 -> straight lines, 1 -> catmull rom curves, 2 -> bezier curves) and
   * a textureName if you want
   */
  GlComplexPolygon(const std::vector<Coord> &coords, Color fcolor,
                   int polygonEdgesType = 0,
                   const std::string &textureName = "", bool textured = true);
  /**
   * @brief Constructor with a vector of coords, a fill color, an outline color,
   * a polygon edges type(0 -> straight lines, 1 -> catmull rom curves, 2 ->
  ...
  GlComplexPolygon(const std::vector<Coord> &coords, Color fcolor, Color ocolor,
                   ...
                   const std::string &textureName = "", bool textured = true);
  /**
   * @brief Constructor with a vector of vector of coords (the first vector of
   * coord is the polygon and others vectors are holes in polygon), a fill
   * color, a polygon edges type(0 -> straight lines, 1 -> catmull rom curves, 2
   * -> bezier curves) and a textureName if you want
   */
   ...
   * @brief Constructor with a vector of vector of coords (the first vector of
   * coord is the polygon and others vectors are holes in polygon), a fill
   * color, an outline color a polygon edges type(0
   * -> straight lines, 1 -> catmull rom curves, 2 -> bezier curves) and a
   * textureName if you want
   */
  GlComplexPolygon(const std::vector<std::vector<Coord>> &coords, Color fcolor,
                   Color ocolor, int polygonEdgesType = 0,
                   const std::string &textureName = "", bool textured = true);
   ...
   *
   * By default if you have a polygon with a size bigger than (1,1,0) the
   * texture will be repeated If you want to don't have this texture repeat you
   * have to modify texture zoom For example if you have a polygon with coords
   * ((0,0,0),(5,0,0),(5,5,0),(0,5,0)) you can set texture zoom to 5. to don't
   * have texture repeat
   */
   ...
   */
  bool textureActivation() { return textured; }
   ...
   * The position parameter determines the way the border is drawn (depending on
   * the polygon points ordering):
   ...
   *
   * The texCoordFactor parameter determines the way the texture is applied : if
   * < 1, the texture will be expanded and > 1, the texture will be compressed
   * The polygonId parameter determines on which contour of the polygon, the
