 ...
 *
 * This class allow to render an axis with graduations. This class is there for
 * code factorisation and should not be used directly. Use derivated classes
 * instead : GlQuantitativeAxis for a numerical graduated axis and
 * GlNominativeAxis for a string graduated axis
 */
...
public:
   ...
   * @param axisBaseCoord the base coord of the axis (if the axis is horizontal,
   * it is the the left end, if vertical it is the down end)
   * @param axisLength the length of the axis
   * @param axisOrientation the orientation of the axis, 2 possible values
   * (HORIZONTAL_AXIS or VERTICAL_AXIS)
   * @param axisColor the color of the axis
   */
  GlAxis(const std::string &axisName, const Coord &axisBaseCoord,
   ...
   */
  const Coord &getAxisBaseCoord() const { return axisBaseCoord; }
  /**
   ...
   *
   * It has to be called when one (ore more) of the setters methods above has
   * been used This method erase the whole axis drawing and redraw the axis line
   * and the caption (if any) The axis graduations have to be reset by calling
   * setAxisGraduations
   */
  virtual void updateAxis();

  /**
   * @brief Method to set the axis graduations. No need to call updateAxis after
   * calling this method.
   *
   * @param axisGradsLabels the labels of the graduations, they will be equally
   * spaced on the axis
   * @param axisGradsLabelsPosition the relative position of the axis
   * graduations label. Two possible values : LEFT_OR_BELOW (if the axis is
   * horizontal, labels will be on the left of the axis, otherwise below) or
   * RIGHT_OR_ABOVE
   *
   */
  void setAxisGraduations(
  ...
  /**
   * @brief Method which adds a caption to the axis. No need to call updateAxis
   * after calling this method.
   *
   * @param captionPos the relative position of the caption. Two possible values
   * : LEFT_OR_BELOW (if the axis is vertical, caption will be below of the
   * axis, otherwise on the left) or RIGHT_OR_ABOVE
   * @param captionHeight the caption text height
   * @param captionFrame if true the caption will be framed
   * @param maxCaptionWidth fill this parameter if you want to restrain the
   * caption width
   * @param captionOffset fill this parameter if you want to fix the offset
   * between the axis and the caption
   * @param caption if this parameter is filled, use this value as caption text,
   * otherwise the caption text will be the axis name
   */
  void addCaption(const CaptionLabelPosition &captionPos,
                  const float captionHeight, const bool captionFrame = false,
