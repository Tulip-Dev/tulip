 ...
 *
 * This class perform LOD computation of GlEntities based on screen projection
 * of entities bounding boxes \warning By default this class don't compute LOD
 * for edges (for optimisation) and return a lod of
 * 10. to these edges, if you want to compute edges' LOD call
 * setComputeEdgesLOD(true)
 */
...
public:
   ...
   */
  void addNodeBoundingBox(unsigned int id, unsigned int pos,
                          const BoundingBox &bb) override;
  /**
   ...
   * If you want to compute LOD for a simple scene, you just have to call this
   * function with the same value for globalViewport and currentViewport But if
   * you want to perform a sub screen part selection you have to call this
   * function with: globalViewport the viewport of the visualisation and
   * currentViewport the viewport of the selection \param globalViewport is used
   * to compute LOD \param currentViewport return -1 for all entities outside
   * this viewport
   */
   ...
   */
  inline void setComputeEdgesLOD(bool state) { computeEdgesLOD = state; }
...
protected:
  ...
  virtual void computeFor2DCamera(LayerLODUnit *layerLODUnit,
                                  const Vector<int, 4> &globalViewport,
                                  const Vector<int, 4> &currentViewport);
