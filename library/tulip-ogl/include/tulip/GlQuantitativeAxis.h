...
/**
 * \brief A class to render an axis graduated with numerical values for a given
 * range
 *
 * This class allows to draw a quantitative axis (i.e. an axis axis graduated
 * with numerical values for a given range)
 */
class TLP_GL_SCOPE GlQuantitativeAxis : public GlAxis {

public:
  /**
   * GlQuantitativeAxis constructor. Create an quantitative axis without
   * graduations (need to call setAxisParameters to build them)
   *
   ...
   * \addArrow If true, an arrow will be added to one end of the axis according
   * to the axis order (ascending or descending) \ascendingOrder If true, the
   * min value will be at the bottom end and the max will be at the top end if
   * the axis is vertical (min at the left and max at the right if it is
   * horizontal). If false this positions are switched
   */
  ...
  /**
   * Method to set the quantitative axis parameters. A call to updateAxis has to
   ...
   * vertical, labels will be on the left of the axis, otherwise below) or
   * RIGHT_OR_ABOVE \param drawFirstLabel If false, the first graduation label
   * will not be drawn (useful when some axes have the same base coord to avoid
   * labels overlapping)
   */
  void setAxisParameters(
      const double min, const double max, const unsigned int nbGraduations,
  ...
  void setAxisParameters(
      const int min, const int max, const unsigned int incrementStep,
      const LabelPosition &axisGradsLabelsPosition = LEFT_OR_BELOW,
      const bool drawFirstLabel = true) {
    setAxisParameters(static_cast<long long>(min), static_cast<long long>(max),
                      static_cast<unsigned long long>(incrementStep),
                      axisGradsLabelsPosition, drawFirstLabel);
  }
  ...
  /**
   * Method to set a logarithmic scale on the axis. A call to updateAxis has to
   * be done after calling this method to build or update the axis graduations
   *
   ...
   * descending). A call to updateAxis has to be done after calling this method
   * to build or update the axis graduations
   */
   ...
   *
   * \param axisPointCoord the axis point coordinates we want to retrieve
   * associated value
   */
