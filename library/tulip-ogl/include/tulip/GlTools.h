...
TLP_GL_SCOPE void
glTest(const std::string &message = std::string("(no description)"),
...
/*
 * Project point on screen according to the transformation matrix (modelview *
 * projection) and the viewport (x, y, width, height) given in parameter.
 */
TLP_GL_SCOPE Coord projectPoint(const Coord &obj, const MatrixGL &,
                                const Vector<int, 4> &viewport);
/*
 * UnProject point from screen to 3D world according to the inverse
 * transformation matrix (modelview
 * * projection)^-1
 ...
 * @since Tulip 4.8
 *
 * @param vertices a vector containing the vertices to compute associated
 * normals
 * @param facesIndices a vector containing the unsigned short indices of the
 * triangles faces (its size must be a multiple of 3)
 * @return a vector, with the same size as the one holding vertices, filled with
 * the computed normals
 *
 */
TLP_GL_SCOPE std::vector<Coord>
 ...
 * @param vertices a vector containing the vertices to compute associated
 * normals
 * @param facesIndices a vector containing the unsigned int indices of the
 * triangles faces (its size must be a multiple of 3)
 * @return a vector, with the same size as the one holding vertices, filled with
 * the computed normals
 *
...
TLP_GL_SCOPE void
tesselateFontIcon(const std::string &fontFile, unsigned int iconCodePoint,
