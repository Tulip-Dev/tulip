...
public:
   ...
   */
  GlEdge(unsigned int eId = UINT_MAX, unsigned int ePos = UINT_MAX,
         bool sel = false)
   ...
   * Return the edge bounding box
   */
  BoundingBox getBoundingBox(const GlGraphInputData *data, const edge e,
   ...
   */
  size_t getVertices(const GlGraphInputData *data, const edge e, const node src,
                     const node tgt, Coord &srcCoord, Coord &tgtCoord,
   ...
   */
  void getColors(const GlGraphInputData *data, const node src, const node tgt,
   ...
   */
  void getEdgeSize(const GlGraphInputData *data, edge e, const Size &srcSize,
                   const Size &tgtSize, const float maxSrcSize,
                   const float maxTgtSize, Size &edgeSize);

  /**
   * Compute edge anchor
   */
  void getEdgeAnchor(const GlGraphInputData *data, const node src,
                     const node tgt, const LineType::RealType &bends,
                     const Coord &srcCoord, const Coord &tgtCoord,
                     const Size &srcSize, const Size &tgtSize, Coord &srcAnchor,
                     Coord &tgtAnchor);

  void setSelectionDraw(bool selectDraw) { selectionDraw = selectDraw; }
...
private:
   ...
   */
  BoundingBox eeGlyphBoundingBox(const Coord &anchor, const Coord &tgt,
                                 float glyphNrm,
  ...
  void getEdgeColor(const GlGraphInputData *data, const edge e, const node src,
                    const node tgt, bool selected, Color &srcCol,
                    Color &tgtCol);
   ...
   */
  float getEdgeWidthLod(const Coord &edgeCoord, const Size &edgeSize,
                        Camera *camera);

  /**
   * Thgis function is used to render edge arrows
   */
  void displayArrowAndAdjustAnchor(
      ...
      bool selected, float selectionOutlineSize, int tgtEdgeGlyph,
      bool hasBends, const Coord &anchor, const Coord &tgtCoord,
      const Coord &srcAnchor, const Coord &tgtAnchor, Coord &lineAnchor,
      EdgeExtremityGlyph *srcEdgeGlyph = nullptr, Camera *camera = nullptr);
};
} // namespace tlp
