...
public:
  ...
  /**
   * Enter into a data node and set the currentPosition integer to the position
   * next the data tag
   */
  static void enterDataNode(const std::string &inString,
                            unsigned int &currentPosition);

  /**
   * Leave a data node and set the currentPosition integer to the position next
   * the data end tag
   */
  static void leaveDataNode(const std::string &inString,
                            unsigned int &currentPosition);

  /**
   * Create and enter into a new children node with name : name
   */
  static void beginChildNode(std::string &outString,
                             const std::string &name = "children");

  /**
   * End last children node with name : name
   */
  static void endChildNode(std::string &outString,
                           const std::string &name = "children");

  /**
   * Enter into a child node and set the currentPosition integer to the position
   * next the child tag \return child name
   */
  static std::string enterChildNode(const std::string &inString,
                                    unsigned int &currentPosition);

  /**
   * Leave into a child node and set the currentPosition integer to the position
   * next the child end tag \return child name
   */
  static void leaveChildNode(const std::string &inString,
                             unsigned int &currentPosition,
  ...
  getProperties(const std::string &inString, unsigned int &currentPosition);
   ...
   */
  template <typename Obj>
  static void setWithXML(const std::string &inString,
                         unsigned int &currentPosition, const std::string &name,
                         std::vector<Obj> &vect) {
    ...
    std::istringstream is(
        inString.substr(currentPosition, endValuePosition - currentPosition));
    Obj data;
    char c = is.get();
  ...
  static bool checkNextXMLtag(const std::string &inString,
                              unsigned int &currentPosition,
  ...
  static void setWithXML(const std::string &inString,
    ...
    if (nameTag == "<" + name + ">") {
      ...
      size_t endValuePosition =
          inString.find("</" + name + ">", currentPosition);
      assert(endValuePosition != std::string::npos);
...
private:
  ...
  static void goToNextCaracter(const std::string &inString,
                               unsigned int &currentPosition);
