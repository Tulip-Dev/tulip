...
static string catmullRomSpecificShaderCode =
    ...
    "(d12alpha*pEnd-d22alpha*pBefore+(2*d12alpha+3*d1alpha*d2alpha+d22alpha)*pStart)/"
    "(3*d1alpha*(d1alpha+d2alpha));"
    "	bezierControlPoints[2] = "
    "(d32alpha*pStart-d22alpha*pAfter+(2*d32alpha+3*d3alpha*d2alpha+d22alpha)*pEnd)/"
    ...
    "	}"
    "	if (i == 0) {"
    "		computeBezierSegmentControlPoints(closedCurve ? getControlPoint(nbControlPoints - "
    "2) : getControlPoint(i) - (getControlPoint(i+1) - getControlPoint(i)), getControlPoint(i), "
    "getControlPoint(i+1), getControlPoint(i+2));"
    "	} else if (i == nbControlPoints - 2) {"
    "		computeBezierSegmentControlPoints(getControlPoint(i-1), getControlPoint(i), "
    "getControlPoint(i+1), closedCurve ? getControlPoint(1) : getControlPoint(i+1) + "
    "(getControlPoint(i+1) - getControlPoint(i)));"
    "	} else if (i == nbControlPoints - 1) {"
    "		computeBezierSegmentControlPoints(getControlPoint(i-2), getControlPoint(i-1), "
    "getControlPoint(i), closedCurve ? getControlPoint(1) : getControlPoint(i) + "
    "(getControlPoint(i) - getControlPoint(i-1)));"
    "	} else {"
    "		computeBezierSegmentControlPoints(getControlPoint(i-1), getControlPoint(i), "
    "getControlPoint(i+1), getControlPoint(i+2));"
    "	}"
    ...
    "	float s2 = s * s;"
    "	float s3 = s2	* s;"
    "	return (bezierControlPoints[0] * s3 + bezierControlPoints[1] * 3.0 * localT * s2 + "
    "bezierControlPoints[2] * 3.0 * t2 * s + bezierControlPoints[3] * t3);"
    "}";
