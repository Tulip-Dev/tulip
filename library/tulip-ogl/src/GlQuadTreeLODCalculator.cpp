...
GlQuadTreeLODCalculator::GlQuadTreeLODCalculator() : haveToCompute(true),haveToInitObservers(true),currentGraph(nullptr),layoutProperty(nullptr),sizeProperty(nullptr),selectionProperty(nullptr) {
}
...
#endif
    {
      if((renderingEntitiesFlag & RenderingSimpleEntities)!=0 && entitiesQuadTree[quadTreesVectorPosition]!=nullptr) {
        if(aX==0 && aY==0) {
          if((renderingEntitiesFlag & RenderingWithoutRemove)==0)
