...
static string genCommonUniformVariables(bool canUseTBO) {
  ostringstream oss;

  oss << "#version 120" << endl;

  if (canUseTBO) {
    oss << "#extension GL_EXT_gpu_shader4 : enable" << endl;
    oss << "uniform samplerBuffer controlPoints;" << endl;
    oss << "uniform int nbControlPoints;" << endl;
    oss << "vec3 getControlPoint(int index) {" << endl;
    oss << "	return texelFetchBuffer(controlPoints, index).xyz;" << endl;
    oss << "}" << endl;
  }
  else {
    ...
    oss << "vec3 getControlPoint(int index) {" << endl;
    oss << "	return texture1D(controlPoints, float(index) / float(controlPointsTexSize - 1)).xyz;" << endl;
    oss << "}" << endl;
  }
...
void AbstractGlCurve::initShader(const std::string &shaderProgramName, const std::string &curveSpecificShaderCode) {
  ...
  if (glVendorOk && GlShaderProgram::shaderProgramsSupported()) {
    ...
    if (curvesShadersMap.find(shaderProgramName) == curvesShadersMap.end()) {
      ...
      curvesShadersMap[shaderProgramName]->addShaderFromSourceCode(Vertex, genCommonUniformVariables(canUseTBO) + curveSpecificShaderCode);
    ...
    if (curvesBillboardShadersMap.find(shaderProgramName) == curvesBillboardShadersMap.end()) {
      ...
      curvesBillboardShadersMap[shaderProgramName]->addShaderFromSourceCode(Vertex, genCommonUniformVariables(canUseTBO) + curveSpecificShaderCode);
...
void AbstractGlCurve::drawCurve(std::vector<Coord> &controlPoints, const Color &startColor, const Color &endColor, const float startSize, const float endSize, const unsigned int nbCurvePoints) {
  ...
  if (curveShaderProgram != NULL && canUseFloatTextures && renderMode != GL_SELECT) {
    ...
    static GLuint controlPointsBufId = 0;

    if (controlPointsTexId == 0) {
      glGenTextures(1, &controlPointsTexId);

      if (canUseTBO) {
        glGenBuffers(1, &controlPointsBufId);
      }
    }
    ...
    for (size_t i = 0 ; i < controlPoints.size() ; ++i) {
      ...
      controlPointsData[4*i+2] = controlPoints[i][2];
    }

    if (canUseTBO) {
      glBindBuffer(GL_TEXTURE_BUFFER_ARB, controlPointsBufId);
      glBufferData(GL_TEXTURE_BUFFER_ARB, controlPoints.size() * 4 * sizeof(float), &controlPointsData[0], GL_STREAM_DRAW);
      ...
      glTexImage1D(GL_TEXTURE_1D, 0, GL_RGBA32F_ARB, 1024, 0, GL_RGBA, GL_FLOAT, &controlPointsData[0]);
    ...
    if (canUseTBO) {
      glBindTexture(GL_TEXTURE_BUFFER_ARB, controlPointsTexId);
      glTexBufferARB(GL_TEXTURE_BUFFER_ARB, GL_RGBA32F_ARB, controlPointsBufId);
    }
    else {
      glBindTexture(GL_TEXTURE_1D, controlPointsTexId);
    }
