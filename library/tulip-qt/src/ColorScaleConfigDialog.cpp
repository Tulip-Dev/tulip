...
ColorScaleConfigDialog::ColorScaleConfigDialog(ColorScale *colorScale, QWidget *parent)
	: QDialog(parent), colorScale(colorScale) {
	setupUi(this);
...
void ColorScaleConfigDialog::setPredefinedGradientsDirectory(const std::string &gradientsDir) {
	gradientsImageDirectory = gradientsDir;
	QDir qdir(gradientsDir.c_str());
	QStringList gradientsImageList = qdir.entryList(QDir::Files | QDir::NoDotAndDotDot);
	...
	if (tabWidget->currentIndex() == 1) {
	  if (predefinedColorScaleRb->isChecked()) {
	    if(predefinedGradientsList->count()>0){
	      setColorScaleFromImage(QString(gradientsImageDirectory.c_str()) + predefinedGradientsList->currentItem()->text());
	    }
		...
		for (int i = 0 ; i < colorsTable->rowCount() ; ++i) {
			...
			colors.push_back(Color(itemColor.red(), itemColor.green(), itemColor.blue(), itemColor.alpha()));
		}
...
void ColorScaleConfigDialog::setColorScaleFromImage(const QString &imageFilePath) {
...
void ColorScaleConfigDialog::displayImageGradientPreview() {
	...
	if (predefinedColorScaleRb->isChecked()) {
	  if(predefinedGradientsList->count()>0){
	    gradientFilePath = QString(gradientsImageDirectory.c_str()) + predefinedGradientsList->currentItem()->text();
	  }
	} else {
...
void ColorScaleConfigDialog::displayUserGradientPreview() {
	...
	if (gradientCB->isChecked()) {
		QLinearGradient qLinearGradient(userGradientPreview->width() / 2, 0, userGradientPreview->width() / 2, userGradientPreview->height() - 1);
		...
		painter.fillRect(0,0,userGradientPreview->width(), userGradientPreview->height(), qLinearGradient);
	} else {
		float rectHeight =  userGradientPreview->height() / colorsVector.size();
		for (unsigned int i = 0 ; i < colorsVector.size() ; ++i) {
			painter.fillRect(0, i * rectHeight, userGradientPreview->width(), (i+1) * rectHeight, QBrush(colorsVector[i]));
		}
	}
	painter.end();
	userGradientPreview->setPixmap(pixmap.scaled(userGradientPreview->width(), userGradientPreview->height()));
}
...
void ColorScaleConfigDialog::colorTableItemDoubleClicked(QTableWidgetItem *item) {
	...
	bool ok = true;
	QRgb newColor = QColorDialog::getRgba(qRgba(itemBgColor.red(), itemBgColor.green(), itemBgColor.blue(), itemBgColor.alpha()), &ok, this);
	item->setBackgroundColor(QColor(qRed(newColor), qGreen(newColor), qBlue(newColor), qAlpha(newColor)));
	displayUserGradientPreview();
}
