...
StringListSelectionWidget::StringListSelectionWidget(QWidget *parent, const unsigned int maxOutputListSize) :
		QWidget(parent) {
	setupUi(this);
...
StringListSelectionWidget::StringListSelectionWidget(const vector<string> &inputStringList, QWidget *parent,  const unsigned int maxOutputListSize) :
		QWidget(parent) {
	setupUi(this);
	outputList->setMaxListSize(maxOutputListSize);
	qtWidgetsConnection();
...
void StringListSelectionWidget::setInputStringList(const vector<string> &inputStringList) {
	for (unsigned int i = 0 ; i < inputStringList.size() ; ++i) {
		inputList->addItemList(QString(inputStringList[i].c_str()));
	}
}

void StringListSelectionWidget::setOutputStringList(const vector<string> &outputStringList) {
	for (unsigned int i = 0 ; i < outputStringList.size() ; ++i) {
		outputList->addItemList(QString(outputStringList[i].c_str()));
	}
}

void StringListSelectionWidget::setInputStringListLabel(const string &inputStringListLabel) {
	inputListLabel->setText(QString(inputStringListLabel.c_str()));
}

void StringListSelectionWidget::setOutputStringListLabel(const string &outputStringListLabel) {
	outputListLabel->setText(QString(outputStringListLabel.c_str()));
}

void StringListSelectionWidget::setMaxOutputListSize(const unsigned int maxOutputListSize) {
	outputList->setMaxListSize(maxOutputListSize);
}
