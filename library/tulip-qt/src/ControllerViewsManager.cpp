...
namespace tlp {
  ...
  //**********************************************************************
  //**********************************************************************
  View* ControllerViewsManager::createView(const string &name,Graph *graph,DataSet dataSet,bool forceWidgetSize,const QRect &rect,bool maximized){ 
  ...
  void ControllerViewsManager::checkViewsGraphsHierarchy(){
    for(map<View *,Graph* >::iterator itView=viewGraph.begin();itView!=viewGraph.end();++itView){
      Graph *newGraph;
      for(list<int>::iterator it=viewsGraphsHierarchy[(*itView).first].begin();it!=viewsGraphsHierarchy[(*itView).first].end();++it){
  ...
  void ControllerViewsManager::createView(QAction *action) {
    createView(action->text().toStdString(),currentGraph,DataSet(),false,QRect(0,0,0,0),false);
  }
  //**********************************************************************
  ...
  bool ControllerViewsManager::changeInteractor(QAction* action,QWidget **configurationWidget) {
    ...
    ControllerViewsTools::changeInteractor(currentView,mainWindowFacade.getInteractorsToolBar(),action,configurationWidget);
