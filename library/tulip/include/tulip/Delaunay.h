...
namespace tlp {
  ...
  /*@{*/
     ...
     * Adapted from an implementation of Sjaak Priester from Amsterdam (see source for details)
     * Version 0.0.1 02/06/2005
     */
  /**
   * Compute the delaunay triangulation and return the set of edges of the triangulation
   * in the vector edges.  Edges are defined using a pair of indexes into the original set
   * of points.
   */
  void delaunayTriangulation (const std::vector<Coord> &points, 
			      std::vector<std::pair<unsigned int, unsigned int> > &edges);
  /**
   * Compute the delaunay triangulation and return the set of triangles of the triangulation
   * in the vector triangles.  Triangles are defined using a triple indexes into the original 
   * set of points.
   */
  void delaunayTriangulation (const std::vector<Coord> &points, 
