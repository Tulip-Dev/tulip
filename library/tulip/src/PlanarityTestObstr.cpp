 ...
 *
 * $Id: PlanarityTestObstr.cpp,v 1.4.2.1 2006-05-04 16:33:11 pmary Exp $
 *
...
void PlanarityTestImpl::calcInfo3Terminals(node &t1, node &t2, node &t3,
  ...
  if (min == dfsPosNum.get(t1.id) || min == dfsPosNum.get(t2.id) || min == dfsPosNum.get(t3.id)) {
    ...
    if (max >= dfsPosNum.get(parent.get(cNode.id).id))
      ...
      if (activeCNodeOf(true, k2) != cNode) {
...
void PlanarityTestImpl::obstructionEdgesCNodeCounter(Graph *sG, node cNode, node w,
  ...
  //#ifdef _DEBUG_MODE_
  // if (f == NULL_NODE) {
  //     cout << "-> Program terminated! (obstruction_edges_cnode_counter-01)";
  //     cout<<"obstruction_edges_cnode_counter:\n";
  //     cout<<"  cnode="<<dfsPosNum.get(cNode.id)<<",f="<<dfsPosNum.get(f.id);
