...
void PlanarityTestImpl::preProcessing(Graph *g) {
  ...
  /// DEBUG {
  // cout << "** Labels (dfs[u], h[u], b[u]): " << endl;
  //   for (int i = 1 ; i <= numberOfNodes ; i++) {
  //     node u = nodeWithDfsPos.get(i);
  //     cout << "(" << dfsPosNum.get(u.id) << "," << largestNeighbor.get(u.id) 
  ...
  */
  ...
  /*
    cout << "** List of children in T:\n";
    Iterator<node> *it = g->getNodes();
    while (it->hasNext()) {
    node u = it->next();
    //cerr << "  " << dfsPosNum.get(u.id) << ": " ;
    node v;
    for (list<node>::iterator i = childrenInT0[u].begin(); i != childrenInT0[u].end(); i++) {
    node v = *i;
    cout << dfsPosNum.get(v.id) << "(" << labelB.get(v.id) << "), ";
    cout << "\n";
    }
    } delete it;
  */
...
void PlanarityTestImpl::addOldCNodeRBCToNewRBC(node oldCNode,
  ...
  while (labelB.get(predNode.id) == dfsPosNum.get(n.id) && predNode != n1 && predNode != n2) {
    ...
    //cout << "tulip predItem : " << predItem->getData().id << endl;
    predNode = predItem->getData();
    //cout << "tulip predNode : " << predNode.id << endl;
    //cout << "tulip tmp : " << tmp->getData().id << endl;
