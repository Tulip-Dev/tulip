 ...
 * Perspectives are chosen by the user when first running the Tulip agent. The main perspective is called "Tulip". This perspective unveils all the
 * Tulip features and aims at being a protyping and researching platform.
 *
 * A Perspective always acts in its own process and communicate with the Tulip agent via TCP sockets.
 * Thus, it is the Perspective's responsibility to offer the possibility to display graphs, run plugins, etc. A lot of helper classes can be found
...
protected:
   ...
   * @endcode
   * @list
   * @li The --perspective argument forces Tulip to start the "Name" perspective. Even if the project states otherwise. If this argument is not
   * specified, Tulip will look in the project's meta-data to dertermine the perspective to launch.
   * @li file_path is the absolute path of the project archive to associate with the perspective. If file_path is not given, the --perspective
   * argument must be declared in order for Tulip to know what perspective to launch. In this case, _project will point to an empty (but valid!)
   * project.
   * @endlist
   */
   ...
   *
   * It is note the Perspective's reponsibility to destroy the main window when the application gets closed. Generally speaking, destorying the
   * _mainWindow pointer could lead to undefined behavior.
   */
  QMainWindow *_mainWindow;

  /**
   * If the user provided a file to the tulip_perspective but this file is not recognized as a valid TulipProject, the full path of the file will be
  ...
  /**
   * @brief Contains extra parameters that have not been parsed by the overleying system. Those are considered to be Perspective-specific and are
...
public:
   ...
   * In order to ease the development process, Perspective are available as a singleton in order for child widgets and plugins to be able to access to
   * the features of a Perspective.
   ...
   *  Perspectives are allowed to reserve graph properties. A reserved graph properties is a core property that cannot be deleted by the user and
   * cannot be renamed.
...
protected slots:
  ...
  /**
   * @brief Send a message to the Tulip agent to make him display an error message that will be shown in the system notification as well as on the
   * welcome page.
