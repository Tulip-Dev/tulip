 ...
 *
 * This widget allow user to configure a CSVToGraphDataMapping object. This
 *object is used during the CSV import process to map CSV columns to graph
...
public:
   ...
   **/
  void updateWidget(tlp::Graph *graph,
                    const CSVImportParameters &importParameters);
  /**
   * @brief Build the mapping object between the CSV columns and the graph
   *elements.
   **/
   ...
   *
   * If return true you are sure that buildMappingObject never return a nullptr
   *object.
   **/
  bool isValid() const;
