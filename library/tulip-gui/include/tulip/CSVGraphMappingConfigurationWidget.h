...
/**
  ...
  *
  * This widget allow user to configure a CSVToGraphDataMapping object. This object is used during the CSV import process to map CSV columns to graph
  *elements like nodes or edges.
  **/
...
private:
  ...
  void selectColumns(const QString &title, std::vector<unsigned int> &columnIds, QPushButton *button);
  void selectProperties(const QString &title, std::vector<std::string> &selProperties, QPushButton *button);
