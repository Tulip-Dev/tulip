...
public:
  ...
  /**
    ...
    **/
  virtual bool parse(CSVContentHandler *handler, tlp::PluginProgress *progress = nullptr) = 0;
};

/**
* @brief Parse a csv data and send each tokens to the given CSVContentHandler object.
*
* Parse a csv data and send each tokens to the given CSVContentHandler object. Get each line of the file in the given range and parse them. This
*object skip empty lines.
* Send the found tokens to the CSVContentHandler interface.
* \code
...
public:
  /**
    ...
    * @param textDelimiter If a token is sourrounded by this charater we ignore all the separators found in this token. Useful if a token contains the
    *separator.
    ...
    **/
  CSVSimpleParser(const std::string &fileName, const QString &separator = ";", const bool mergesep = false, char textDelimiter = '"',
                  char delimiterMark = '.', const std::string &fileEncoding = std::string("UTF-8"), unsigned int firstLine = 0,
...
private:
  void tokenize(const std::string &str, std::vector<std::string> &tokens, const QString &delimiters, const bool mergedelim, char _textDelimiter,
