...
/**
  ...
  It provides convenience methods allowing to manipulate the QuaZIP library on a
  higher level. QuaZIPFacade contains only static methods, as it should only be
  used as a toolbox.
  */
...
public:
  /**
    ...
    This method compress the content of a directory (without taking the top
    ...
    @warning If an archive already exists with that name, it will be
    overwritten.
  ...
  /**
    ...
    Files contained in the archive will be stored inside the specified
    directory. Making it the root path of the uncompressed files.
    @param rootPath The absolute path of the directory where files should be
    written.
    @param archivePath The absolute file path of the ZIP file to uncompress.
    @param progress A progress handler
    @warning If some files already exist in the target directory, they will be
    overwritten.
