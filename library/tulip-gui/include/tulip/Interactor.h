...
/**
  ...
  Basically, The interactor class is an overlay to the Qt's event filtering
  mechanism. It adds several features like the ability to define priorities,
  custom cursors, etc

  When an interactor is constructed, the following methods are called in this
  order:
  @li The constructor. This method should almost be a no-op. Interactors may be
  instantiated by the plugin manager system and doing unneeded operations in the
  constructor may lead to poor performances when the application loads up.
  @li isCompatible. When creating a View, the application will check for all
  interactors to see if they could be installed on it. This is done using the
  isCompatible method.
  @li construct. Initialize the interactor. Since the constructor should be a
  no-op, initialization code should be put here.
  @li setView. Defines the view this interactor is bound to. The Interactor may
  keep a reference to the view object to perform specific operations on user
  inputs.

  Methods listed above are only called once. Once the interactor is initialized,
  it may be installed/removed several times on different QObjects. It will then
  repond to user inputs via the eventFilter method
...
public:
  std::string category() const override { return INTERACTOR_CATEGORY; }
  std::string icon() const override {
    return ":/tulip/gui/icons/32/plugin_interactor.png";
  }
  /**
    @brief Checks the compatibility between the interactor and the given view
    (identified by its name). If this method returns true, it's very likely that
    the interactor will be installed on the associated view.
    */
  virtual bool isCompatible(const std::string &viewName) const = 0;

  /**
    @deprecated Use QWidget *configurationDocWidget() and/or QWidget
    *configurationOptionsWidget() instead
    @return the configuration widget used to set up the interactor.
    @warning This method MUST ALWAYS return the same pointer. Doing otherwise
    may lead to memory leaks.
    @note The configuration widget has to be instantiated from the construct
    method.
    @note It is up to the interactor developer to delete the configuration
    widget
    */
  virtual QWidget *configurationWidget() const { return nullptr; }
  virtual QLabel *configurationDocWidget() const { return nullptr; }
  virtual QWidget *configurationOptionsWidget() const { return nullptr; }

  /**
    @return the interactor's priority.
    Priority defines how interactors gets ordered when displayed in the View's
    toolbar. Interactors with the top-most priority value will be displayed at
    the beginning of the list while lowest priority will be position at the end.
    */
  virtual unsigned int priority() const = 0;

  /**
    @return a QAction associated to this interactor.
    This is used by the overleying system to associate an icon/text to the
    interactor.
    @warning The parent (QObject::parent()) object of this QAction MUST BE the
    Interactor.
    */
  ...
  /**
    ...
    @warning The returned object MUST be the same as the one passed down to the
    setView method.
    */
  ...
  /**
    ...
    When the interactor gets active on a view, the View's cursor is changed to
  ...
  /**
    ...
    This method should be used instead of the constructor to initialize the
    interactor.
    */
...
public slots:
  /**
    ...
    @warning The view() method MUST ALWAYS return the same pointer as the one
  ...
  /**
    ...
    A call to this method means that the interactor should start listening to
    the target's events and handle them. Returning true prevents further
    handling of the event. Doing otherwise means that the interactor will let
    following filters to hand over this kind of event.
    */
  ...
  /**
    @brief Informs the interactor when the undo command (Ctrl+Z) has been
    triggered
    */
...
/**
 * @ingroup Plugins
 * @brief The InteractorLister class lists compatible interactors for a given
 * tlp::View
 */
 ...
 *
 * @brief Copy an existing Tulip interactor and sets it compatible with a given
 * View.
 *
 * This macro is used when you're making your own View and want to use an
 * existing interactor with it. Interactors are declared to be compatible with a
 * list of View. This macro extends the compatibility of an existing interactor
 * by subclassing it.
 *
 * @note: This macro used the same interactor priority as the base interactor.
 * To define your own priority, see INTERACTORPLUGINVIEWEXTENSIONWITHPRIORITY
 *
 * @param CLASS_NAME The name of the interactor class to generate.
 * @param STRING_CLASS_NAME The name of the interactor plugin to generate (see
 * tlp::Plugin::name())
 * @param BASE_INTERACTOR_STRING_NAME The name of the interactor to extend
 * @param VIEW_STRING_NAME The name of the View to set the interactor compatible
 * with
...
#define INTERACTORPLUGINVIEWEXTENSION(                                         \
    CLASS_NAME, STRING_CLASS_NAME, BASE_INTERACTOR_STRING_NAME,                \
  ...
  public:                                                                      \
    ...
    tlp::Interactor *getComponent() const {                                    \
      if (!_component) {                                                       \
        _component = tlp::PluginLister::getPluginObject<Interactor>(           \
 ...
 * @brief Similar to INTERACTORPLUGINVIEWEXTENSION but allows to define the
 * generated interactor's priority.
 * @see tlp::Interactor::priority()
 * @see INTERACTORPLUGINVIEWEXTENSION
 */
#define INTERACTORPLUGINVIEWEXTENSIONWITHPRIORITY(                             \
    CLASS_NAME, STRING_CLASS_NAME, BASE_INTERACTOR_STRING_NAME,                \
    VIEW_STRING_NAME, AUTHOR, DATE, DESCRIPTION, VERSION, PRIORITY)            \
  class CLASS_NAME : public tlp::Interactor {                                  \
    mutable tlp::Interactor *_component;                                       \
                                                                               \
  public:                                                                      \
