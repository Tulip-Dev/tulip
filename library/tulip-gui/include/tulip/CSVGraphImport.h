 ...
 *
 * Contains all the parameters defined by user for a given CSV column (the name
 *of the column, its data type and if user want to import it).
 **/
...
public:
   ...
   **/
  const std::string &dataType() const { return _type; }

  bool needMultiValues() const { return _valueSeparator != 0; }

  char getMultiValueSeparator() const { return _valueSeparator; }
  ...
  void clearExceptions() { _exceptions.clear(); }
...
public:
  CSVImportParameters(
      unsigned int fromLine = 0, unsigned int toLine = UINT_MAX,
   ...
   **/
  CSVColumn::Action getColumnActionForToken(unsigned int column,
  ...
  /**
   * @brief Return true if the given row is between the first row to import and
   *the last row to import
   **/
...
/**
 * @brief Abstract class handling node or edge mapping between a CSV column and
 *a graph property.
 *
 * Be sure there is a property with the given name in the graph or an error will
 *occur. Automatically handle CSV file parsing just implements the
 *buildIndexForRow function to fill the rowToGraphId map with the right graph
 *element.
 **/
class TLP_QT_SCOPE AbstractCSVToGraphDataMapping
...
public:
  ...
  std::pair<tlp::ElementType, std::vector<unsigned int>> getElementsForRow(
      const std::vector<std::vector<std::string>> &tokens) override;

protected:
  /**
   * @brief Create a new element if no elements for the given row was found.
   * @return Return the graph element id or UINT_MAX if no new element is
   *created.
   **/
  virtual unsigned int
  buildIndexForRow(unsigned int row, const std::vector<std::string> &keys) = 0;
...
/**
 * @brief Try to map CSV file rows to nodes according to value between a CSV
 *column and a graph property.
 *
 * Be sure there is a property with the given name in the graph before using it.
 **/
class TLP_QT_SCOPE CSVToGraphNodeIdMapping
...
public:
   ...
   * @param createNode If set to true if there is no node for an id in the CSV
   *file a new node will be created for this id.
   **/
...
/**
 * @brief Try to map CSV file rows to edges according to value between a CSV
 *column and a graph property.
 *
 * Be sure there is a property with the given name in the graph before using it.
 **/
class TLP_QT_SCOPE CSVToGraphEdgeIdMapping
    : public AbstractCSVToGraphDataMapping {
public:
  /**
...
/**
 * @brief Try to map CSV file rows to edges according to edge source and
 *destination.
 *
 * For each row in the CSV file create an edge in the graph between source and
 *destination nodes. Find source node by comparing id in the source CSV column
 *and destination node by comparing id in the destination CSV column.
 **/
class TLP_QT_SCOPE CSVToGraphEdgeSrcTgtMapping : public CSVToGraphDataMapping {
public:
  /**
   * @param graph The graph where the edges will be searched.
   * @param srcColumnIndex The index of the column with the source node id in
   *the CSV file.
   * @param tgtColumnIndex The index of the column with the taret node id in the
   *CSV file.
   ...
   * @param firstRow The first row to search ids.
   * @param lastRow The last row to search ids.
   * @param createMissinElements If true create source node, destination node if
   *one of them is not found in the graph.
   **/
...
/**
 * @brief Interface to perform mapping between CSV columns and graph properties
...
public:
   ...
   * @param token The current token. May be needed to determine column data
   *type.
   *
   * The token parameter is used to guess property type if needed.
   **/
  virtual tlp::PropertyInterface *
  getPropertyInterface(unsigned int column, const std::string &token) = 0;
};

/**
 * @brief Proxy to handle all the properties operations like access, creation,
 *data type detection during the CSV parsing process.
 *
...
public:
  ...
  getPropertyInterface(unsigned int column, const std::string &token) override;
...
private:
  ...
  QWidget *parent;
  PropertyInterface *generateApproximateProperty(const std::string &name,
                                                 const std::string &type);
};

/**
 * @brief Manage all the CSV import process. Use the mapping object to find the
 *graph element in function of the row and the propertiesManager to find the
 *property corresponding to the column. The import parameters are used to filter
 *the rows and the columns to import.
 **/
