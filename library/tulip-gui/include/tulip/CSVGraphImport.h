...
public:
  ...
  virtual std::pair<tlp::ElementType, std::vector<unsigned int>> getElementsForRow(const std::vector<std::string> &tokens,
...
public:
  AbstractCSVToGraphDataMapping(tlp::Graph *graph, tlp::ElementType type, const std::vector<unsigned int> &columnIds,
                                const std::vector<std::string> &propertyNames);
  virtual ~AbstractCSVToGraphDataMapping() {
  }

  void init(unsigned int rowNumber);
  std::pair<tlp::ElementType, std::vector<unsigned int>> getElementsForRow(const std::vector<std::string> &tokens,
...
public:
  /**
    ...
    **/
  CSVToGraphNodeIdMapping(tlp::Graph *graph, const std::vector<unsigned int> &columnIds, const std::vector<std::string> &propertyNames,
...
public:
  ...
  * @param lastRow The last row to search ids.
  **/
  CSVToGraphEdgeIdMapping(tlp::Graph *graph, const std::vector<unsigned int> &columnIds, const std::vector<std::string> &propertyNames);
...
/**
  ...
  *
  * For each row in the CSV file create an edge in the graph between source and destination nodes. Find source node by comparing id in the source CSV
  *column and destination node by comparing id in the destination CSV column.
  **/
...
public:
  ...
  **/
  CSVToGraphEdgeSrcTgtMapping(tlp::Graph *graph, const std::vector<unsigned int> &srcColumnIds, const std::vector<unsigned int> &tgtColumnIds,
                              const std::vector<std::string> &srcPropNames, const std::vector<std::string> &tgtPropNames,
...
public:
  CSVImportColumnToGraphPropertyMappingProxy(tlp::Graph *graph, const CSVImportParameters &importParameters, QWidget *parent = nullptr);
...
/**
  * @brief Manage all the CSV import process. Use the mapping object to find the graph element in function of the row and the propertiesManager to
...
public:
  CSVGraphImport(CSVToGraphDataMapping *mapping, CSVImportColumnToGraphPropertyMapping *propertiesManager,
