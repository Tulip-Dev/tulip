 ...
 *    -> SIMPLE_LIST : the widget contains only one strings list, the selection of strings is done via the checkboxes located on the left of the items
 * list
 *    -> DOUBLE_LIST : the widget contains two lists, the left one contains the unselected string list and the right one the selected strings list. To
 * select
 *                     a string (resp. unselect a string), it has to be moved from the list on the left to the list on the right (resp. from the list
 * on the right to
...
public:
   ...
   */
  StringsListSelectionWidget(QWidget *parent = nullptr, const ListType listType = DOUBLE_LIST, const unsigned int maxSelectedStringsListSize = 0);
   ...
   * \param maxSelectedStringsListSize the maximum number of strings that can be selected (if 0, no size restriction)
   */
  StringsListSelectionWidget(const std::vector<std::string> &unselectedStringsList, QWidget *parent = nullptr, const ListType listType = DOUBLE_LIST,
