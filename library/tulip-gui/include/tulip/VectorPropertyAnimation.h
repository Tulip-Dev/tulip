...
public:
  VectorPropertyAnimation(tlp::Graph *graph, PropType *start, PropType *end, PropType *out, tlp::BooleanProperty *selection = nullptr,
                          int frameCount = 1, bool computeNodes = true, bool computeEdges = true, QObject *parent = nullptr);

  virtual ~VectorPropertyAnimation() {
  }

protected:
  virtual RealType getNodeFrameValue(const RealType &startValue, const RealType &endValue, int frame);
  virtual RealType getEdgeFrameValue(const RealType &startValue, const RealType &endValue, int frame);
