...
template <typename T>
void GraphTableModel::removeFromVector(const std::set<T> &objects, std::vector<T> &vect,
...
void GraphTableModel::addToVector(const std::set<T> &objects, std::vector<T> &vect,
                                  TLP_HASH_MAP<T, int> &objToIndex, bool addRows, Compare *comp) {
  // Reserve size
  ...
  if (comp == NULL) {
    ...
    for (it = vect.begin(), current = 0; it != vect.end() && !sortedObjects.empty();
         ++it, ++current) {
...
GraphTableModel::computeArea(const std::set<T> &elementsToFind, const std::vector<T> &elements,
                             const TLP_HASH_MAP<T, int> &objToIndex) const {
  // Init from and to values
  int first = elements.size() - 1, last = 0;

  for (typename std::set<T>::const_iterator it = elementsToFind.begin(); it != elementsToFind.end();
       ++it) {
