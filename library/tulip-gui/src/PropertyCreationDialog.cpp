...
PropertyCreationDialog::PropertyCreationDialog(QWidget *parent)
    : QDialog(parent), ui(new Ui::PropertyCreationDialog), _createPropertyButton(nullptr), _graph(nullptr), _createdProperty(nullptr) {
  initGui();
}

PropertyCreationDialog::PropertyCreationDialog(Graph *graph, QWidget *parent, const std::string &selectedType)
    : QDialog(parent), ui(new Ui::PropertyCreationDialog), _graph(graph), _createdProperty(nullptr) {
  initGui();
...
void PropertyCreationDialog::accept() {
  ...
  if (_graph == nullptr) {
    QMessageBox::warning(this, "Fail to create property", "The parent graph is invalid", QMessageBox::Ok, QMessageBox::Ok);
    error = true;
  }
  ...
  if (propertyName.isEmpty()) {
    QMessageBox::warning(this, "Fail to create property", "You can't create a property with an empty name", QMessageBox::Ok, QMessageBox::Ok);
    error = true;
  }

  if (_graph->existLocalProperty(QStringToTlpString(propertyName))) {
    QMessageBox::warning(this, "Fail to create property", "A property with the same name already exist", QMessageBox::Ok, QMessageBox::Ok);
    error = true;
  }
  ...
  if (!error) {
    _graph->push();
    _createdProperty =
        _graph->getLocalProperty(QStringToTlpString(propertyName), propertyTypeLabelToPropertyType(ui->propertyTypeComboBox->currentText()));
  }
