...
CSVSimpleParser::CSVSimpleParser(const string &fileName, const QString &separator, const bool mergesep, char textDelimiter, char decimalMark,
                                 const string &fileEncoding, unsigned int firstLine, unsigned int lastLine)
    : _fileName(fileName), _separator(separator), _textDelimiter(textDelimiter), _decimalMark(decimalMark), _fileEncoding(fileEncoding),
...
bool CSVSimpleParser::parse(CSVContentHandler *handler, PluginProgress *progress) {
  ...
  if (*csvFile) {
    ...
    if (codec == nullptr) {
      qWarning() << __PRETTY_FUNCTION__ << ":" << __LINE__ << " Cannot found the convertion codec to convert from " << _fileEncoding
...
void CSVSimpleParser::tokenize(const string &str, vector<string> &tokens, const QString &delimiters, const bool mergedelim, char textDelimiter,
                               unsigned int) {
