...
ItemsListWidget::ItemsListWidget(QWidget *parent, const unsigned int maxListSize) : QListWidget(parent), maxListSize(maxListSize) {
  setAcceptDrops(true);
}
...
bool ItemsListWidget::addItemList(QString item) {
  if (maxListSize == 0 || (static_cast<unsigned int>(count()) < maxListSize)) {
    addItem(item);
    return true;
  } else {
    return false;
