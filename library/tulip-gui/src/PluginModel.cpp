  ...
  if(index.row() < _list.size()) {
    ...
    if(role == Qt::DisplayRole) {
      ...
      if (TulipMaterialDesignIcons::isMaterialDesignIconSupported(p.icon())) {
        pix = FontIconManager::instance()->getMaterialDesignIcon(static_cast<md::iconCodePoint>(TulipMaterialDesignIcons::getMaterialDesignIconCodePoint(p.icon())), Qt::black).pixmap(QSize(32, 32));
      }
      return pix;
    }
  }
