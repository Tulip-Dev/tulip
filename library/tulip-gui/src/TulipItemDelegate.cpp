...
TulipItemDelegate::TulipItemDelegate(QObject *parent)
    : QStyledItemDelegate(parent), _currentMonitoredChild(nullptr), _currentMonitoredCombo(nullptr) {
  ...
  registerCreator<TulipFontAwesomeIcon>(new TulipFontGlyphIconCreator(true));
  registerCreator<TulipMaterialDesignIcon>(new TulipFontGlyphIconCreator(false));
}
...
void TulipItemDelegate::paint(QPainter *painter, const QStyleOptionViewItem &option, const QModelIndex &index) const {
  ...
  if (bgColor.isValid() && bgColor.type() == QVariant::Color)
    ...
    painter->setBrush((tv && tv->alternatingRowColors() && (index.row() % 2)) ? option.palette.alternateBase() : option.palette.base());
  }
  ...
  if (!v.isValid()) {
#ifndef NDEBUG
    qWarning() << "Value for row(" << index.model()->headerData(index.row(), Qt::Vertical).toString() << ") - column("
               << index.model()->headerData(index.column(), Qt::Horizontal).toString() << ") is invalid";
#endif
    return;
  }
...
QVariant TulipItemDelegate::showEditorDialog(tlp::ElementType elType, tlp::PropertyInterface *pi, tlp::Graph *g, TulipItemDelegate *delegate,
  ...
  if (dialogParent == nullptr && Perspective::instance())
  ...
  if (dlg == nullptr) {
    ...
    dlg->setWindowTitle(elType == NODE ? "Set nodes values" : "Set edges values");
