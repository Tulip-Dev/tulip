...
PropertyConfigurationWidget::PropertyConfigurationWidget(
    ...
    PropertyNameValidator *validator, QWidget *parent)
    : QWidget(parent),
  ...
  propertyEditButton->setSizePolicy(
      QSizePolicy(QSizePolicy::Fixed, QSizePolicy::Fixed));
  layout()->addWidget(propertyEditButton);
  connect(propertyEditButton, SIGNAL(released()), this,
          SLOT(showPropertyCreationDialog()));
...
void PropertyConfigurationWidget::setPropertyType(const string &pType) {
  ...
  propertyEditButton->setText(QString("%1\n[%2]")
                                  .arg(getPropertyName())
                                  .arg(propertyTypeToPropertyTypeLabel(_type)));
}
...
void PropertyConfigurationWidget::setPropertyName(const QString &pName) {
  ...
  propertyEditButton->setToolTip(
      QString("<center><b>Column #%1</b></center>name: %2<br/>type: "
...
void PropertyConfigurationWidget::typeCBChanged(const QString &type) {
  ...
  const std::set<std::string> &props =
      CSVImportConfigurationWidget::getPropsForTypename(
          propertyTypeLabelToPropertyType(type));
...
void PropertyConfigurationWidget::showPropertyCreationDialog() {
  ...
  connect(ui->typeCB, SIGNAL(currentIndexChanged(QString)), this,
          SLOT(typeCBChanged(QString)));
  ...
  ui->exceptionTableWidget->horizontalHeader()->setSectionResizeMode(
      QHeaderView::Stretch);
  connect(ui->addExceptionButton, SIGNAL(released()), this,
          SLOT(addException()));
  connect(ui->delCurrentExceptionButton, SIGNAL(released()), this,
          SLOT(delCurrentException()));
  ...
  if (dialog.exec() == QDialog::Accepted) {
    ...
    for (int i = 0; i < ui->exceptionTableWidget->rowCount(); ++i) {
      ...
      auto action =
          static_cast<QComboBox *>(ui->exceptionTableWidget->cellWidget(i, 1))
              ->currentIndex();
...
void CSVTableHeader::paintSection(QPainter *painter, const QRect &rect,
  ...
  painter->drawPixmap(rect.x() + (rect.width() - cbRect.width()) / 2,
                      rect.y() + (rect.height() - cbRect.height()) / 2, pix, 0,
                      0, cbRect.width(), cbRect.height());
#else
  cb.rect = QRect(rect.x() + (rect.width() - cbRect.width()) / 2,
...
bool CSVTableWidget::line(unsigned int row, const vector<string> &lineTokens) {
  ...
  if ((row < firstLineIndex) || // Wait for the first line index
                                // If the maximum line number is reached ignore
                                // the token.
...
CSVImportConfigurationWidget::CSVImportConfigurationWidget(QWidget *parent)
    : QWidget(parent), ui(new Ui::CSVImportConfigurationWidget),
      ...
      maxLineNumber(0), parser(NULL), firstLine(0),
      guessFirstLineIsHeader(true), keepPropertyWidgets(false) {
  ui->setupUi(this);
...
void CSVImportConfigurationWidget::setMaxPreviewLineNumber(
...
void CSVImportConfigurationWidget::addPropertyToPropertyList(
  ...
  PropertyConfigurationWidget *propertyConfigurationWidget =
      createPropertyConfigurationWidget(
          ...
          isEditable, propertyType, ui->previewTableWidget);
...
const vector<CSVColumn *>
CSVImportConfigurationWidget::getPropertiesToImport() const {
...
const string &CSVImportConfigurationWidget::guessPropertyDataType(
...
const string &CSVImportConfigurationWidget::combinePropertyDataType(
...
const string &
CSVImportConfigurationWidget::guessDataType(const string &data) const {
