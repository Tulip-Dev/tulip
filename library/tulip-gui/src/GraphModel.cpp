...
QVariant GraphModel::nodeValue(unsigned int id, PropertyInterface * prop) {
  ...
  if (dynamic_cast<IntegerProperty*>(prop) != NULL) {
    ...
    if (prop->getName() == "viewIcon")
      return QVariant::fromValue<TulipFontIcon>(TulipFontIcon(tlpStringToQString(static_cast<StringProperty*>(prop)->getNodeValue(n))));
...
QVariant GraphModel::nodeDefaultValue(PropertyInterface * prop) {
  if (dynamic_cast<IntegerProperty*>(prop) != NULL) {
    ...
    if (prop->getName() == "viewIcon")
      return QVariant::fromValue<TulipFontIcon>(TulipFontIcon(tlpStringToQString(static_cast<StringProperty*>(prop)->getNodeDefaultValue())));
...
#define SET_ALL_NODE_VALUE(PROP,TYPE) else if (dynamic_cast<PROP*>(prop) != NULL) setValueToGraphNodes<PROP>(prop, v.value<TYPE>(), graph)
bool GraphModel::setAllNodeValue(PropertyInterface * prop, QVariant v, Graph *graph) {
  if (dynamic_cast<IntegerProperty*>(prop) != NULL) {
    ...
    if (prop->getName() == "viewFont")
      setValueToGraphNodes<StringProperty>(prop, QStringToTlpString(v.value<TulipFont>().fontFile()), graph);
    else if (prop->getName() == "viewIcon")
      setValueToGraphNodes<StringProperty>(prop, QStringToTlpString(v.value<TulipFontIcon>().iconName), graph);
    else if (prop->getName() == "viewTexture")
      setValueToGraphNodes<StringProperty>(prop, QStringToTlpString(v.value<TextureFile>().texturePath), graph);
    else
      setValueToGraphNodes<StringProperty>(prop, QStringToTlpString(v.value<QString>()), graph);
  }
  else if (dynamic_cast<BooleanVectorProperty*>(prop) != NULL)
    setValueToGraphNodes<BooleanVectorProperty>(prop, v.value<QVector<bool> >().toStdVector(), graph);
...
bool GraphModel::setNodeValue(unsigned int id, PropertyInterface * prop, QVariant v) {
  ...
  if (dynamic_cast<IntegerProperty*>(prop) != NULL) {
    ...
    if (prop->getName() == "viewFont")
      ...
      static_cast<StringProperty*>(prop)->setNodeValue(n, QStringToTlpString(v.value<TulipFontIcon>().iconName));
    else if (prop->getName() == "viewTexture")
...
QVariant GraphModel::edgeValue(unsigned int id, PropertyInterface * prop) {
  ...
  if (dynamic_cast<IntegerProperty*>(prop) != NULL) {
    ...
    if (prop->getName() == "viewIcon")
      return QVariant::fromValue<TulipFontIcon>(TulipFontIcon(tlpStringToQString(static_cast<StringProperty*>(prop)->getEdgeValue(e))));
...
QVariant GraphModel::edgeDefaultValue(PropertyInterface * prop) {
  if (dynamic_cast<IntegerProperty*>(prop) != NULL) {
    ...
    if (prop->getName() == "viewIcon")
      return QVariant::fromValue<TulipFontIcon>(TulipFontIcon(tlpStringToQString(static_cast<StringProperty*>(prop)->getEdgeDefaultValue())));
...
bool GraphModel::setEdgeValue(unsigned int id, PropertyInterface* prop, QVariant v) {
  ...
  if (dynamic_cast<IntegerProperty*>(prop) != NULL) {
    ...
    if (prop->getName() == "viewFont")
      ...
      static_cast<StringProperty*>(prop)->setEdgeValue(e, QStringToTlpString(v.value<TulipFontIcon>().iconName));
    else if (prop->getName() == "viewTexture")
...
#define SET_ALL_EDGE_VALUE(PROP,TYPE) else if (dynamic_cast<PROP*>(prop) != NULL) setValueToGraphEdges<PROP>(prop, v.value<TYPE>(), graph)
bool GraphModel::setAllEdgeValue(PropertyInterface* prop, QVariant v, Graph *graph) {
  if (dynamic_cast<IntegerProperty*>(prop) != NULL) {
    if (prop->getName() == "viewShape")
      ...
      setValueToGraphEdges<IntegerProperty>(prop, v.value<EdgeExtremityShape::EdgeExtremityShapes>(), graph);
    ...
    if (prop->getName() == "viewFont")
      setValueToGraphEdges<StringProperty>(prop, QStringToTlpString(v.value<TulipFont>().fontFile()), graph);

    else if (prop->getName() == "viewIcon")
      setValueToGraphEdges<StringProperty>(prop, QStringToTlpString(v.value<TulipFontIcon>().iconName), graph);

    else if (prop->getName() == "viewTexture")
      setValueToGraphEdges<StringProperty>(prop, QStringToTlpString(v.value<TextureFile>().texturePath), graph);

    else
      setValueToGraphEdges<StringProperty>(prop, QStringToTlpString(v.value<QString>()), graph);
  }
  else if (dynamic_cast<BooleanVectorProperty*>(prop) != NULL)
    setValueToGraphEdges<BooleanVectorProperty>(prop, v.value<QVector<bool> >().toStdVector(), graph);
...
void NodesGraphModel::treatEvent(const Event& ev) {
  ...
  if (graphEv) {
    ...
    if (graphEv->getType() == GraphEvent::TLP_ADD_NODE) {
      ...
      std::vector<node> addedNodes = graphEv->getNodes();
      for (std::vector<tlp::node>::const_iterator it = addedNodes.begin(); it != addedNodes.end(); ++it) {
        // if the node was removed then readded before the call to Observable::unholdObservers(), remove
        // it from the elementsToModify list as no update has to be performed in the model for that element
...
void EdgesGraphModel::treatEvent(const Event& ev) {
  ...
  if (dynamic_cast<const GraphEvent*>(&ev) != NULL) {
    ...
    if (graphEv->getType() == GraphEvent::TLP_ADD_EDGE) {
      ...
      std::vector<edge> addedEdges = graphEv->getEdges();
      for (std::vector<tlp::edge>::const_iterator it = addedEdges.begin(); it != addedEdges.end(); ++it) {
        // if the edge was removed then readded before the call to Observable::unholdObservers(), remove
        // it from the elementsToModify list as no update has to be performed in the model for that element
