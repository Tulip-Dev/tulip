...
void ColorScaleConfigDialog::importColorScaleFromFile(const QString &currentDir) {
  QString imageFilePath = QFileDialog::getOpenFileName(this, tr("Open Image File"), currentDir, tr("Image Files (*.png *.jpg *.bmp)"));
...
void ColorScaleConfigDialog::displayGradientPreview(const QList<QColor> &colorsVector, bool gradient, QLabel *displayLabel) {
  ...
  if (gradient) {
    QLinearGradient qLinearGradient(displayLabel->width() / 2, 0, displayLabel->width() / 2, displayLabel->height() - 1);
    ...
    for (int i = 0; i < colorsVector.size(); ++i) {
      painter.fillRect(0, i * rectHeight, displayLabel->width(), (i + 1) * rectHeight, QBrush(colorsVector.at(i)));
    }
  }
...
void ColorScaleConfigDialog::saveCurrentColorScale() {
  ...
  bool ok;
  QString text = QInputDialog::getText(this, tr("Color scale saving"), tr("Enter a name for this color scale : "), QLineEdit::Normal, "unnamed", &ok);
...
void ColorScaleConfigDialog::deleteSavedColorScale() {
  if (_ui->savedColorScalesList->count() > 0 && _ui->savedColorScalesList->currentItem()) {
    ...
    if (QMessageBox::question(this, "Color scale deleting", "Delete saved color scale " + savedColorScaleId + " ?",
...
void ColorScaleConfigDialog::setColorScale(const ColorScale &colorScale) {
  if (colorScale.colorScaleInitialized()) {
    ...
    for (std::map<float, tlp::Color>::iterator it = colorMap.begin(); it != colorMap.end();) {
      QTableWidgetItem *item = new QTableWidgetItem();
      item->setFlags(Qt::ItemIsEnabled);
      item->setBackgroundColor(QColor(it->second.getR(), it->second.getG(), it->second.getB(), it->second.getA()));
