...
bool MouseBoxZoomer::eventFilter(QObject *widget, QEvent *e) {
  ...
  if (e->type() == QEvent::MouseButtonPress) {
    ...
    if (qMouseEv->buttons() == _mButton && (_kModifier == Qt::NoModifier || qMouseEv->modifiers() & _kModifier)) {
      if (!_dragStarted) {
  ...
  if (e->type() == QEvent::MouseButtonRelease) {
    ...
    if ((qMouseEv->button() == _mButton && (_kModifier == Qt::NoModifier || qMouseEv->modifiers() & _kModifier))) {
      if (glw->getScene()->getMainGlGraph()->getGraph() != _graph) {
        _graph = nullptr;
        _dragStarted = false;
      }
...
bool MouseBoxZoomer::draw(GlMainWidget *glw) {
  ...
  Camera *camera = glw->getScene()->getMainLayer()->getCamera();
  camera->initGl2D();
  tlp::Vec2f bl(glw->screenToViewport(std::min(_firstX, _curX)), glw->screenToViewport(std::min(glw->height() - _firstY, glw->height() - _curY)));
  tlp::Vec2f tr(glw->screenToViewport(std::max(_firstX, _curX)), glw->screenToViewport(std::max(glw->height() - _firstY, glw->height() - _curY)));
