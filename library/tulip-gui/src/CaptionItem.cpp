...
CaptionItem::CaptionItem(View *view)
    : view(view), _graph(NULL), _metricProperty(NULL), _colorProperty(NULL), _sizeProperty(NULL),
      _backupColorProperty(NULL), _backupBorderColorProperty(NULL) {
  ...
  *_colorProperty = *_backupColorProperty;
  ...
  if (_captionType == NodesColorCaption || _captionType == NodesSizeCaption) {
    ...
    for (; itE->hasNext();) {
      ...
      if (_metricProperty->getEdgeValue(eit) < beginMetric ||
          _metricProperty->getEdgeValue(eit) > endMetric) {
        tmp[3] = 25;
        borderTmp[3] = 25;
