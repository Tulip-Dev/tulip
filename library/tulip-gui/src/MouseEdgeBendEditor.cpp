...
MouseEdgeBendEditor::MouseEdgeBendEditor()
    : glMainWidget(nullptr), layer(nullptr), edgeEntity(nullptr), targetTriangle(Coord(0, 0, 0), Size(1, 1, 1)), circleString(nullptr),
...
bool MouseEdgeBendEditor::eventFilter(QObject *widget, QEvent *e) {
  ...
  if (e->type() == QEvent::MouseButtonRelease && qMouseEv->button() == Qt::LeftButton && _operation != NONE_OP) {
    ...
    if (selectedEntity == "targetTriangle") {
      ...
      if (glMainWidget->pickNodesEdges(qMouseEv->x(), qMouseEv->y(), selectedEntity) &&
        ...
        glMainWidget->getScene()->getGlGraphComposite()->getGraph()->setEnds(
            mEdge, glMainWidget->getScene()->getGlGraphComposite()->getGraph()->ends(mEdge).first, node(selectedEntity.getComplexEntityId()));
      }
      ...
      if (glMainWidget->pickNodesEdges(qMouseEv->x(), qMouseEv->y(), selectedEntity) &&
          selectedEntity.getEntityType() == SelectedEntity::NODE_SELECTED) {
        glMainWidget->getScene()->getGlGraphComposite()->getGraph()->setEnds(
            mEdge, node(selectedEntity.getComplexEntityId()), glMainWidget->getScene()->getGlGraphComposite()->getGraph()->ends(mEdge).second);
      }
    }
...
void MouseEdgeBendEditor::mMouseTranslate(int newX, int newY, GlMainWidget *glMainWidget) {
  ...
  if (selectedEntity == "targetTriangle") {
    targetTriangle.translate(
        Coord(-glMainWidget->screenToViewport(editPosition[0] - newX), glMainWidget->screenToViewport(editPosition[1] - newY), 0));
...
void MouseEdgeBendEditor::mMouseCreate(int x, int y, GlMainWidget *glMainWidget) {
  Coord viewportClick(glMainWidget->screenToViewport(glMainWidget->width() - x), glMainWidget->screenToViewport(y), 0);
  ...
  if (edgeSelected) {
    ...
    if (_shape->getNodeValue(mNode) == complexPolygonGlyphId && complexPolygonGlyphId != 0) {
      if (_coordsVectorProperty) {
        ...
        for (vector<Coord>::const_iterator it = baseCoordinates.begin(); it != baseCoordinates.end(); ++it) {
          Coord tmp(Coord((((*it)[0] - min[0]) / (max[0] - min[0])) * nodeSize[0] - nodeSize[0] / 2.,
                          (((*it)[1] - min[1]) / (max[1] - min[1])) * nodeSize[1] - nodeSize[1] / 2., 0));
          coordinatesWithRotation.push_back(_layout->getNodeValue(mNode) + Coord(tmp[0] * cos(rotation) - tmp[1] * sin(rotation),
