...
static void copyTextureFileInProject(const QString &textureFilePath, tlp::TulipProject *project, QStringList &projectTexturesFolders,
...
void GraphHierarchiesModel::writeTextureFilesInProject(tlp::TulipProject *project, tlp::PluginProgress *progress) {
  ...
  foreach (const QString &textureProjectFileToRemove, projectTexturesFiles) { project->removeFile(textureProjectFileToRemove); }
  foreach (const QString &textureProjectFolderToRemove, projectTexturesFolders) { project->removeDir(textureProjectFolderToRemove); }
}
