        ...
        if (_graphs.contains(ge->getGraph()->getRoot())) {
            if (ge->getType() == GraphEvent::TLP_BEFORE_ADD_DESCENDANTGRAPH) {
                ...
                if (hasIndex(parentIndex.row(), parentIndex.column(), parentIndex.parent())) {
                    ...
                    // its sub-graphs were moved to its parent so we need to remove
                    // the corresponding rows in the parent index as these sub-graphs
                    // will be restored to the readded sub-graph
                    if (sg->numberOfSubGraphs() > 0 && parentGraph->isSubGraph(sg->getNthSubGraph(0))) {
                        Graph *firstSg = sg->getNthSubGraph(0);
                        QModelIndex indexFirstSg = indexOf(firstSg);
                        assert(indexFirstSg.isValid());
                        beginRemoveRows(parentIndex, indexFirstSg.row(), indexFirstSg.row()+sg->numberOfSubGraphs()-1);
                        endRemoveRows();
                        beginInsertRows(parentIndex,parentGraph->numberOfSubGraphs()-sg->numberOfSubGraphs(),parentGraph->numberOfSubGraphs()-sg->numberOfSubGraphs());
                        _indexCache[sg] = createIndex(parentGraph->numberOfSubGraphs()-sg->numberOfSubGraphs(), 0, const_cast<Graph*>(sg));
                    }
                    else {
                        beginInsertRows(parentIndex,parentGraph->numberOfSubGraphs(),parentGraph->numberOfSubGraphs());
                        _indexCache[sg] = createIndex(parentGraph->numberOfSubGraphs(), 0, const_cast<Graph*>(sg));
                    }
                }
            }
                ...
                // we need to move the corresponding rows in the model
                if (sg->numberOfSubGraphs() > 0) {
                    beginInsertRows(parentIndex, parentGraph->numberOfSubGraphs()-sg->numberOfSubGraphs(), parentGraph->numberOfSubGraphs()-1);
                    endInsertRows();
                }
