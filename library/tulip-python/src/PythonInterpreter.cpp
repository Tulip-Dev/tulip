...
PythonInterpreter::PythonInterpreter() : _runningScript(false), _defaultConsoleWidget(NULL) {
...
void PythonInterpreter::loadTulipPythonPlugins(const QString &pluginsPath) {
...
QVector<QString> PythonInterpreter::getObjectDictEntries(const QString &objectName, const QString &prefixFilter) {
    ...
    if (runString(objectName)) {
        runString(QString("printObjectClass(")+objectName+")");
        QStringList objectDictList = consoleOuputString.split("\n");
