...
static QSet<QString> getParametersListForPlugin(const QString &pluginName,
  ...
  if (PluginLister::pluginExists(QStringToTlpString(pluginName))) {
    ...
    for (const ParameterDescription &pd :
         PluginLister::getPluginParameters(QStringToTlpString(pluginName)).getParameters()) {
...
void AutoCompletionDataBase::analyseCurrentScriptCode(const QString &code, const int currentLine,
  ...
  while (!in.atEnd()) {
    ...
    auto expPos = line.indexOf(methodCallRegexp);
    if (expPos != -1) {
      QString expr = line.mid(expPos, line.indexOf('(', expPos) - expPos);
    ...
    expPos = line.indexOf(graphPropRegexp, 0, &match);
    if (_graph && expPos != -1) {
      QString expr = line.mid(expPos, match.capturedLength());
    ...
    expPos = line.indexOf(graphPropAccessRegexp, 0, &match);
    if (_graph && expPos != -1) {
      QString expr = line.mid(expPos, match.capturedLength());
    ...
    expPos = line.indexOf(graphPropAccessRegexp2, 0, &match);
    if (expPos != -1) {
      QString expr = line.mid(expPos, match.capturedLength());
      int pos = expr.indexOf("[");
      QString varName = expr.mid(0, pos);
