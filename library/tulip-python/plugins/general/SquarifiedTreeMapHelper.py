...
# Simple Python plugin to ease the configuration of a Treemap visualization in the main Tulip GUI.
# As the treemap layout is different from classical node link diagram representation, some
# visual properties setup has to be done in order to get an aesthetic visualization of it in the main Tulip GUI.
# Notably, edge colors must be set to full transparency to avoid rendering artifacts, node shapes
# must be set to square, node border colors must be adjusted (everything but black) and
# node border widths must be visible (different from 0).
...
class SquarifiedTreeMapHelper(tlp.Algorithm):
  def __init__(self, context):
    ...
    self.addNumericPropertyParameter('metric', 'An optionnal metricused to estimate the size allocated to each node', '', False)
    self.addFloatParameter('aspect ratio', 'The aspect ratio (height/width) for the rectangle corresponding to the root node', '1')
    self.addBooleanParameter('treemap type', 'If true, use original Treemaps (B. Shneiderman) otherwise use Squarified Treemaps (J. J. van Wijk)', 'false')
    self.addColorParameter('border color', 'The border color that will be applied to all treemap nodes', '(255, 255, 255, 255)')
    self.addLayoutPropertyParameter('layout', 'The output treemap layout', 'viewLayout', True, False, True)
    self.addSizePropertyParameter('sizes', 'The output treemap sizes', 'viewSize', True, False, True)
    self.addIntegerPropertyParameter('shapes', 'The output treemap shapes', 'viewShape', True, False, True)
    self.addColorPropertyParameter('colors', 'The output treemap colors', 'viewColor', True, False, True)
    self.addColorPropertyParameter('border colors', 'The output treemap border colors', 'viewBorderColor', True, False, True)
    self.addDoublePropertyParameter('border widths', 'The output treemap border widths', 'viewBorderWidth', True, False, True)
