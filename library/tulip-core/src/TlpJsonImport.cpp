...
public:
  TlpJsonGraphParser(tlp::Graph *parentGraph, tlp::PluginProgress *progress)
      : YajlParseFacade(progress), _parsingEdges(false), _parsingNodes(false), _newEdge(false), _edgeSource(UINT_MAX), _parsingNodesIds(false),
        _parsingEdgesIds(false), _parsingEdgesNumber(false), _parsingInterval(false), _newInterval(false), _intervalSource(UINT_MAX),
        _graph(parentGraph), _dataSet(&const_cast<DataSet &>(parentGraph->getAttributes())), _parsingAttributes(false),
        _currentAttributeName(std::string()), _currentAttributeTypeName(std::string()), _parsingProperties(false), _currentProperty(nullptr),
        _propertyName(std::string()), _currentIdentifier(UINT_MAX), _parsingPropertyType(false), _parsingPropertyNodeValues(false),
        _parsingPropertyEdgeValues(false), _parsingPropertyDefaultEdgeValue(false), _parsingPropertyDefaultNodeValue(false),
  ...
  void setGraphPropertiesValues() {
    for (std::map<tlp::Graph *, TemporaryGraphProperty>::const_iterator graphIterator = _graphProperties.begin();
      ...
      for (TemporaryGraphProperty::const_iterator propertyIterator = graphIterator->second.begin(); propertyIterator != graphIterator->second.end();
           ++propertyIterator) {
        tlp::GraphProperty *prop = graphIterator->first->getProperty<tlp::GraphProperty>(propertyIterator->first);

        for (TemporaryGraphValue::const_iterator valueIterator = propertyIterator->second.begin(); valueIterator != propertyIterator->second.end();
             ++valueIterator) {
  ...
  virtual void parseMapKey(const std::string &value) {
    if (_parsingProperties && !_parsingPropertyNodeValues && !_parsingPropertyEdgeValues && !_parsingPropertyDefaultEdgeValue &&
  ...
  virtual void parseString(const std::string &value) {
    if (_parsingProperties) {
      if (_parsingPropertyType && !_propertyName.empty()) {
        ...
        _parsingPathViewProperty = (_propertyName == std::string("viewFont") || _propertyName == std::string("viewTexture"));
...
public:
  PLUGININFORMATION("JSON Import", "Charles Huet", "18/05/2011", "Imports a graph recorded in a file using the Tulip JSON format", "1.0", "File")
