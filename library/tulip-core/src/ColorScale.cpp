...
ColorScale::ColorScale(const std::map<float, Color> &colorMap, const bool gradient) : colorMap(colorMap), gradient(gradient) {
}
  ...
  if (colorMap.size() == 0) {
    ...
    if (gradient) {
      ...
      for (unsigned int i = 0; i < 4; ++i) {
        ret[i] = static_cast<unsigned char>((double(startColor[i]) + (double(endColor[i]) - double(startColor[i])) * ratio));
      }
