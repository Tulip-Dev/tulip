...
void PlanarityTestImpl::obstrEdgesTerminal(Graph *G, node w, node t, node u) {
  ...
  edge e =
      G->existEdge(nodeLabelB.get(t.id), nodeWithDfsPos.get(labelB.get(t.id)));
  assert(e.isValid());
  obstructionEdges.push_back(e);
...
//=================================================================
/*
 * Adds part of the boundary cycle of "cnode" that contains w1 and doesn't
 * contain path between w2 and w3 to "el".
 */
void PlanarityTestImpl::addPartOfBc(Graph *sG, node cNode, node n1, node n2,
                                    node n3) {
...
void PlanarityTestImpl::obstrEdgesPNode(Graph *sG, node p, node u) {
  ...
  edge e =
      sG->existEdge(nodeLabelB.get(p.id), nodeWithDfsPos.get(labelB.get(p.id)));
  assert(e.isValid());
  obstructionEdges.push_back(e);
}
//=================================================================
void PlanarityTestImpl::calcInfo3Terminals(node &t1, node &t2, node &t3,
  ...
  if (min == dfsPosNum.get(t1.id) || min == dfsPosNum.get(t2.id) ||
      min == dfsPosNum.get(t3.id)) {
 ...
 */
void PlanarityTestImpl::obstructionEdgesT0(Graph *sG, node w, node t1, node t2,
                                           node t3, node v) {
  if (t3 == NULL_NODE)
    t3 = v;
...
void PlanarityTestImpl::obstructionEdgesCountMin1(Graph *sG, node n, node cNode,
  ...
  edge e = sG->existEdge(nodeLabelB.get(t1.id),
                         nodeWithDfsPos.get(labelB.get(t1.id)));
  assert(e.isValid());
  obstructionEdges.push_back(e);
...
//  * - cnode is part of a K_{3,3} obstruction.
//  */
void PlanarityTestImpl::obstructionEdgesCountMin23(Graph *sG, node n,
                                                   node cNode, node t1, node t2,
...
//   edge e = sG->existEdge(nodeLabelB.get(t.id),
//   nodeWithDfsPos.get(labelB.get(t.id))); assert(e.isValid());
//   obstructionEdges.push_back(e);
...
// */
void PlanarityTestImpl::obstructionEdgesK5(Graph *sG, node w, node cNode,
                                           node t1, node t2, node t3) {
  if (t3 == NULL_NODE)
    t3 = parent.get(cNode.id);
...
void PlanarityTestImpl::obstructionEdgesPossibleObstrConfirmed(Graph *sG,
  ...
  edge e =
      sG->existEdge(nodeLabelB.get(v.id), nodeWithDfsPos.get(labelB.get(v.id)));
  assert(e.isValid());
 ...
 * K33
 * Pre-conditions:
 * - for i = 1, 2, if t_i is not nil, then it is a terminal node that is
 * descendant of cnode.
 */
void PlanarityTestImpl::obstructionEdgesCNodeCounter(Graph *sG, node cNode,
                                                     node w, node jl, node jr,
                                                     node t1, node t2) {
  ...
  while (it.hasNext()) {
    ...
    if (v == jl || v == jr)
      flag = true;
    else if (flag && (v == t12 || v == t22 ||
                      labelB.get(v.id) == dfsPosNum.get(w.id))) {
      f = v;
      break;
    }
  }
  ...
  //#ifdef _DEBUG_MODE_
  // if (f == NULL_NODE) {
  //     tlp::debug() << "-> Program terminated!
  //     (obstruction_edges_cnode_counter-01)";
  //     tlp::debug()<<"obstruction_edges_cnode_counter:\n";
  //     tlp::debug()<<"
  //     cnode="<<dfsPosNum.get(cNode.id)<<",f="<<dfsPosNum.get(f.id);
  //     tlp::debug()<<"\n
  ...
  assert(e.isValid());
  obstructionEdges.push_back(e);
  e = sG->existEdge(nodeLabelB.get(jl.id),
                    nodeWithDfsPos.get(labelB.get(jl.id)));
  assert(e.isValid());
  obstructionEdges.push_back(e);
  e = sG->existEdge(nodeLabelB.get(jr.id),
                    nodeWithDfsPos.get(labelB.get(jr.id)));
  assert(e.isValid());
  obstructionEdges.push_back(e);
  extractBoundaryCycle(sG, cNode, obstructionEdges);
}
