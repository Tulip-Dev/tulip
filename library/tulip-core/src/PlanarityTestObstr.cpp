...
//=================================================================
void PlanarityTestImpl::calcInfo3Terminals(node &t1, node &t2, node &t3, int &countMin, int &countF, node &cNode, node &q) {
  countMin = countF = 0;
...
//  */
void PlanarityTestImpl::obstructionEdgesCountMin1(Graph *sG, node n, node cNode, node t1, node t2, node t3) {
  if (t3 == nullptr_NODE)
...
//  * - cnode is part of a K_{3,3} obstruction.
//  */
void PlanarityTestImpl::obstructionEdgesCountMin23(Graph *sG, node n, node cNode, node t1, node t2, node t3, node q, node v) {
...
// */
void PlanarityTestImpl::obstructionEdgesK5(Graph *sG, node w, node cNode, node t1, node t2, node t3) {
  if (t3 == nullptr_NODE)
    t3 = parent.get(cNode.id);
 ...
 * of cnode.
 */
void PlanarityTestImpl::obstructionEdgesCNodeCounter(Graph *sG, node cNode, node w, node jl, node jr, node t1, node t2) {
  ...
  bool flag = false;
  node f = nullptr_NODE, t12 = nullptr_NODE, t22 = nullptr_NODE;

  if (t1 != nullptr_NODE && t1 != cNode)
    t12 = lastPNode(t1, cNode);

  if (t2 != nullptr_NODE && t2 != cNode)
