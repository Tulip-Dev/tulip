...
bool TreeTest::isFreeTree(const tlp::Graph *graph) {
  ...
  return (firstNode.isValid() && instance->isFreeTree(graph, firstNode)) ? ConnectedTest::isConnected(graph) : false;
} // isFreeTree
//====================================================================
...
struct dfsFreeTreeStruct {
  ...
  dfsFreeTreeStruct(node root = node(), node from = node(), Iterator<node> *it = nullptr) : curRoot(root), cameFrom(from), neighbours(it) {
  }
  ~dfsFreeTreeStruct() {
    if (neighbours)
      delete neighbours;
...
struct dfsMakeRootedTreeStruct {
  ...
  dfsMakeRootedTreeStruct(node root, node from, Iterator<edge> *it) : curRoot(root), cameFrom(from), ioEdges(it) {
  }
};
...
static Graph *computeTreeInternal(Graph *graph, Graph *rGraph, bool isConnected, PluginProgress *pluginProgress,
...
#define REVERSED_EDGES "ReversedEdges"
  ...
  if (isConnected || ConnectedTest::isConnected(gClone)) {
    ...
    return computeTreeInternal(gClone->addSubGraph(&treeSelection), rGraph, true, pluginProgress, reversedEdges);
  }
