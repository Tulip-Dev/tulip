...
public:
  void computeMetaValue(AbstractSizeProperty *prop, node mN, Graph *sg, Graph *) {
    ...
    if (prop->getName() == "viewSize") {
      ...
      BoundingBox box = tlp::computeBoundingBox(sg, sg->getProperty<LayoutProperty>("viewLayout"), sg->getProperty<SizeProperty>("viewSize"),
      ...
      prop->setNodeValue(mN, (((SizeProperty *)prop)->getMax(sg) + ((SizeProperty *)prop)->getMin(sg)) / 2.0f);
  }
};
...
//==============================
SizeProperty::SizeProperty(Graph *sg, const std::string &n) : AbstractProperty<SizeType, SizeType>(sg, n) {
...
PropertyInterface *SizeVectorProperty::clonePrototype(Graph *g, const std::string &n) const {
  ...
  // allow to get an unregistered property (empty name)
  SizeVectorProperty *p = n.empty() ? new SizeVectorProperty(g) : g->getLocalProperty<SizeVectorProperty>(n);
  p->setAllNodeValue(getNodeDefaultValue());
  p->setAllEdgeValue(getEdgeDefaultValue());
  return p;
}
