...
public:
  void computeMetaValue(AbstractLayoutProperty *layout, node mN, Graph *sg, Graph *) {
    ...
    default:
      ...
      layout->setNodeValue(mN, (((LayoutProperty *)layout)->getMax(sg) + ((LayoutProperty *)layout)->getMin(sg)) / 2.0f);
    }
  }
};
...
LayoutProperty::LayoutProperty(Graph *sg, const std::string &n)
    : LayoutMinMaxProperty(sg, n, Coord(FLT_MAX, FLT_MAX, FLT_MAX), Coord(-FLT_MAX, -FLT_MAX, -FLT_MAX), tlp::LineType::RealType(),
...
PropertyInterface *CoordVectorProperty::clonePrototype(Graph *g, const std::string &n) const {
  ...
  // allow to get an unregistered property (empty name)
  CoordVectorProperty *p = n.empty() ? new CoordVectorProperty(g) : g->getLocalProperty<CoordVectorProperty>(n);
  p->setAllNodeValue(getNodeDefaultValue());
  p->setAllEdgeValue(getEdgeDefaultValue());
  return p;
}
