 ...
 */
void Ordering::updateNewSelectableNodes(node node_f, node no_tmp2, edge, node node_last, vector<Face> v_faces, bool one_face, bool was_visited,
                                        bool selection_face) {
...
//==========================================================
Ordering::Ordering(PlanarConMap *G, PluginProgress *pluginProgress, int minProgress, int deltaProgress, int maxProgress)
    : Gp(computePlanarConMap(G->addCloneSubGraph())), oute(), outv(), visitedNodes(), visitedFaces(), seqP(), isOuterFace(), contour(),
      is_selectable(), is_selectable_visited(), is_selectable_face(), is_selectable_visited_face(), left(), right() {
  ...
  while ((Gp->nbFaces() > 2) && (nbTours > 1)) {
    ...
    // give some feedback
    if (pluginProgress && (pluginProgress->progress(minProgress + (deltaProgress * (nbMax - nbTours)) / nbMax, maxProgress) != TLP_CONTINUE))
      return;
