...
public:
  PLUGININFORMATION("TLP Export","Auber David","31/07/2001",
                    "<p>Supported extensions: tlp, tlpz (compressed), tlp.gz (compressed)</p><p>Exports a graph in a file using the TLP format (Tulip Software Graph Format).<br/>See <b>http://tulip.labri.fr->Framework->TLP File Format</b> for more details.</p>",
                    "1.1", "File")
  ...
  void saveLocalProperties(ostream &os, Graph *g) {
    ...
    while (itP->hasNext()) {
      ...
      if (g->getSuperGraph()==g) {
        ...
        os << "(property " << " " << g->getId() << " " << prop->getTypename() << " " ;
      }
