...
public:
  ...
  string fileExtension() const override { return "tlp"; }
  ...
  TLPExport(tlp::PluginContext *context) : ExportModule(context), progress(0) {
    ...
    addInParameter<string>("text::comments", paramHelp[2],
  ...
  //====================================================
  inline node getNode(node n) { return node(graph->nodePos(n)); }
  //====================================================
  inline edge getEdge(edge e) { return edge(graph->edgePos(e)); }
  //=====================================================
  void saveGraphElements(ostream &os, Graph *g) {
    ...
    if (g->getSuperGraph() != g) {
      ...
      for (unsigned i = 0; i < nbElts; ++i) {
        ...
        os << "(edge " << i << " " << getNode(ends.first).id << " "
           << getNode(ends.second).id << ")";
  ...
  void saveLocalProperties(ostream &os, Graph *g) {
    ...
    for (PropertyInterface *prop : itP) {
      ...
      bool isPathViewProp = (prop->getName() == string("viewFont") ||
      ...
      os << "(default \"" << convert(nDefault) << "\" \"" << convert(eDefault)
         << "\")" << endl;
      ...
      while (itN->hasNext()) {
        ...
        os << "(node " << getNode(itn).id << " \"" << convert(tmp) << "\")"
           << endl;
      }
      delete itN;
      ...
      if (prop->getTypename() == GraphProperty::propertyTypename) {
        ...
        while (itE->hasNext()) {
          ...
          os << "(edge " << getEdge(ite).id << " \"" << convert(tmp) << "\")"
             << endl;
        }
      }
      delete itE;
    ...
    if (!attributes.empty()) {
      ...
      for (const pair<string, DataType *> &attribute : attributes.getValues()) {
        if (attribute.second->getTypeName() == string(typeid(node).name())) {
          ...
          vector<node> *vn =
              static_cast<vector<node> *>(attribute.second->value);
          ...
          vector<edge> *ve =
              static_cast<vector<edge> *>(attribute.second->value);
