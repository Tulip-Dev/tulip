 ...
 * - T_w is biconnected.
*/
void PlanarityTestImpl::calculatePartialEmbedding(Graph *sG, node w, node newCNode, list<edge> &listBackEdges, list<node> &terminalNodes) {
  ...
  case 1: {
    ...
    bEdgesRepres = groupBackEdgesByRepr(sG, listBackEdges, backEdgeRepresentant, traversedNodes, listRepresentants);
    ...
    toEmbedLater = embedUpwardT(true, term, w, sG, w, bEdgesRepres, traversedNodes, embedList[newCNode]);
  ...
  case 2: {
    ...
    bEdgesRepres = groupBackEdgesByRepr(sG, listBackEdges, backEdgeRepresentant, traversedNodes, listRepresentants);
    list<node> toEmbedLater;
    toEmbedLater = embedUpwardT(true, term2, w, sG, w, bEdgesRepres, traversedNodes, embedList[newCNode]);
  ...
  for (list<node>::iterator it = listRepresentants.begin(); it != listRepresentants.end(); ++it)
...
*/
void PlanarityTestImpl::markPathInT(node t, node w, map<node, node> &backEdgeRepresentant, list<node> &traversedNodes) {
 ...
 * - for all nodes u in T_w, has_back_edge[u] == false.
*/
map<node, list<edge>> PlanarityTestImpl::groupBackEdgesByRepr(Graph *sG, list<edge> &listBackEdges, map<node, node> &backEdgeRepresentant,
 ...
 *   b_edges_repres[u].
 */
list<node> PlanarityTestImpl::embedUpwardT(bool embBackEdgesOutW, node t1, node t2, Graph *sG, node w, map<node, list<edge>> &bEdgesRepres,
                                           list<node> &traversedNodes, tlp::BmdList<edge> &embList) {
  list<node> toEmbedLater;
  ...
  while (predU != t2) {
    if (isCNode(u)) {
      ...
      addOldCNodeToEmbedding(embBackEdgesOutW, sG, w, oldCNode, f, bEdgesRepres, traversedNodes, toEmbedLater, embList);
 ...
 */
void PlanarityTestImpl::addOldCNodeToEmbedding(bool embBackEdgesOutW, Graph *sG, node w, node oldCNode, node u, map<node, list<edge>> &bEdgesRepres,
                                               list<node> &traversedNodes, list<node> &toEmbedLater, tlp::BmdList<edge> &embList) {
  ...
  bool flipped = ((!listNodesL.empty() && (jl == u || u == nullptr_NODE)) || (jr != u && u != nullptr_NODE));
  ...
  // forall(t, listNodesR)
  for (list<node>::iterator it = listNodesR.begin(); it != listNodesR.end(); ++it) {
    t = *it;
 ...
 * nodes, for one of the two terminals (see calculate_partial_embedding).
 */
void PlanarityTestImpl::embedBackEdges(bool embBackEdgesOutW, Graph *sG, node repr, list<node> &traversedNodes, list<edge> &listBackEdges,
...
*/
int PlanarityTestImpl::sortBackEdgesByDfs(Graph *sG, node, node repr, list<edge> &listBackEdges, vector<edge> &backEdge) {
