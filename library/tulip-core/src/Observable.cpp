...
//======================================================================
template <unsigned int linkType> struct LinkFilter {
  bool operator()(tlp::node n) {
    edge link(oGraph.existEdge(n, oNode));
    return (link.isValid() && (oType[link] & linkType));
  }

  LinkFilter(const VectorGraph &graph, const EdgeProperty<unsigned char> &type, node n) : oGraph(graph), oType(type), oNode(n) {
  }
...
#pragma omp critical(ObservableGraphUpdate)
#endif
  {
    ...
    if (!_oAlive[_n]) {
      tlp::error() << "[ERROR]: in " << __PRETTY_FUNCTION__ << " at " << __FILE__ << ':' << __LINE__ << endl;
    ...
    if ((noDelay == false) && (_oEventsToTreat[_n] == 0)) {
      ...
      noDelay = true;
      for (edge e : _oGraph.getInEdges(_n)) {
...
#pragma omp critical(ObservableGraphUpdate)
#endif
  {
    ...
    if (!link.isValid()) {
      ...
      link = _oGraph.addEdge(((Observable &)obs).getBoundNode(), ((Observable *)this)->getBoundNode());
      _oType[link] = type;
    } else {
#ifndef NDEBUG
...
void Observable::observableDeleted() {
  ...
  if (hasOnlookers()) {
    Event msg(*this, Event::TLP_INVALID); // create a modify event to prevent raise exception, (hack) to forbid creation of Delete exception without
                                          // calling that fucnton
...
void Observable::sendEvent(const Event &message) {
  ...
  node backn = _n; /** to keep trace of the node if the observable is deleted during the notification, in that crazy case, (*this) is dead thus n is
                      not accessible*/
  ++_oNotifying;
    ...
    for (itobs = listenerTonotify.begin(); itobs != listenerTonotify.end(); ++itobs) {
      ...
      if (!_oAlive[backn]) {
        throw ObservableException(
            "An observable has been deleted during the notifification of its observer (ie. an observer has deleted its caller during an update)");
      }
    }
  }
