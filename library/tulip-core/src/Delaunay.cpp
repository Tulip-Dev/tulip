...
static bool runQHull(int dim, vector<double> &points, vector<pair<unsigned int, unsigned int>> &edges, vector<vector<unsigned int>> &simplices) {
...
// |    |
// A -- D
static vector<Coord> buildGrid(const Coord &A, const Coord &B, const Coord &C, const Coord &D, const float nbSubDiv = 10) {
  vector<Coord> ret;
...
static tlp::Coord computeTetrahedronCircumscribedCenter(const tlp::Coord &A, const tlp::Coord &B, const tlp::Coord &C, const tlp::Coord &D) {
...
struct Face {
  ...
  bool operator==(const Face &f) const {
    return sortedIndexes[0] == f.sortedIndexes[0] && sortedIndexes[1] == f.sortedIndexes[1] && sortedIndexes[2] == f.sortedIndexes[2];
  }
...
bool tlp::delaunayTriangulation(vector<Coord> &points, vector<pair<unsigned int, unsigned int>> &edges, vector<vector<unsigned int>> &simplices,
                                bool voronoiMode) {
