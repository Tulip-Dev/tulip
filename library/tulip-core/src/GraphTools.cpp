      ...
      if (!nodeFlag[i]) {
        ...
        if (!degZ) {
          ...
          if (graph->indeg(n) < graph->indeg(goodNode))
            ...
            if ((graph->indeg(n) == graph->indeg(goodNode)) && (graph->outdeg(n) > graph->outdeg(goodNode)))
              goodNodePos = i;
          }
        }
      }
...
#pragma omp parallel for
  ...
  while (numClasses > 1) {
    ...
    for (; iE < nbEdges; ++iE) {
      ...
      if ((srcClass = classes.getNodeValue(curEnds.first)) != (tgtClass = classes.getNodeValue(curEnds.second)))
        break;
    }
...
static void bfs(const Graph *graph, node root, std::vector<tlp::node> &nodes, MutableContainer<bool> &visited) {
