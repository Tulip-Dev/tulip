...
public:
  ...
  virtual void computeMetaValue(AbstractColorProperty *color, edge mE, Iterator<edge> *itE, Graph *) {
...
// Return 0 if the colors are equal otherwise return -1 if the first object is lower than the second and 1 if the first object is greater than the
// second.
...
PropertyInterface *ColorVectorProperty::clonePrototype(Graph *g, const std::string &n) const {
  ...
  // allow to get an unregistered property (empty name)
  ColorVectorProperty *p = n.empty() ? new ColorVectorProperty(g) : g->getLocalProperty<ColorVectorProperty>(n);
  p->setAllNodeValue(getNodeDefaultValue());
  p->setAllEdgeValue(getEdgeDefaultValue());
  return p;
}
