 ...
 * This is used to return major version number, as version numbers are formatted
 * as X.Y.Z, X being the major, Y the minor, and Z the patch version.
 *
 ...
 * If the string contains only one dot (X.Y), then everything after the first
 * dot is returned (Y). If the string is a full version with two dots (X.Y.Z),
 * everything between the first and last dots is returned (Y). If there are more
 * than two dots, everything between the first and last dots is returned.
 */
 ...
 *
 * This class holds meta-information about a plug-in (group/author/version...).
 * It stands as a unique base-class for every plugin type. This interface is not
 * intended to be directly sublassed. Plugin objects are mainly used internally
 * into the plugin lister system.
 *
 * This class also holds extra information about the Tulip system such as the
 * library version the plugin was built against. Plugin creation is handled by
 * factories generated by the PLUGIN macro and the default Plugin constructor
 * should never be called as is.
 *
 * @see tlp::FactoryInterface for more advanced operation such as plugin
 * creation and retrieving dependencies.
 ...
 * @see tlp::TemplateAlgorithm and its subclasses for plugins operating on graph
 * properties
...
public:
  ...
  /**
   * @brief Returns the name of the plug-in, as registered in the Tulip plug-in
   * system. This name must be unique, and if multiple plug-ins have the same
   * name, only the latest encountered will be considered.
   ...
   * This information can contains anything, and the developer is completely
   * free to put anything here. Most plug-ins by the Tulip team use an html
   * format to generate help from these information.
   ...
   * @TODO this member should be removed once there is a system in Tulip to
   *handle glyphs.
   *
  ...
  /**
   * @return Return the a string indicating the programming language used to
   * write the plugin (C++, Python).
   */
 ...
 * This is an helper macro that defines every function related to a plugin
 * meta-information (Plugin name, author, etc). When creating a new plugin, this
 * macro avoids having to define pure-virtual methods located into the Plugin
 * interface and put them on the same line.
 * @note PLUGINIFORMATION should be declared into the Plugin's class body into
 * the public scope
 *
 ...
 * @param GROUP The plugin's group (tlp::Plugin::group()). If the plugin does
 * not belong to any group, set GROUP to "".
 *
 * @see tlp::Plugin
 * @see PLUGIN
 */
#define PLUGININFORMATION(NAME, AUTHOR, DATE, INFO, RELEASE, GROUP)            \
  ...
  std::string date() const override { return DATE; }                           \
  std::string info() const override { return INFO; }                           \
  std::string release() const override { return RELEASE; }                     \
  std::string tulipRelease() const override { return TULIP_VERSION; }          \
 ...
 *
 * A plugin factory handles the creation process of a tlp::Plugin subclass. This
 *class should never be used directly. See the PLUGIN macro for additional
 *information.
 * @see PLUGIN
 **/
 ...
 * This macro is mandatory in order to register a plugin into Tulip. This will
generate a Factory that will handle the plugin's creation.
...
class MyPlugin: public tlp::PluginBase { // tlp::PluginBase is replaced by the
actual Plugin interface (tlp::Algorithm, tlp::View, etc) public:
  PLUGININFORMATION("My plugin", "Me", "28/09/2012", "My first plugin example",
"1.0", "")
...
#define PLUGIN(C)                                                              \
  ...
  public:                                                                      \
    C##Factory() { registerFactory(this); }                                    \
    ~C##Factory() {}                                                           \
