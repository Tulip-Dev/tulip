...
/**
 * This class represents the list of strings (identifying the graph properties)
 * that can be used as plugin parameter to select some of the properties of a
 * given graph.
 */
...
public:
   ...
   * @param selectViewProperties a flag indicating if the view properties have
   * to be selected. Default is false
   */
   ...
   **/
  const std::vector<std::string> &getProperties() const { return _props; }

  /**
   * Returns the currently selected string entries.
   */
  const std::vector<std::string> &getSelected() const { return _selected; }
   ...
   */
  void push_back(const std::string &element) { _props.push_back(element); }
   ...
   */
  inline bool emptySelected() const { return _selected.empty(); }
