 ...
 * The color scale can be either a gradient or defined by colors associated to
 * consecutive intervals. If the color scale is a gradient, returned colors are
 * interpolated in function of a position between 0.0 and 1.0. If the color
 * scale is not a gradient returned colors are computed according to the
 * interval the position belongs to.
 * @code
 * // Creates the color scale.
 * tlp::ColorScale colorScale;
 * // Clears the color scale as the default constructor initializes a non empty
 * one colorScale.clear();
...
public:
   ...
   * @param colors a vector of colors defining the color scale (first color is
   * at position 0.0, last color at position 1.0)
  ...
  /**
   * Initializes a color scale with a map of stop points and colors passed as
   * parameter.
   * @since Tulip 4.10
   * @param colorMap a map of stop points and colors defining the color scale
   * (The keys of the map must be between 0.0 and 1.0, other ones will be
   * ignored.)
   * @param gradient specifies if the color scale should be a gradient or not
   *
   */
  ColorScale(const std::map<float, Color> &colorMap,
   ...
   *
   * @warning If the scale was already configured the previous configuration is
   * lost.
   *
   ...
   *
   * This method computes the color associated to a specific position in the
   * color scale and returns it.
   * @param pos This value defines the position of the color in the scale and
   * must be between 0.0 and 1.0 (it will be clamped otherwise)
   ...
   *
   */
  bool colorScaleInitialized() const { return !colorMap.empty(); }

  /**
   * @brief Returns a map corresponding to the color scale.
   * The index of the map is the position for the corresponding color in the
   * color scale. The index is comprised between 0 and 1.
   *
   */
  const std::map<float, Color> &getColorMap() const { return colorMap; }

  /**
   * @brief Sets the map of stop points and colors used to perform color
   * mapping.
   *
   * @warning The keys of the map must be between 0.0 and 1.0, other values will
   * be ignored.
   *
   */
   ...
   */
  void setGradient(const bool g) { gradient = g; }
  ...
  /**
   * @brief Tests color scale equality with a regular one defined by a vector of
   * colors.
   *
   */
  ...
  /**
   * @brief Tests if the color scale has regular stop points, meaning the
   * distance between each consecutive stop is constant.
   *
   * @since Tulip 4.10
   *
   */
