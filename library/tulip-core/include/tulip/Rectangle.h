 ...
 */
template <typename Obj, typename OTYPE = double> struct Rectangle : public Array<Vector<Obj, 2, OTYPE>, 2> {
  /**
  ...
  */
  ...
  /**
       * @return the true if there is an intersection else false, the intersection parameter is used to stored the Rectangle pf intersection (if it
   * exists).
       * \warning the rectangle must be valid (tested in debug mode)
       */
