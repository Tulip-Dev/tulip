 ...
 * It is mostly used to determine whether or not two object are in a state of
 collision.
 *
 * It is defined by two 3d points, the first one (A) being the lowest point, the
 second (B) being the highest.
 * As a bounding box is a mathematical entity describing the lowest and highest
 points, whether these points are in the top-left corner or
 * lower-right corner depends on the axes we use.
 * Below is a crude ASCII-art description of the axes we use in our 3D world and
 the points where the min and max are thus positioned.
 * Through the rest of this class's documentation, it will be assumed that this
 is the convention.
 *
 *
 * @verbatim
...
struct TLP_SCOPE BoundingBox : public Array<Vec3f, 2> {
   ...
   * @param compute indicates whether the bounding box has to be computed with
   *the min/max args
   *
   **/
   ...
   **/
  inline void init(const tlp::Vec3f &coord) { (*this)[0] = (*this)[1] = coord; }
  ...
  /**
   * @brief Expands the bounding box to one containing the point passed as
   *parameter. If the parameter is inside the bounding box, it remains
   *unchanged.
   *
  ...
  /**
   * @brief Expands the bounding box to one containing all the point of the
   *vector passed as parameter.
   *
   * @param coords A vector of point in the 3D space we want the bounding box to
   *encompass.
   * @return void
   **/
  ...
  /**
   * @brief Expands the bounding box to one containing the bounding box passed
   *as parameter. If the parameter is inside the bounding box, it remains
   *unchanged.
   *
  ...
  /**
   * @brief Translates the bounding box by the displacement given by the vector
   *passed as parameter.
   *
   * @param vec The displacement vector in 3D space to translate this bounding
   *box by.
   * @return void
   **/
  ...
  /**
   * @brief Scales the bounding box, i.e. multiplying its components by a vector
   *passed as parameter.
   *
  ...
  /**
   * @brief Checks whether the bounding box's lowest point is less than it's
   *highest point. "Less Than" means axis-by-axis comparison, i.e. x1 < x2 && y1
   *< y2 && z1 < z2.
   *
  ...
  /**
   * @brief Checks if the given vector is inside the current bounding box. If
   *the bounding box is invalid the result is always false.
   * @param coord A point in the 3D space.
   * @param noCheck if true current bounding box validity is not checked
  ...
  /**
   * @brief Checks if the given bounding box is inside the current bounding box.
   *If one of the bounding boxes is invalid the result is always false.
  ...
  /**
   * @brief Checks if the given bounding box intersects the current one. If one
   *of the bounding box is invalid return false.
  ...
  /**
   * @brief Checks if the bounding box intersects a given line segment. If the
   *bounding box is invalid the result is always false.
   * @param segStart the start point of the line segment on which to check
   *intersection
   * @param segEnd the end point of the line segment on which to check
   *intersection
  ...
  /**
  * @brief The vector passed as parameter is modified to contain the 8 points of
  the bounding box.
