...
template <typename TYPE> struct StoredType {
  ...
  enum { isPointer = 0 };
  // simply get
  inline static TYPE &get(const TYPE &val) { return const_cast<TYPE &>(val); }
  // equality test
  inline static bool equal(const TYPE &val1, const TYPE &val2) {
    return val2 == val1;
  }
  // cloning before storage
  inline static Value clone(const TYPE &val) { return val; }
  // destruction of stored value
  inline static void destroy(Value) {}
  // the default value of that type
  inline static Value defaultValue() { return static_cast<Value>(0); }
};

#define DECL_STORED_PTR(T)                                                     \
  template <> struct StoredType<T> {                                           \
    typedef T Value;                                                           \
    ...
    enum { isPointer = 1 };                                                    \
                                                                               \
    inline static T get(T val) { return val; }                                 \
                                                                               \
    inline static bool equal(const T val1, const T val2) {                     \
      return val2 == val1;                                                     \
    }                                                                          \
                                                                               \
    inline static T clone(T val) { return val; }                               \
                                                                               \
    inline static void destroy(T val) { delete val; }                          \
    inline static T defaultValue() { return nullptr; }                         \
  }
...
#define DECL_STORED_STRUCT(T)                                                  \
  template <> struct StoredType<T> {                                           \
    ...
    enum { isPointer = 1 };                                                    \
                                                                               \
    inline static T &get(const Value &val) { return *val; }                    \
                                                                               \
    ...
    inline static bool equal(const T &val2, Value val1) {                      \
      return val2 == *val1;                                                    \
                                                                               ...
                                                                               \
    inline static Value clone(const T &val) { return new T(val); }             \
                                                                               \
    inline static void destroy(Value val) { delete val; }                      \
    inline static Value defaultValue() { return new T(); }                     \
  };
#else
#define DECL_STORED_STRUCT(T)                                                  \
  template <> struct StoredType<T> {                                           \
