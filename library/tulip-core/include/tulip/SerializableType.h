...
namespace tlp {
template <typename T> class TLP_SCOPE SerializableType : public TypeInterface<T> {
public:
...
class TLP_SCOPE SerializableVectorType : public TypeInterface<std::vector<ELT_TYPE>> {
  static bool readVector(std::istream &is, std::vector<ELT_TYPE> &v, char openChar, char sepChar, char closeChar) {
    v.clear();
...
public:
  ...
  static bool read(std::istream &iss, typename TypeInterface<std::vector<ELT_TYPE>>::RealType &v, char openChar = '(', char sepChar = ',',
                   char closeChar = ')') {
