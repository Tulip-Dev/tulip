...
public:
  ...
  virtual std::vector<edge> getEdges(const node source, const node target, bool directed = true) const;
  ...
  virtual void addEdges(const std::vector<std::pair<node, node>> &edges, std::vector<edge> &addedEdges);
...
//====================================================================================
template <typename PropertyType> PropertyType *GraphDecorator::getProperty(const std::string &name) {
