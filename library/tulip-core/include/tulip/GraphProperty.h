...
typedef AbstractProperty<tlp::GraphType, tlp::EdgeSetType>
 ...
 *
 * @warning This property is mainly used into the meta node engine. Using
 * GraphProperty outside of this system is strongly discouraged since it could
...
public:
  ...
  bool setStringValueToGraphNodes(const std::string &v,
                                  const tlp::Graph *graph) override;
  bool setEdgeStringValue(const edge e, const std::string &v) override;
  bool setAllEdgeStringValue(const std::string &v) override;
  bool setStringValueToGraphEdges(const std::string &v,
                                  const tlp::Graph *graph) override;
  ...
  unsigned int nodeValueSize() const override { return sizeof(unsigned int); }
