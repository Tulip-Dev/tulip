...
/**
 * @ingroup Checks
 * @brief @brief Performs a test of connexity on the graph, and provides a
 *function to make a graph connected. From Wikipedia: "A graph is said to be
 *connected if every pair of vertices in the graph are connected." (i.e. there
 *is a path between every pair of vertices).
 **/
class TLP_SCOPE ConnectedTest {
public:
  /**
   * @brief Checks if a graph is connected (i.e. there is a path between every
   *pair of vertices).
   *
   ...
   *
   * @param graph The graph in which to count the number of connected
   *components.
  ...
  /**
   * @brief Computes the sets of connected components and stores the result in
   *the components vector.
   *
   * @param graph The graph on which to compute connected components.
   * @param components The components that were found. It is passed as a
   *reference to avoid copying the data when returning.
   * @return void
   * @note The components parameter can be returned with c++11 thanks to move
   *constructors without performance loss, change this function once c++11
   *compilers are used.
   **/
  static void
  computeConnectedComponents(const Graph *graph,
