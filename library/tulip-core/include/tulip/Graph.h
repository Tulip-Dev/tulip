 ...
 * @param progress A PluginProgress to report the progress of the operation, as well as final state. Defaults to nullptr.
 * @return bool Whether the export was successfull or not.
 **/
 ...
 * Returns the graph with imported data, or nullptr if the import failed. In this case, the Pluginprogress should have an error that can be displayed.
 *
 * @param format The format to use to import the graph.
 * @param dataSet The parameters to pass to the import plugin (file to read, ...)
 * @param progress A PluginProgress to report the progress of the operation, as well as final state. Defaults to nullptr.
 * @param newGraph The graph to import the data into. This can be usefull to import data into a subgraph. Defaults to nullptr.
...
public:
   ...
   * @return :Graph* The newly created clone subgraph. nullptr will be returned if addSibling is set to true and this graph is a root graph.
   **/
   ...
   * @return A graph with the given id, or nullptr if no such graph exists in this graph's descendants.
  ...
  */
   ...
   * Graph::delLocalProperty().
   *
   * @warning using the wrong propertyType will result in a segmentation fault. Using an invalid property type will always return nullptr.
   * @param name The unique identifier of the property.
