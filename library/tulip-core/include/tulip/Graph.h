 ...
 *import plugins.
 * Since Tulip 4.8, the selection of the import plugin is based on the provided
 *filename extension.
 * The import will fail if the selected import plugin is not loaded.
 * The graph file formats that can currently be imported are : TLP (*.tlp,
 **.tlp.gz), TLP Binary (*.tlpb, *.tlpb.gz), TLP JSON (*.json),
 * Gephi (*.gexf), Pajek (*.net, *.paj), GML (*.gml), Graphviz (*.dot) and
 *UCINET (*.txt)
 *
 ...
 *Tulip supported ouput graph file format)..
 *
 * This function serializes the corresponding graph and all its subgraphs
 *(depending on the format) to a file
 * through the available Tulip export plugins.
 * Since Tulip 4.8, the selection of the export plugin is based on the provided
 *filename extension.
 * The export will fail if the selected export plugin is not loaded.
 * The file formats the graph can be exported to are : TLP (*.tlp, *.tlp.gz),
 *TLP Binary (*.tlpb, *.tlpb.gz),
 * TLP JSON (*.json) and GML (*.gml)
 *
 * This function checks the file name for the '.gz' extension and uses a
 *compressed output if supported (TLP and TLP Binary only).
 *
 ...
 * @param dataSet The parameters to pass to the export plugin (e.g. additional
 *data, options for the format)
 ...
 *in order to import data into it.
 * Returns the graph with imported data, or nullptr if the import failed. In
 ...
 * edges).
 * If no selection is done (inSel=nullptr), the whole graph is reseted to
 * default value.
 ...
 *exists in each of its parent graphs.
 * This is also true of properties; every property in a graph exist in all of
 *its subgraphs, except if it has been replaced
 * by a local property.
 *
 ...
 *triangulation.
 * You need to create a subgraph that is a clone of the original (say this is A)
 *to keep the original graph,
 * and another copy (say this one is B) on which you will perform the delaunay
 *triangulation.
 *
 ...
 *property local.
 * This results in A and B having different values for the layout, but
 *everything else in common.
 * You then can apply two different algorithms on A and B (e.g. Bubble Tree and
 *Tree Radial).
 *
 * @chapter Meta Nodes
...
public:
   ...
   *
   * If a BooleanProperty is provided, only nodes and edges for which it is true will be added to the subgraph.
   ...
   * the ancestor graphs.
   * This node must exists in the graph hierarchy (which means it must exist in
   * the root graph).
   * You cannot add a node to the root graph this way (as it must already be an
  ...
  */
   ...
   * These nodes are also removed in the sub-graphs hierarchy of the current
   * graph.
   * @warning the graph does not take ownership of the Iterator.
   ...
   * can now be used by another property.
  ...
  /**
   * @brief Gets a reference on a boolean property of this graph or one of its
   * ancestors.
   * The name of a property identifies it uniquely.
   * Either there already exists a property with the given name, in which case
   * it is returned.
  ...
  /**
   * @brief Gets a reference on a color property of this graph or one of its
   * ancestors.
   * The name of a property identifies it uniquely.
   * Either there already exists a property with the given name, in which case
   * it is returned.
  ...
  /**
   * @brief Gets a reference on a double property of this graph or one of its
   * ancestors.
   * The name of a property identifies it uniquely.
   * Either there already exists a property with the given name, in which case
   * it is returned.
  ...
  /**
   * @brief Gets a reference on an integer property of this graph or one of its
   * ancestors.
   * The name of a property identifies it uniquely.
   * Either there already exists a property with the given name, in which case
   * it is returned.
  ...
  /**
   * @brief Gets a reference on a layout property of this graph or one of its
   * ancestors.
   * The name of a property identifies it uniquely.
   * Either there already exists a property with the given name, in which case
   * it is returned.
  ...
  /**
   * @brief Gets a reference on a size property of this graph or one of its
   * ancestors.
   * The name of a property identifies it uniquely.
   * Either there already exists a property with the given name, in which case
   * it is returned.
  ...
  /**
   * @brief Gets a reference on a string property of this graph or one of its
   * ancestors.
   * The name of a property identifies it uniquely.
   * Either there already exists a property with the given name, in which case
   * it is returned.
  ...
  /**
   * @brief Gets a reference on a boolean vector property of this graph or one
   * of its ancestors.
   * The name of a property identifies it uniquely.
   * Either there already exists a property with the given name, in which case
   * it is returned.
  ...
  /**
   * @brief Gets a reference on a color vector property of this graph or one of
   * its ancestors.
   * The name of a property identifies it uniquely.
   * Either there already exists a property with the given name, in which case
   * it is returned.
  ...
  /**
   * @brief Gets a reference on a double vector property of this graph or one of
   * its ancestors.
   * The name of a property identifies it uniquely.
   * Either there already exists a property with the given name, in which case
   * it is returned.
  ...
  /**
   * @brief Gets a reference on an integer vector property of this graph or one
   * of its ancestors.
   * The name of a property identifies it uniquely.
   * Either there already exists a property with the given name, in which case
   * it is returned.
  ...
  /**
   * @brief Gets a reference on a coord vector property of this graph or one of
   * its ancestors.
   * The name of a property identifies it uniquely.
   * Either there already exists a property with the given name, in which case
   * it is returned.
  ...
  /**
   * @brief Gets a reference on a size vector property of this graph or one of
   * its ancestors.
   * The name of a property identifies it uniquely.
   * Either there already exists a property with the given name, in which case
   * it is returned.
  ...
  /**
   * @brief Gets a reference on a string vector property of this graph or one of
   * its ancestors.
   * The name of a property identifies it uniquely.
   * Either there already exists a property with the given name, in which case
   * it is returned.
   ...
   * @param result The property in which to put the results. All previous values
   * will be erased.
   * @param errorMessage Stores the error message if the plugin fails.
   * @param progress A means of feedback during the plugin execution. Some
   * plugins support being stopped or cancelled through this.
   * @param parameters The parameters of the algorithm. Some algorithms use this
   * DataSet to output some additional information.
   * @return Whether the plugin executed successfully or not. If not, check the
   * error message.
   *
   ...
   * revert to this state later on, using pop().
   * All modifications except those altering the ordering of the edges will be
   * undone.
   *
   * This allows to undo/redo modifications on a graph.
   * This is mostly useful from a user interface point of view, but some
   * algorithms use this mechanism to clean up before finishing.
   * For instance:
   * @code
...
//================================================================================
// these functions allow to use tlp::Graph as a key in a hash-based data
