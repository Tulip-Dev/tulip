 ...
 * \param pascalTriangle a vector of vector of double to store the result. If that vector already contains m Pascal triangle rows and n > m, the first
 * m row are not recomputed and the vector is expanded to store the new rows.
 */
 ...
 *      -> the spline is not C2 continuous.  The second derivative is linearly interpolated within each segment, causing the curvature to vary
 ...
 * \param alpha curve parameterization parameter (0 <= alpha <= 1), alpha = 0 -> uniform parameterization, alpha = 0.5 -> centripetal
 * parameterization, alpha = 1.0 -> chord-length parameterization
 */
TLP_SCOPE Coord computeCatmullRomPoint(const std::vector<Coord> &controlPoints, const float t, const bool closedCurve = false,
 ...
 *  \param nbCurvePoints number of points to generate
 */
TLP_SCOPE void computeCatmullRomPoints(const std::vector<Coord> &controlPoints, std::vector<Coord> &curvePoints, const bool closedCurve = false,
                                       const unsigned int nbCurvePoints = 100, const float alpha = 0.5);
 ...
 *  \param nbCurvePoints number of points to generate
 */
TLP_SCOPE void computeOpenUniformBsplinePoints(const std::vector<Coord> &controlPoints, std::vector<Coord> &curvePoints,
