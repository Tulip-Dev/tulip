...
#define MINMAX_PAIR(TYPE)                                                      \
  std::pair<typename TYPE::RealType, typename TYPE::RealType>
#define MINMAX_MAP(TYPE)                                                       \
  typename std::unordered_map<unsigned int, MINMAX_PAIR(TYPE)>

namespace tlp {

/**
 * @brief Abstracts the computation of minimal and maximal values on node and
 *edge values of properties.
 *
 * The value is lazily computed on first request.
 * The value is cached, and the cache is invalidated whenever it cannot be
 *simply updated.
 **/
template <typename nodeType, typename edgeType,
          typename propType = PropertyInterface>
class MinMaxProperty
    : public tlp::AbstractProperty<nodeType, edgeType, propType> {
public:
  /**
   ...
   * @param NodeMin The minimal value the property can take for nodes (e.g.
   *INT_MIN)
   * @param NodeMax The maximal value the property can take for nodes (e.g.
   *INT_MIN)
   * @param EdgeMin The minimal value the property can take for edges (e.g.
   *INT_MIN)
   * @param EdgeMax The maximal value the property can take for edges (e.g.
   *INT_MIN)
   **/
  MinMaxProperty(tlp::Graph *graph, const std::string &name,
   ...
   * It is only computed if it has never been retrieved before, or if the cached
  ...
  /**
   * @brief Updates the value on a node, and updates the minimal/maximal cached
   *values if necessary. Should be called by subclasses in order to keep the
   *cache up to date.
   *
  ...
  /**
   * @brief Updates the value on an edge, and updates the minimal/maximal cached
   *values if necessary. Should be called by subclasses in order to keep the
   *cache up to date.
   *
  ...
  /**
   * @brief Updates the value of all nodes, setting the maximum and minimum
   *values to this. Should be called by subclasses in order to keep the cache up
   *to date.
   *
  ...
  /**
   * @brief Updates the value of all edges, setting the maximum and minimum
   *values to this. Should be called by subclasses in order to keep the cache up
   *to date.
   *
   * @param newValue The new maximal and minimal value.
