 ...
 * for 2D, triangles for 3D) and a list of neighbors facets for each facet. A
 * facet is defined by a list of indexes in the points vector. The neighbors
 * facets of a facet are defined by a list of indexes in the convexHullFacets
 * vector (facetNeighbors[i] stores the neighbors of facet convexHullFacets[i]).
 *
 * The convexHullFacets and facetNeighbors vectors are automatically cleared.
 */
TLP_SCOPE void
convexHull(const std::vector<Coord> &points,
