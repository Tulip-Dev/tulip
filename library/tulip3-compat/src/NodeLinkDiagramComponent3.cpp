 ...
 *
 */
#include "tulip3/NodeLinkDiagramComponent3.h"
...
VIEWPLUGIN(NodeLinkDiagramComponent3, "Node Link Diagram view (Tulip 3)", "Tulip Team", "16/04/2008", "Node link diagram", "1.0")

//==================================================
NodeLinkDiagramComponent3::NodeLinkDiagramComponent3():GlMainView3(),qtMetaNode(true),currentMetaNodeRenderer(NULL) {
}
//==================================================
NodeLinkDiagramComponent3::~NodeLinkDiagramComponent3() {
}
//==================================================
QWidget *NodeLinkDiagramComponent3::construct(QWidget *parent) {
  QWidget *widget=GlMainView3::construct(parent);
...
// Accessor and set
//==================================================
void NodeLinkDiagramComponent3::setData(Graph *graph,DataSet dataSet) {
...
//==================================================
void NodeLinkDiagramComponent3::getData(Graph **graph,DataSet *dataSet) {
  dataSet->set<DataSet>("data",mainWidget->getData());
  *graph=mainWidget->getGraph();
}
//==================================================
void NodeLinkDiagramComponent3::setGraph(Graph *graph,bool initView) {
...
//==================================================
void NodeLinkDiagramComponent3::changeGraph(Graph *graph) {
...
//==================================================
std::list<std::pair<QWidget *,std::string> > NodeLinkDiagramComponent3::getConfigurationWidget() {
...
void NodeLinkDiagramComponent3::specificEventFilter(QObject *,QEvent *event) {
...
void NodeLinkDiagramComponent3::buildContextMenu(QObject *object,QContextMenuEvent *event,QMenu *contextMenu) {
  contextMenu->addMenu(viewMenu);
  contextMenu->addMenu(optionsMenu);

  GlMainView3::buildContextMenu(object,event,contextMenu);
...
void NodeLinkDiagramComponent3::computeContextMenuAction(QAction *action) {
  Graph *graph=mainWidget->getScene()->getGlGraphComposite()->getInputData()->getGraph();
...
void NodeLinkDiagramComponent3::checkAlgorithmResult() {
  if(!mainWidget->getScene()->getGlGraphComposite())
    return;

  Graph *graph=mainWidget->getScene()->getGlGraphComposite()->getInputData()->getGraph();

  if(graph->attributeExist("NodeLinkDiagramComponent3")) {
    DataSet NodeLinkDiagramComponent3DataSet;
    graph->getAttribute("NodeLinkDiagramComponent3",NodeLinkDiagramComponent3DataSet);

    //remove old info
    std::list<std::string> toRemove;

    for(std::map<std::string,DataSet>::iterator it=algorithmInfoDataSet.begin(); it!=algorithmInfoDataSet.end(); ++it) {
      if(!NodeLinkDiagramComponent3DataSet.exist((*it).first)) {
    ...
    Iterator< std::pair<std::string, DataType*> > *infoDataSetIt=NodeLinkDiagramComponent3DataSet.getValues();
    ...
    for(vector<string>::iterator it=toRemove.begin();it!=toRemove.end();++it){
      NodeLinkDiagramComponent3DataSet.remove(*it);
    }
    graph->setAttribute("NodeLinkDiagramComponent3",NodeLinkDiagramComponent3DataSet);*/
  }
  else {
    for(std::map<std::string,DataSet>::iterator it=algorithmInfoDataSet.begin(); it!=algorithmInfoDataSet.end(); ++it) {
      DataSet layerAndCompositeDataSet=(*it).second;
...
// GUI functions
//==================================================
void NodeLinkDiagramComponent3::drawAfterRenderingParametersChange() {
  draw();
}
//==================================================
void NodeLinkDiagramComponent3::draw() {
...
//==================================================
void NodeLinkDiagramComponent3::refresh() {
  draw();
}
//==================================================
void NodeLinkDiagramComponent3::init() {
  for(std::map<std::string,DataSet>::iterator it=algorithmInfoDataSet.begin(); it!=algorithmInfoDataSet.end(); ++it) {
    std::string layerName;
...
//==================================================
void NodeLinkDiagramComponent3::centerView() {
  mainWidget->getScene()->centerScene();
  overviewWidget->getView()->getScene()->centerScene();
  draw();
}
//==================================================
void NodeLinkDiagramComponent3::showDialog(QAction* action) {
  GlMainView3::showDialog(action);

  std::string name(action->text().toStdString());

  if(name =="Augmented Display") {
    Graph *graph=mainWidget->getScene()->getGlGraphComposite()->getInputData()->getGraph();
    AugmentedDisplayDialog dialog(widget,graph,"NodeLinkDiagramComponent3");

    if(dialog.exec()==QDialog::Accepted) {
      std::vector<std::string> toRemove;
      dialog.getRemovedList(toRemove);
      DataSet nldcDataSet;
      graph->getAttribute("NodeLinkDiagramComponent3",nldcDataSet);
      ...
      graph->setAttribute("NodeLinkDiagramComponent3",nldcDataSet);
      mainWidget->draw();
    }
  }
}
//==================================================
void NodeLinkDiagramComponent3::gridOptions() {
  if (gridOptionsWidget == 0)
